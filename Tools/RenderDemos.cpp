...
// what YouKnow actually sounds like. No samples or external processing are
...
using youknow::EnvPolarity;
using youknow::HighPassMode;
using youknow::KeyMode;
using youknow::PwmSource;
...
// YouKnow is a stereo instrument by construction - its two channels are the
...
int runSmokeTest (const std::filesystem::path& directory)
{
    ...
    std::printf ("YouKnow demo renderer smoke test passed (peak %.3f).\n",
                 take.peak());
    return 0;
}
...
constexpr const char* peaksTableBegin =
    "<!-- peaks-table-begin: regenerated by YouKnowRenderDemos;"
...
int main (int argc, char** argv)
{
    ...
    for (const auto& argument : arguments)
    {
        ...
        {
            std::printf ("usage: YouKnowRenderDemos [--smoke] [output-directory]\n");
            return 0;
        }
        else
        {
