...
//
// Usage:
//   YouKnowRenderRealismComparison retrigger-release-tail before <output-dir>
//   YouKnowRenderRealismComparison common-vca-level before <output-dir>
//   YouKnowRenderRealismComparison bbd-transfer-clock-law before <output-dir>
//   YouKnowRenderRealismComparison voice-vca-feedthrough before <output-dir>
//   YouKnowRenderRealismComparison bbd-host-grid-alias before <output-dir>
//   ... make and rebuild one DSP change ...
//   YouKnowRenderRealismComparison <scenario> after <output-dir>
//
...
using youknow::VcaMode;
using youknow::YouKnowEngine;
using youknow::YouKnowTestAccess;
using youknow::tools::realism::Level;
using youknow::tools::realism::StereoBuffer;
using youknow::tools::realism::applyGain;
using youknow::tools::realism::comparisonBlockSize;
using youknow::tools::realism::comparisonSampleRate;
using youknow::tools::realism::decibels;
using youknow::tools::realism::difference;
using youknow::tools::realism::jsonNumber;
using youknow::tools::realism::listeningTargetPeak;
using youknow::tools::realism::measure;
using youknow::tools::realism::readFloatWav;
using youknow::tools::realism::updateGeneratedSection;
using youknow::tools::realism::validate;
using youknow::tools::realism::writeFloatWav;
using youknow::tools::realism::writeText;
...
RenderedScenario renderCommonVcaLevel()
{
    ...
    const auto setVcaByte = [&](std::uint8_t storedByte, const char* section,
        ...
        if (YouKnowEngine::storedControlDacCode(parameters.vcaLevel)
...
RenderedScenario renderBbdHostGridAlias()
{
    ...
    for (const bool highQuality : { true, false })
    {
        appendSection(
            ...
            YouKnowTestAccess::renderFixedBbdProbe(
    ...
    for (const auto& item : sweepCases)
    {
        ...
        appendSection(
            ...
            1.0, realisedCycles,
            YouKnowTestAccess::renderSweptBbdProbe(
    ...
    std::string& error)
{
    const std::array<std::string, 8> required {{
        "  \"schema_version\": 1,",
        "  \"tool\": \"YouKnowRenderRealismComparison\",",
    ...
    const std::string_view currentFingerprint = YOUKNOW_DSP_SOURCE_SHA256;
                                 ...
                                 const RenderedScenario& rendered)
{
    ...
    output << "{\n"
           ...
           << "  \"stage\": \"" << stage << "\",\n"
           << "  \"dsp_source_sha256\": \"" << YOUKNOW_DSP_SOURCE_SHA256 << "\",\n"
           << "  \"compiler_id\": \""
           << jsonEscape(YOUKNOW_COMPARISON_COMPILER_ID) << "\",\n"
           << "  \"compiler_version\": \""
           << jsonEscape(YOUKNOW_COMPARISON_COMPILER_VERSION) << "\",\n"
...
std::string readmeProse()
{
    return
        ...
        "change:\n\n"
        "```bash\n"
        "cmake --build build-dsp --parallel --target YouKnowRenderRealismComparison\n"
        "./build-dsp/YouKnowRenderRealismComparison retrigger-release-tail before \\\n"
        "  Docs/audio/realism-comparisons/retrigger-release-tail\n"
        "# Apply and rebuild the DSP change, then:\n"
        "./build-dsp/YouKnowRenderRealismComparison retrigger-release-tail after \\\n"
        "  Docs/audio/realism-comparisons/retrigger-release-tail\n"
        "```";
}
...
std::string commonVcaReadmeProse()
{
    return
        ...
        "```bash\n"
        "cmake --build build-dsp --parallel --target YouKnowRenderRealismComparison\n"
        "./build-dsp/YouKnowRenderRealismComparison common-vca-level before \\\n"
        "  Docs/audio/realism-comparisons/common-vca-level\n"
        "# Apply and rebuild the DSP change, then:\n"
        "./build-dsp/YouKnowRenderRealismComparison common-vca-level after \\\n"
        "  Docs/audio/realism-comparisons/common-vca-level\n"
        "```";
}
...
std::string bbdTransferReadmeProse()
{
    return
        ...
        "```bash\n"
        "cmake --build build-dsp --parallel --target YouKnowRenderRealismComparison\n"
        "./build-dsp/YouKnowRenderRealismComparison bbd-transfer-clock-law before \\\n"
        "  Docs/audio/realism-comparisons/bbd-transfer-clock-law\n"
        "# Apply and rebuild the single Chorus change, then:\n"
        "./build-dsp/YouKnowRenderRealismComparison bbd-transfer-clock-law after \\\n"
        "  Docs/audio/realism-comparisons/bbd-transfer-clock-law\n"
        "```";
}
...
std::string voiceVcaFeedthroughReadmeProse()
{
    return
        ...
        "```bash\n"
        "cmake --build build-dsp --parallel --target YouKnowRenderRealismComparison\n"
        "./build-dsp/YouKnowRenderRealismComparison voice-vca-feedthrough before \\\n"
        "  Docs/audio/realism-comparisons/voice-vca-feedthrough\n"
        "# Apply and rebuild the single DSP change, then:\n"
        "./build-dsp/YouKnowRenderRealismComparison voice-vca-feedthrough after \\\n"
        "  Docs/audio/realism-comparisons/voice-vca-feedthrough\n"
        "```";
}
...
std::string bbdHostGridAliasReadmeProse()
{
    return
        ...
        "```bash\n"
        "cmake --build build-dsp --parallel --target YouKnowRenderRealismComparison\n"
        "./build-dsp/YouKnowRenderRealismComparison bbd-host-grid-alias before \\\n"
        "  Docs/audio/realism-comparisons/bbd-host-grid-alias\n"
        "# Apply and rebuild exactly one future BBD host-grid change, then:\n"
        "./build-dsp/YouKnowRenderRealismComparison bbd-host-grid-alias after \\\n"
        "  Docs/audio/realism-comparisons/bbd-host-grid-alias\n"
        "```";
}
                 ...
                 const RenderedScenario& rendered, std::string& error)
{
    ...
    if (std::filesystem::exists(rawPath))
    {
        ...
        if (!readFloatWav(rawPath, archived, error))
            return false;
        if (!youknow::tools::realism::buffersEqual(archived, rendered.audio))
        {
...
bool runBbdAliasBaselineContractSelfTest(std::string& error)
{
    ...
    const std::string currentFingerprint = YOUKNOW_DSP_SOURCE_SHA256;
    if (!isLowerHex(currentFingerprint, 64u))
    {
