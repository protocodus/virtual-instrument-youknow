...
- The original Juno-106 control path has one chorus-enable line and one binary
  ...
  MN3101 clock oscillators through D3/R41/R47/C15/R46 into Tr23/Tr28 (p. 15),
  which clamps the clocks about 0.17 s after the return has muted and
  releases them about 35 ms after the button comes on, 78 ms before the
  return opens — so the BBDs do stop behind the mute, and what the return
  reopens onto is freshly clocked content. The Exact reference keeps the
