...
struct YouKnowTestAccess
{
    ...
    static float boostBranchStep(YouKnowEngine& engine, float coupled,
                                 bool selected) noexcept
    {
        return engine.processBoostBranch(coupled, selected);
    }

    static double boostBranchNodeVolts(const YouKnowEngine& engine) noexcept
    {
        return engine.boostBranch_.vN;
    }

    static double engineProcessingRate(const YouKnowEngine& engine) noexcept
    {
        return engine.oversampledRate_;
    }

    static float voicePulseDuty(const YouKnowEngine& engine, int voice) noexcept
    {
        return engine.voices_[static_cast<std::size_t>(voice)].pulseDuty;
    }

    static int voiceCard(const YouKnowEngine& engine, int voice) noexcept
    {
    ...
    {
        return YouKnowEngine::pwmDutyAcceptanceHalfWidth;
    }

    static constexpr float headroom() noexcept
    {
        return YouKnowEngine::otaHeadroomVolts;
    }

    static constexpr float thermalVoltage() noexcept
    {
        return YouKnowEngine::thermalVoltage;
    }
    ...
    {
        return YouKnowEngine::stageAttenuation;
    }
    ...
    {
        YouKnowEngine::HighPass coupling;
        coupling.reset();
        const float g = std::tan(
            static_cast<float>(3.14159265358979323846)
            * YouKnowEngine::outputCouplingCornerHz()
            / static_cast<float>(sampleRate));
        std::vector<float> output(input.size());
        for (std::size_t index = 0; index < input.size(); ++index)
            output[index] = coupling.process(
                input[index], g, 0.0f,
                YouKnowEngine::outputCouplingHighGain());
        return output;
    }
        ...
        float volumePosition)
    {
        ...
        const float g = std::tan(
            static_cast<float>(3.14159265358979323846)
            * YouKnowEngine::outputCouplingCornerHz(volumePosition)
            / static_cast<float>(sampleRate));
        const float gain =
            YouKnowEngine::outputCouplingHighGain(volumePosition);
        std::vector<float> output(input.size());
        for (std::size_t index = 0; index < input.size(); ++index)
        ...
        const std::vector<float>& input, double sampleRate)
    {
        ...
        const float g = std::tan(
            static_cast<float>(3.14159265358979323846)
            * YouKnowEngine::voiceBusCouplingCornerHz()
            / static_cast<float>(sampleRate));
        std::vector<float> output(input.size());
        for (std::size_t index = 0; index < input.size(); ++index)
        ...
        HighPassMode mode)
    {
        ...
        const float g = std::tan(
            static_cast<float>(3.14159265358979323846)
            * YouKnowEngine::voiceBusCouplingCornerHz(mode)
            / static_cast<float>(sampleRate));
        std::vector<float> output(input.size());
        for (std::size_t index = 0; index < input.size(); ++index)
            output[index] = coupling.process(input[index], g, 0.0f, 1.0f);
    ...
    static void setNoiseSourceSupportState(
        YouKnowEngine& engine, double highPass, double lowPass) noexcept
    {
    ...
    static float processMainNoiseSource(
        YouKnowEngine& engine, float input, float level,
    ...
    static void forceNoiseSourceProcessingQuality(
        YouKnowEngine& engine, bool enabled) noexcept
    {
        ...
        const std::vector<float>& input, double sampleRate)
    {
        ...
        const float g = std::tan(
            static_cast<float>(3.14159265358979323846)
            * YouKnowEngine::commonVcaInputCouplingCornerHz()
            / static_cast<float>(sampleRate));
        std::vector<float> output(input.size());
        for (std::size_t index = 0; index < input.size(); ++index)
            output[index] = coupling.process(input[index], g, 0.0f, 1.0f);
...
struct ReferenceBbdCore
{
    ...
    float process(float input, float clockHz, float sampleRate)
    {
        ...
        while (clockPhase >= 1.0)
        {
            ...
            const float bounded = YouKnowTestAccess::bbdTransfer(atEdge);
...
void testCascadeAgainstReferenceSolve()
{
    ...
    for (double cutoff : { 120.0, 900.0, 4200.0 })
    {
        for (double feedback : { 0.0, 2.0, 3.0, 3.8 })
        {
            ...
            const auto model = YouKnowTestAccess::renderCascade(
    ...
    const auto kept = YouKnowTestAccess::cascadeVoltagesAfterRetime(
        voltage, 0.03125f, 0.125f);
    for (std::size_t stage = 0; stage < voltage.size(); ++stage)
        expectNear(kept[stage], voltage[stage], 0.0,
                   "HQ retiming disturbed OTA capacitor charge "
                       + std::to_string(stage));
    expect(YouKnowTestAccess::cascadeRetimeStepError(0.03125f, 0.125f)
               < 1.0e-5f,
           "a settled cascade stepped when the numerical grid refined");
    expect(YouKnowTestAccess::cascadeRetimeStepError(0.125f, 0.03125f)
               < 1.0e-5f,
           "a settled cascade stepped when the numerical grid coarsened");
    expect(YouKnowTestAccess::cascadeIdentityRetimeError() == 0.0f,
...
void testCascadeOscillationThreshold()
{
    ...
    const auto ring = [&](float feedback) {
        ...
        input[0] = 0.5f;
        const auto output = YouKnowTestAccess::renderCascade(
...
void testCascadeSurvivesAdversarialControl()
{
    ...
    YouKnowTestAccess::Cascade cascade;
    cascade.reset();
    bool finite = true;
    for (std::size_t index = 0; index < input.size(); ++index)
    {
...
void testNoteTimerLaw()
{
    ...
    {
        const auto pair = YouKnowEngine::dcoPitchPair(vector.word);
    ...
    expect(YouKnowEngine::dcoPitchPair(0x0000u).divider
               == YouKnowEngine::dcoPitchPair(0x2fffu).divider
               && YouKnowEngine::dcoPitchPair(0xffffu).divider
                      == YouKnowEngine::dcoPitchPair(0x9700u).divider,
           "the B-2 lower or upper coordinate clamp retained a fraction");
    expect(YouKnowEngine::dcoPitchPair(0x96ffu).divider
               < YouKnowEngine::dcoPitchPair(0x9700u).divider,
           "the real B-2 upper-clamp count discontinuity disappeared");

    for (const auto& item : cases)
    {
        expectNear(YouKnowEngine::rangeClockHz(item.range), item.clock, 1.0,
                   "range divider clock");
    ...
    const auto floored = YouKnowEngine::dcoDivider(8.0);
    expectNear(YouKnowEngine::dcoQuantisedFrequency(floored, DcoRange::Sixteen),
               16.25, 0.02, "the 16' range does not use the B-2 low clamp");
    expect(YouKnowEngine::dcoQuantisedFrequency(
               YouKnowEngine::dcoDivider(4.0), DcoRange::Sixteen)
           == YouKnowEngine::dcoQuantisedFrequency(floored, DcoRange::Sixteen),
    ...
    expect(YouKnowEngine::dcoDivider(261.63)
           == YouKnowEngine::dcoDivider(261.63),
...
void testNoteTimerDividerDefensiveGuard()
{
    ...
    expect(YouKnowEngine::dcoDivider(0.0) == maximumDivider,
           "a zero frequency did not saturate to the widest divider");
    expect(YouKnowEngine::dcoDivider(-440.0) == maximumDivider,
           "a negative frequency did not saturate to the widest divider");
    expect(YouKnowEngine::dcoDivider(
...
void testCutoffControlLaw()
{
    ...
    expectNear(YouKnowEngine::vcfCutoffHz(6272.0f), 248.0, 1.0,
               "cutoff law misses the service calibration anchor");
    expectNear(YouKnowEngine::vcfCutoffHz(6272.0f + 2286.0f), 992.0, 4.0,
    ...
    for (float counts : { 0.0f, 2000.0f, 6000.0f, 9000.0f })
        expectNear(YouKnowEngine::vcfCutoffHz(counts + 1143.0f)
                       / YouKnowEngine::vcfCutoffHz(counts),
                   2.0, 0.01, "cutoff law is not 1143 counts per octave");

    expectNear(YouKnowEngine::vcfCutoffHz(0.0f), 5.53, 0.01,
    ...
    expectNear(YouKnowEngine::vcfPanelCounts(1.0f), 16256.0, 0.5,
               "cutoff panel travel");
    expectNear(YouKnowEngine::vcfPanelCounts(0.0f), 0.0, 0.5,
               "cutoff panel travel starts at zero");
    expect(YouKnowEngine::vcfPanelCounts(0.5f)
               == YouKnowEngine::vcfPanelCounts(0.503f),
    ...
    {
        const double exponential = YouKnowEngine::vcfBaseFrequencyHz
            * std::exp2(counts / YouKnowEngine::vcfCountsPerOctave);
        expectNear(YouKnowEngine::vcfCutoffHz(counts), exponential,
                   std::max(1.0e-4, exponential * 2.0e-6),
                   "50 kHz policy altered the validated exponential range");
    }

    float previousCutoff = YouKnowEngine::vcfCutoffHz(0.0f);
    for (int counts = 1; counts <= 20000; ++counts)
    {
        const float cutoff = YouKnowEngine::vcfCutoffHz(static_cast<float>(counts));
    ...
    expectNear(YouKnowEngine::vcfCutoffHz(20000.0f), 50000.0, 1.0e-3,
               "default cutoff policy never reaches its 50 kHz safety cap");
    expectNear(YouKnowEngine::vcfEffectiveCutoffHz(20000.0f, 8.0f),
               50000.0, 1.0e-3,
    ...
    {
        const float effective = YouKnowEngine::vcfEffectiveCutoffHz(counts, 0.0f);
        const float law = YouKnowEngine::vcfCutoffHz(counts);
    ...
    for (float counts = 0.0f; counts <= 20000.0f; counts += 50.0f)
        expect(YouKnowEngine::vcfEffectiveCutoffHz(counts, 0.0f)
                   <= YouKnowEngine::vcfCutoffHz(counts) + 1.0e-3f,
    ...
    {
        const float counts = point.dacCode * YouKnowEngine::vcfDacCountStep
                           + YouKnowEngine::vcfConverterCarryCounts(
                                 point.dacCode * YouKnowEngine::vcfDacCountStep);
        const double cents = 1200.0 * std::log2(
            YouKnowEngine::vcfEffectiveCutoffHz(counts, 0.0f) / point.hertz);
    ...
    constexpr float perCent = YouKnowEngine::vcfCountsPerOctave / 1200.0f;
    expectNear(YouKnowEngine::vcfConverterCarryCounts(0.0f), 0.0, 1.0e-6,
               "the converter carries an offset below its first bit boundary");
    expectNear(YouKnowEngine::vcfConverterCarryCounts(8192.0f)
                   - YouKnowEngine::vcfConverterCarryCounts(8188.0f),
               23.31 * perCent, 1.0e-4,
               "the major carry at DAC 2048 is not the measured 23.31 cents");
    expectNear(YouKnowEngine::vcfConverterCarryCounts(4096.0f)
                   - YouKnowEngine::vcfConverterCarryCounts(4092.0f),
               -4.64 * perCent, 1.0e-4,
               "the carry at DAC 1024 is not the measured -4.64 cents");
    expectNear(YouKnowEngine::vcfConverterCarryCounts(12288.0f)
                   - YouKnowEngine::vcfConverterCarryCounts(12284.0f),
               -4.48 * perCent, 1.0e-4,
...
void testStoredControlDigitalVectors()
{
    ...
    for (const auto& vector : vectors)
    {
        ...
        expect(YouKnowEngine::storedControlAlignedWord(position)
                   == vector.alignedWord,
               "stored control has the wrong aligned work word at byte "
                   + std::to_string(vector.byte));
        expect(YouKnowEngine::storedControlDacCode(position) == vector.dacCode,
...
{
    using Profile = YouKnowEngine::VoicedResonanceCompatibilityProfile;
    ...
    {
        const double reference = YouKnowEngine::vcfEffectiveCutoffHz(
            counts, Profile::loopGain(0.0f));
        for (const float panel : { 0.00f, 0.30f, 0.50f, 0.70f, 0.80f })
        {
            const double hertz = YouKnowEngine::vcfEffectiveCutoffHz(
...
{
    using Voiced = YouKnowEngine::VoicedResonanceCompatibilityProfile;
    using Derived = YouKnowEngine::CircuitDerivedResonanceProfile;
...
{
    using Profile = YouKnowEngine::CircuitDerivedNoiseLevelProfile;
...
void testEnvelopeAndAmplifierLaws()
{
    ...
    for (const auto& vector : vectors)
    {
        ...
        expect(YouKnowEngine::envelopeAttackIncrement(position) == vector.attack,
               "B-2 attack coefficient vector mismatch at code "
                   + std::to_string(vector.code));
        expect(YouKnowEngine::envelopeDecayReleaseMultiplier(position) == vector.fall,
               "B-2 decay/release coefficient vector mismatch at code "
                   + std::to_string(vector.code));

        std::uint16_t release = YouKnowEngine::envelopePeak;
        int passes = 0;
        while (release != 0u && passes <= vector.releasePasses)
        {
            release = YouKnowEngine::envelopeReleaseLevel(release, vector.fall);
            ++passes;
        }
        expect(passes == vector.releasePasses && release == 0u,
               "B-2 release-to-zero pass count mismatch at code "
                   + std::to_string(vector.code));
        expectNear(YouKnowEngine::envelopeAttackSeconds(position),
                   vector.attackPasses * 0.0042, 1.0e-5,
                   "B-2 attack duration mismatch at code "
                       + std::to_string(vector.code));
        expectNear(YouKnowEngine::envelopeReleaseSeconds(position),
    ...
    expect(YouKnowEngine::envelopeReleaseLevel(0x1234u, 0x5678u) == 0x0625u,
           "decay helper restored the intentionally omitted low-low product");
    expect(YouKnowEngine::envelopeDecayLevel(
               0x1334u, 0x0100u, 0x5678u) == 0x0725u,
           "decay is not sustain plus the exact truncated distance product");
    expect(YouKnowEngine::envelopeAttackLevel(0x3ff0u, 0x0020u) == 0x3fffu,
           "integer attack does not saturate at 14 bits");
    expectNear(YouKnowEngine::envelopeDacFraction(0x0003u), 0.0, 0.0,
               "envelope low recurrence bits leaked into the physical DAC");
    expectNear(YouKnowEngine::envelopeDacFraction(0x0004u), 1.0 / 4095.0,
               1.0e-9, "envelope DAC does not discard exactly two low bits");
    expectNear(YouKnowEngine::envelopeDacFraction(0x3fffu), 1.0, 0.0,
               "envelope DAC does not reach full scale at the 14-bit peak");
    expect(YouKnowTestAccess::attackLevelAfterRetrigger(0x1800u, 0x007fu)
               == 0x187fu,
    ...
    expect(YouKnowEngine::storedControlAlignedWord(0.0f) == 0u
           && YouKnowEngine::storedControlAlignedWord(64.0f / 127.0f) == 0x2000u
           && YouKnowEngine::storedControlAlignedWord(1.0f) == 0x3f80u,
    ...
    expectNear(YouKnowEngine::envelopeDecaySeconds(0.0f), 0.0042, 1.0e-6,
               "fastest decay-to-minus-20-dB is not one pass");
    expectNear(YouKnowEngine::envelopeDecaySeconds(64.0f / 127.0f),
               527.0 * 0.0042, 1.0e-4,
               "mid decay-to-minus-20-dB misses the integer recurrence");
    expectNear(YouKnowEngine::envelopeDecaySeconds(1.0f),
    ...
    using VoiceVcaLaw = YouKnowEngine::VoiceVcaControlLaw;
    ...
    for (const int storedByte : { 0, 32, 64, 96, 127 })
    {
        ...
        const double modelControl =
            YouKnowEngine::commonVcaControlVolts(position);
        const double modelGain = YouKnowEngine::patchLevelGain(position);
    ...
    expect(YouKnowEngine::patchLevelGain(0.0f) > 0.0f,
    ...
    expectNear(YouKnowEngine::commonVcaHoldTimeConstantSeconds(),
    ...
    expectNear(YouKnowEngine::voiceSummerGain, 3.3 / 33.0, 1.0e-7,
               "voice summer is not 3.3 kOhm / 33 kOhm");
    expectNear(YouKnowEngine::voiceBusInput(6.0f), 0.6, 1.0e-7,
    ...
    expectNear(YouKnowEngine::voiceBusCouplingCornerHz(), expectedBusCorner,
    ...
    expectNear(YouKnowEngine::voiceBusCouplingCornerHz(HighPassMode::One),
               loadedBusCorner, 1.0e-6,
               "flat HPF leg does not load C14 through its 47 kOhm input");
    expectNear(YouKnowEngine::voiceBusCouplingCornerHz(HighPassMode::Boost),
               loadedBusCorner, 1.0e-6,
    ...
    expectNear(YouKnowEngine::voiceBusCouplingCornerHz(HighPassMode::Two),
               cutLoadedBusCorner, 1.0e-6,
               "Cut II does not load C14 through its mux-side R21 bleed");
    expectNear(YouKnowEngine::voiceBusCouplingCornerHz(HighPassMode::Three),
               cutLoadedBusCorner, 1.0e-6,
               "Cut III does not load C14 through its mux-side R23 bleed");
    expectNear(YouKnowEngine::commonVcaInputCouplingCornerHz(),
               expectedBusCorner, 1.0e-6,
               "common-VCA C12/R36 input-coupling corner");
    expectNear(YouKnowEngine::voiceSummerGain * Chorus::dryMixGain,
               10.0 / 47.0, 1.0e-6,
               "net per-voice dry gain misses the jack-board ratios");
    expectNear(YouKnowEngine::voiceSummerGain * Chorus::wetMixGain,
               10.0 / 39.0, 1.0e-6,
...
void testPulseWidthAndHighPassLaws()
{
    ...
    expect(YouKnowEngine::pwmDacCode(
    ...
    expectNear(YouKnowEngine::pwmDacVolts(0x0fffu), 6.0, 1.0e-6,
               "pulse threshold with the control at rest");
    expectNear(YouKnowEngine::pwmDacVolts(0u), -0.8, 1.0e-6,
               "pulse-off DAC code missed its documented control voltage");
    expectNear(YouKnowEngine::pwmDutyCycle(6.0f), 0.5, 1.0e-5,
               "threshold at half the ramp does not bisect it");
    expectNear(YouKnowEngine::pwmDutyCycle(0.6f), 0.95, 1.0e-5,
               "the printed +0.6 V point is not a 95% pulse");
    const float physicalMaximum = YouKnowEngine::pwmDacVolts(
        manualCode(101));
    expectNear(YouKnowEngine::pwmDutyCycle(physicalMaximum),
               0.947106227, 1.0e-6,
               "the loaded knob maximum left Roland's 93-97% service window");
    expect(YouKnowEngine::pwmDutyCycle(
               YouKnowEngine::pwmDacVolts(manualCode(127))) == 1.0f,
           "the raw seven-bit PWM overrange no longer pins the comparator");
    expectNear(YouKnowEngine::pwmDutyCycle(-0.8f), 1.0, 1.0e-6,
               "pulse-off control does not pin the comparator high");
    expectNear(YouKnowEngine::pwmDutyCycle(3.0f, 1.03f),
               1.0 - 3.0 / (12.0 * 1.03), 1.0e-6,
               "a stronger physical ramp did not widen the comparator pulse");
    expectNear(YouKnowEngine::pwmDutyCycle(3.0f, 0.97f),
               1.0 - 3.0 / (12.0 * 0.97), 1.0e-6,
               "a weaker physical ramp did not narrow the comparator pulse");
    expectNear(YouKnowEngine::pwmDutyCycle(6.0f, 0.25f), 0.0, 0.0,
               "an under-compensated ramp cannot pin the comparator low");
    expectNear(YouKnowEngine::pwmDutyCycle(-0.8f, 1.03f), 1.0, 1.0e-6,
               "ramp variation defeated the pulse-off pinned state");
    for (int raw = 0; raw <= 101; ++raw)
    {
        const float duty = YouKnowEngine::pwmDutyCycle(
            YouKnowEngine::pwmDacVolts(manualCode(raw)));
    ...
    // change to the duty law that quietly rescaled it would pass otherwise.
    {
        ...
        const double wide = YouKnowEngine::pwmDutyCycle(mid - offsetVolts);
        const double narrow = YouKnowEngine::pwmDutyCycle(mid + offsetVolts);
        ...
        expectNear(YouKnowEngine::pwmDutyCycle(6.0f + offsetVolts), 0.5, 1.0e-5,
                   "an offset pushed the pulse below the panel's 50% floor");
        expectNear(YouKnowEngine::pwmDutyCycle(6.0f - offsetVolts), 0.52, 5.0e-4,
    ...
    expectNear(YouKnowEngine::highPassShelfGain(HighPassMode::Boost),
               1.0 + (47.0 / 220.0) * (1.0 + 100.0 / 10.0), 1.0e-6,
               "boost DC gain is not 1 + (R29/R24)(1 + R18/R19)");
    expectNear(YouKnowEngine::highPassHighGain(HighPassMode::Boost),
               1.0 + (47.0 / 220.0) * (47.0 / 57.0), 1.0e-6,
               "boost plateau is not 1 + (R29/R24) * C9/(C9 + C8)");
    expectNear(YouKnowEngine::highPassCornerHz(HighPassMode::Boost),
    ...
    // shelf to the exact response everywhere in the band.
    {
        ...
        const double shelf =
            YouKnowEngine::highPassShelfGain(HighPassMode::Boost);
        const double high =
            YouKnowEngine::highPassHighGain(HighPassMode::Boost);
        const double poleHz =
            YouKnowEngine::highPassCornerHz(HighPassMode::Boost);
        double worstDb = 0.0;
    ...
    expectNear(YouKnowEngine::highPassShelfGain(HighPassMode::One), 1.0, 1.0e-6,
               "the flat leg does not pass the low band untouched");
    expectNear(YouKnowEngine::highPassHighGain(HighPassMode::One), 1.0, 1.0e-6,
               "the flat leg does not pass the high band untouched");
    expect(YouKnowEngine::highPassShelfGain(HighPassMode::Two) == 0.0f
           && YouKnowEngine::highPassShelfGain(HighPassMode::Three) == 0.0f,
    ...
    expectNear(YouKnowEngine::highPassCornerHz(HighPassMode::Two),
               corner(15.0e-9), 0.5,
               "middle high-pass corner is not 47 kOhm against 15 nF");
    expectNear(YouKnowEngine::highPassCornerHz(HighPassMode::Three),
               corner(4.7e-9), 0.5,
    ...
    expectNear(YouKnowEngine::outputCouplingCornerHz(), expectedCorner,
               1.0e-5, "final output-coupling corner");
    expectNear(YouKnowEngine::outputCouplingHighGain(), expectedHighGain,
    ...
    {
        expectNear(YouKnowEngine::outputCouplingHighGain(position),
                   expectedLoadedGain(position), 1.0e-7,
                   "loaded 10KB wiper gain at shaft position "
                       + std::to_string(position));
        expectNear(YouKnowEngine::outputCouplingCornerHz(position),
                   expectedLoadedCorner(position), 1.0e-5,
                   "loaded output-coupling corner at shaft position "
                       + std::to_string(position));
    }
    expectNear(YouKnowEngine::outputCouplingHighGain(0.5f)
    ...
    {
        const double corner =
            YouKnowEngine::outputCouplingCornerHz(position);
        const double gain =
            YouKnowEngine::outputCouplingHighGain(position);
        const double g = std::tan(pi * corner / loadedRate);
        const double pole = (1.0 - g) / (1.0 + g);
        const auto response = YouKnowTestAccess::renderLoadedOutputCoupling(
    ...
    for (const double sampleRate : { 44100.0, 48000.0, 96000.0, 192000.0 })
    {
        ...
        const auto response =
            YouKnowTestAccess::renderOutputCoupling(step, sampleRate);
    ...
    for (const double sampleRate : { 44100.0, 48000.0, 192000.0 })
    {
        ...
        const std::vector<float> step(static_cast<std::size_t>(samples), 1.0f);
        const auto response =
            YouKnowTestAccess::renderVoiceBusCoupling(step, sampleRate);
        const int atTau = static_cast<int>(std::llround(
        ...
        const auto vcaResponse =
            YouKnowTestAccess::renderCommonVcaInputCoupling(step, sampleRate);
    ...
    for (const double sampleRate : { 44100.0, 48000.0, 192000.0 })
    {
             ...
             { HighPassMode::Two, HighPassMode::Three })
        {
            ...
            const auto response = YouKnowTestAccess::renderVoiceBusCoupling(
    ...
    for (std::size_t index = 0; index < blockInput.size(); ++index)
        blockInput[index] = static_cast<float>(
            ...
            + (index >= 997 ? 0.2 : -0.1));
    const auto wholeBlock = YouKnowTestAccess::renderVoiceBusCouplingInBlocks(
        blockInput, 48000.0, HighPassMode::Two, blockInput.size());
    const auto splitBlocks = YouKnowTestAccess::renderVoiceBusCouplingInBlocks(
    ...
    const auto stateContract =
        YouKnowTestAccess::voiceBusCouplingStateContract();
...
void testPwmDutyCycleDefensiveGuard()
{
    ...
    const double nominal = YouKnowEngine::pwmDutyCycle(6.0f);
    expectNear(YouKnowEngine::pwmDutyCycle(
    ...
    const double unscaled = YouKnowEngine::pwmDutyCycle(3.0f, 1.0f);
    expectNear(YouKnowEngine::pwmDutyCycle(
...
void testSharedHighPassAgainstNominalNetwork()
{
    ...
    const auto shippedResponse = [](HighPassMode mode, Complex s) {
        ...
        if (mode == HighPassMode::One)
            return coupling;
        const double corner = YouKnowEngine::highPassCornerHz(mode);
        if (mode == HighPassMode::Two || mode == HighPassMode::Three)
...
void testServiceSpecificationEndpointReconciliation()
{
    ...
    const std::uint16_t slowestAttack =
        YouKnowEngine::envelopeAttackIncrement(1.0f);
           ...
           "the slowest B-2 attack increment moved from 21");
    {
        ...
        while (level != YouKnowEngine::envelopePeak && passes < 1000)
        {
            level = YouKnowEngine::envelopeAttackLevel(level, slowestAttack);
            ++passes;
            if (passesToNinety == 0
                && static_cast<double>(level)
                       >= 0.9 * YouKnowEngine::envelopePeak)
    ...
    const std::uint16_t slowestFall =
        YouKnowEngine::envelopeDecayReleaseMultiplier(1.0f);
    expect(slowestFall == 65524u,
           "the slowest B-2 decay/release coefficient moved from 65524");
    {
        const double oneTau =
            std::exp(-1.0) * YouKnowEngine::envelopePeak;
        std::uint16_t level = YouKnowEngine::envelopePeak;
        int passes = 0;
        while (static_cast<double>(level) >= oneTau && passes < 10000)
        {
            level = YouKnowEngine::envelopeReleaseLevel(level, slowestFall);
            ++passes;
        }
        expect(passes == 3008,
    ...
    expectNear(1.0 / (8.0 * 30.0), nominalPassSeconds, clusterTolerance,
               ...
               "pass");
    expect(YouKnowEngine::lfoRateHz(0.0f) > 0.03f
               && YouKnowEngine::lfoRateHz(0.0f) < 0.04f,
    ...
    expectNear(std::log(9.0) * YouKnowTestAccess::voiceVcaHoldSlewSeconds(),
               0.0015, 5.0e-5,
...
void testModulationAndGlideLaws()
{
    ...
    {
        const float position = static_cast<float>(vector.code) / 127.0f;
        expect(YouKnowEngine::lfoRateIncrement(position) == vector.coefficient,
               "B-2 LFO coefficient vector mismatch at code "
                   + std::to_string(vector.code));
        expectNear(YouKnowEngine::lfoRateHz(position),
    ...
    for (int byte = 0; byte <= 127; ++byte)
    {
        ...
        const int attack = YouKnowEngine::envelopeAttackIncrement(position);
        const int fade = YouKnowEngine::lfoDelayFadeIncrement(position);
        const int holdPasses = (16384 + attack - 1) / attack;
        const int fadePasses = (65536 + fade - 1) / fade;
        expectNear(YouKnowEngine::lfoDelaySeconds(position),
                   (holdPasses + fadePasses - 1) * 0.0042, 1.0e-5,
                   "LFO delay does not use exact attack-hold plus fade at byte "
                       + std::to_string(byte));
    }
    expectNear(YouKnowEngine::lfoDelaySeconds(0.0f), 0.0084, 1.0e-6,
               "LFO delay byte zero misses its two-pass completion");
    expectNear(YouKnowEngine::lfoDelaySeconds(1.0f), 4.3512, 1.0e-4,
    ...
    for (const auto& bin : fadeBins)
    {
        ...
        {
            const auto coefficient = YouKnowEngine::lfoDelayFadeIncrement(
    ...
    expect(YouKnowEngine::portamentoSeconds(0.0f) == 0.0f,
           "portamento is not switched off at the bottom of its travel");
    expect(YouKnowEngine::portamentoIncrement(0.0f) == 0u,
           "raw portamento code zero is not immediate");
    expect(YouKnowEngine::portamentoSeconds(1.0f / 255.0f) == 0.0f,
           "raw portamento code one does not retain the zero/immediate entry");
    expect(YouKnowEngine::portamentoIncrement(1.0f / 255.0f) == 0u,
           "raw portamento code one selects a nonzero coefficient");
    expect(YouKnowEngine::portamentoIncrement(2.0f / 255.0f) == 255u
           && YouKnowEngine::portamentoIncrement(3.0f / 255.0f) == 255u,
           "raw portamento codes 2/3 miss coefficient 255");
    expectNear(YouKnowEngine::portamentoSeconds(2.0f / 255.0f),
               13.0 * 0.0042, 1.0e-6,
               "first active portamento code does not respect the 8-bit coefficient ceiling");
    expect(YouKnowEngine::portamentoIncrement(127.0f / 255.0f) == 13u
           && YouKnowEngine::portamentoIncrement(128.0f / 255.0f) == 13u,
           "raw portamento codes 127/128 miss coefficient 13");
    expectNear(YouKnowEngine::portamentoSeconds(127.0f / 255.0f),
               237.0 * 0.0042, 1.0e-6,
               "raw portamento code 127 misses the 995.4 ms vector");
    expectNear(YouKnowEngine::portamentoSeconds(128.0f / 255.0f),
               237.0 * 0.0042, 1.0e-6,
               "raw portamento code 128 misses the 995.4 ms vector");
    expect(YouKnowEngine::portamentoIncrement(254.0f / 255.0f) == 1u
           && YouKnowEngine::portamentoIncrement(1.0f) == 1u,
           "raw portamento codes 254/255 miss coefficient one");
    expectNear(YouKnowEngine::portamentoSeconds(254.0f / 255.0f),
               3072.0 * 0.0042, 1.0e-4,
               "raw portamento code 254 misses the 12.9024 second vector");
    expectNear(YouKnowEngine::portamentoSeconds(1.0f),
               3072.0 * 0.0042, 1.0e-4,
               "raw portamento code 255 misses the 12.9024 second vector");
    for (int raw = 2; raw < 255; raw += 2)
        expect(YouKnowEngine::portamentoIncrement(raw / 255.0f)
                   == YouKnowEngine::portamentoIncrement((raw + 1) / 255.0f),
    ...
    expect(YouKnowEngine::portamentoTravelAdcFraction(0.0f) == 0.0f
               && YouKnowEngine::portamentoTravelAdcFraction(1.0f) == 1.0f,
           "the loaded portamento divider moved its track endpoints");
    expectNear(YouKnowEngine::portamentoTravelAdcFraction(0.5f),
               23.5 / 59.5, 1.0e-6,
    ...
    for (int step = 0; step <= 64; ++step)
    {
        ...
        const float fraction =
            YouKnowEngine::portamentoTravelAdcFraction(travel);
        ...
        if (step != 0 && step != 64)
            expect(fraction < travel,
                   ...
                   "linear travel");
        expectNear(YouKnowEngine::portamentoTravelForAdcFraction(fraction),
                   travel, 1.0e-5,
    ...
    expectNear(YouKnowEngine::portamentoSeconds(
                   YouKnowEngine::portamentoTravelAdcFraction(0.5f)),
               YouKnowEngine::portamentoSeconds(101.0f / 255.0f), 0.0f,
...
{
    using Destination = YouKnowEngine::ConverterDestination;
    using Write = YouKnowEngine::ConverterWrite;
    constexpr std::array<Write, YouKnowEngine::converterWritesPerPass> expected {{
    ...
    const auto& actual = YouKnowEngine::converterWriteOrder();
    ...
    const auto normalized = YouKnowEngine::converterEventPhases(
    ...
    const auto phaseZero = YouKnowEngine::converterEventPhases(
    ...
    const auto measured = YouKnowEngine::converterEventPhases(
    ...
    expectNear(YouKnowEngine::outputReferenceGain(
                   YouKnowEngine::compatibilityOutputReferenceRmsVolts),
               1.0, 1.0e-6,
               "compatibility output reference does not preserve unity gain");
    expectNear(YouKnowEngine::outputReferenceGain(1.0f),
               YouKnowEngine::internalVoltsPerUnit
                   * YouKnowEngine::minus18DbfsAmplitude,
               1.0e-7, "one-volt RMS output reference violates the boundary law");
    const double lower = 0.25 * YouKnowEngine::outputReferenceGain(1.0f);
    const double doubled = 0.50 * YouKnowEngine::outputReferenceGain(1.0f);
    ...
    expectNear(YouKnowEngine::outputReferenceGain(2.0f),
               0.5 * YouKnowEngine::outputReferenceGain(1.0f), 1.0e-7,
    ...
    expect(YouKnowEngine::outputReferenceGain(0.0f) == 1.0f,
           "a zero output reference did not fall back to unity gain");
    expect(YouKnowEngine::outputReferenceGain(-1.0f) == 1.0f,
           "a negative output reference did not fall back to unity gain");
    expect(YouKnowEngine::outputReferenceGain(
...
void testPanelLawsInvert()
{
    ...
    {
        roundTrip(position, YouKnowEngine::envelopeAttackSeconds,
                  YouKnowEngine::panelPositionForAttack, "attack");
        roundTrip(position, YouKnowEngine::envelopeDecaySeconds,
                  YouKnowEngine::panelPositionForDecay, "decay");
        roundTrip(position, YouKnowEngine::envelopeReleaseSeconds,
                  YouKnowEngine::panelPositionForRelease, "release");
        roundTrip(position, YouKnowEngine::lfoDelaySeconds,
                  YouKnowEngine::panelPositionForLfoDelay, "modulation delay");
        ...
        const float rate = YouKnowEngine::lfoRateHz(position);
        expectNear(YouKnowEngine::lfoRateHz(
                       YouKnowEngine::panelPositionForLfoRate(rate)),
                   rate, 1.0e-4,
    ...
    for (float position = 0.0f; position <= 0.80f; position += 0.1f)
        roundTrip(position, [](float p) {
                      return YouKnowEngine::vcfCutoffHz(
                          YouKnowEngine::vcfPanelCounts(p));
                  },
                  YouKnowEngine::panelPositionForCutoff, "cutoff");

    const double uncappedAt13716 = YouKnowEngine::vcfBaseFrequencyHz
        * std::exp2(13716.0 / YouKnowEngine::vcfCountsPerOctave);
    expectNear(YouKnowEngine::vcfCutoffHz(13716.0f), uncappedAt13716,
               uncappedAt13716 * 2.0e-6,
               "the cutoff safety policy introduced a pre-cap knee");
    const float ceilingHz = YouKnowEngine::vcfCutoffHz(
    ...
    expectNear(YouKnowEngine::vcfCutoffHz(16383.0f), 50000.0, 1.0e-3,
    ...
    {
        const float seconds = YouKnowEngine::portamentoSeconds(position);
        expectNear(YouKnowEngine::portamentoSeconds(
                       YouKnowEngine::panelPositionForPortamento(seconds)),
                   seconds, 1.0e-6,
                   "portamento does not round trip through its displayed value");
    }
    expectNear(YouKnowEngine::panelPositionForPortamento(0.0f), 0.0, 1.0e-6,
    ...
    expectNear(YouKnowEngine::panelPositionForCutoff(1.0f), 0.0, 1.0e-6,
               "a cutoff below the range does not clamp to the bottom");
    expectNear(YouKnowEngine::panelPositionForLfoRate(1000.0f), 1.0, 1.0e-6,
...
void testPanelLawInversesRejectNonPositiveInput()
{
    ...
    expectFloorsToZero(YouKnowEngine::panelPositionForAttack, "attack");
    expectFloorsToZero(YouKnowEngine::panelPositionForDecay, "decay");
    expectFloorsToZero(YouKnowEngine::panelPositionForRelease, "release");
    expectFloorsToZero(YouKnowEngine::panelPositionForLfoRate, "modulation rate");
    expectFloorsToZero(YouKnowEngine::panelPositionForLfoDelay, "modulation delay");
    expectFloorsToZero(YouKnowEngine::panelPositionForCutoff, "cutoff");
    expectFloorsToZero(YouKnowEngine::panelPositionForPortamento, "portamento");
}
...
void testComparatorEdgesSitOnOneThreshold()
{
    ...
    const auto rampAt = [](double phase, double reset) {
        ...
        if (phase < rise)
            return static_cast<double>(YouKnowEngine::rampSegmentVoltage(
    ...
    for (double reset : { 0.0001, 0.01, 0.055, 0.25 })
    {
        ...
        {
            const double riseEdge = YouKnowEngine::pulseRisePhase(
                duty, static_cast<float>(reset));
            const double fallEdge = YouKnowEngine::pulseFallPhase(
                duty, static_cast<float>(reset));
    ...
    {
        const double high = YouKnowEngine::pulseFallPhase(duty, 1.0e-6f)
                          - YouKnowEngine::pulseRisePhase(duty, 1.0e-6f);
...
void testJuno60FallbackBucketBrigadeTiming()
{
    ...
    // to run, or the two pieces of the model would contradict each other.
    {
        ...
        expect(YouKnowTestAccess::runningChorusMode(chorus)
                   == ChorusMode::Two,
    ...
    constexpr float datasheetClockHz = 100000.0f;
    {
        ...
        for (long long sample = 0; sample < settle + window; ++sample)
        {
            const double volts = static_cast<double>(
                YouKnowTestAccess::processBbdCore(
...
void testChorusBypassStateAndWetMuteTiming()
{
    ...
    YouKnowTestAccess::setChorusWetGain(
        bypassedForComparison,
        YouKnowTestAccess::chorusWetGain(alwaysOn));
    ...
    const auto tenToNinetySeconds = [](float rate) {
        ...
        {
            chorus.process(0.0f, ChorusMode::One, 0.0f, left, right);
            const float fraction = YouKnowTestAccess::chorusWetGain(chorus)
                                 / target;
...
void testChorusRateChangePreservesPhysicalState()
{
    ...
    const auto before = YouKnowTestAccess::chorusPhysicalState(chorus);
    ...
    const auto after = YouKnowTestAccess::chorusPhysicalState(chorus);
    expect(after == before,
           "a numerical sample-rate change power-cycled BBD/free-running state");
    expect(YouKnowTestAccess::chorusAudioRateSupportIsClear(chorus),
...
void testBucketBrigadeDatasheetAnchors()
{
    ...
    const auto completeHeldGainAt = [=](double frequency) {
        ...
        for (int index = 0; index < settle + window; ++index)
        {
            ...
            const float output = YouKnowTestAccess::transferLossStep(
...
void testBbdTransferDefensiveGuards()
{
    ...
    expect(YouKnowTestAccess::bbdTransfer(
    ...
    const float positiveRail = YouKnowTestAccess::bbdTransfer(
        std::numeric_limits<float>::max());
    const float negativeRail = YouKnowTestAccess::bbdTransfer(
        -std::numeric_limits<float>::max());
...
void testBbdTransferApproximationTracksItsReference()
{
    ...
    expect(!std::signbit(YouKnowTestAccess::bbdTransfer(0.0f))
               && std::signbit(YouKnowTestAccess::bbdTransfer(-0.0f)),
    ...
    const auto expectReferenceUlp = [&](float input) {
        ...
        const float actual = YouKnowTestAccess::bbdTransfer(input);
    ...
    float previous = YouKnowTestAccess::bbdTransfer(0.0f);
    for (int index = 0; index <= 65536; ++index)
    {
        const float input = static_cast<float>(
            4.0 * level * static_cast<double>(index) / 65536.0);
        const float positive = YouKnowTestAccess::bbdTransfer(input);
        const float negative = YouKnowTestAccess::bbdTransfer(-input);
...
void testBbdInputCubicInterpolation()
{
    const auto interpolate = [](float current, float previous,
                                float previous2, float previous3,
                                double age) {
        return YouKnowTestAccess::interpolateBbdInput(
    ...
    YouKnowTestAccess::setBbdInputHistory(history, 0.25f, -0.5f, 0.75f);
    history.reset();
    expect(YouKnowTestAccess::bbdInputHistory(history)
               == std::array<float, 3> {},
...
void testBbdOutputPolyBlepReferenceAndBounds()
{
    ...
    expectNear(YouKnowTestAccess::bbdPolyBlepResidual(0.0), -0.5, 1.0e-15,
               "BBD polyBLEP residual at the discontinuity");
    expect(YouKnowTestAccess::bbdPolyBlepResidual(0.5) < 0.0,
           "BBD polyBLEP lost its pre-zero residual sign");
    expect(YouKnowTestAccess::bbdPolyBlepResidual(0.75) > 0.0,
           "BBD polyBLEP lost its post-zero residual sign");
    expectNear(YouKnowTestAccess::bbdPolyBlepResidual(1.0), 1.0 / 24.0,
               1.0e-14, "BBD polyBLEP residual at one sample");
    expectNear(YouKnowTestAccess::bbdPolyBlepResidual(2.0), 0.0, 1.0e-15,
               "BBD polyBLEP did not end at two samples");
    expectNear(YouKnowTestAccess::bbdPolyBlepResidual(1.0 - 1.0e-8),
               YouKnowTestAccess::bbdPolyBlepResidual(1.0 + 1.0e-8),
    ...
    for (std::size_t sample = 0; sample < clocks.size(); ++sample)
    {
        ...
        const float actual = YouKnowTestAccess::processBbdCore(
    ...
    if (std::getenv("YOUKNOW_AUDIT_BBD_BLEP") != nullptr)
        std::cout << "BBD BLEP independent-vector max error: "
                  << maximumError << '\n';

    const auto coreState = YouKnowTestAccess::bbdCorePhysicalState(production);
    ...
    const auto beforeCorrection =
        YouKnowTestAccess::bbdCorePhysicalState(production);
    const int eventsBefore = YouKnowTestAccess::bbdBlepEventCount(production);
    const double correctionA = YouKnowTestAccess::bbdCoreCorrection(
        production, static_cast<double>(clocks.back()) / 8000.0);
    const double correctionB = YouKnowTestAccess::bbdCoreCorrection(
        production, static_cast<double>(clocks.back()) / 8000.0);
    ...
    expect(afterCorrection == beforeCorrection
               && YouKnowTestAccess::bbdBlepEventCount(production)
                    == eventsBefore,
    ...
    for (int sample = 0; sample < 16; ++sample)
    {
        ...
        expect(YouKnowTestAccess::bbdBlepEventCount(maximumRate)
    ...
    {
        const float output = YouKnowTestAccess::processBbdCore(
        ...
        const auto state = YouKnowTestAccess::bbdCorePhysicalState(noiseOnly);
    ...
    expect(YouKnowTestAccess::bbdBlepEventCount(maximumRate) == 0,
...
{
    expectNear(YouKnowTestAccess::bbdPolyBlepResidual(-0.5), 0.0, 1.0e-15,
               "BBD polyBLEP residual did not fall back to zero for a negative distance");
    expectNear(YouKnowTestAccess::bbdPolyBlepResidual(
    ...
    expectNear(YouKnowTestAccess::bbdPolyBlepResidual(2.0 + 1.0e-9), 0.0,
               1.0e-15,
...
void testBbdOutputPolyBlepSeparatesPhysicalAndNumericalAliases()
{
    ...
    const auto render = [](float clockHz) {
        ...
        for (std::size_t sample = 0; sample < result.first.size(); ++sample)
        {
            ...
            result.second[sample] = YouKnowTestAccess::processBbdCore(
                line, input, clockHz, sampleRate, 0.0f);
            result.first[sample] = YouKnowTestAccess::bbdRawHeld(line);
        }
            ...
            const float input = static_cast<float>(
                0.02 * std::sin(2.0 * pi * inputFrequency
                                * static_cast<double>(sample)
                                / processingRate));
            rendered.first[sample] = YouKnowTestAccess::processBbdFullLine(
                rawLine, input, 10000.0f, false);
            rendered.second[sample] = YouKnowTestAccess::processBbdFullLine(
...
void testHighPassStateGuardSelfHeals()
{
    ...
    const float g = 0.5f;
    expect(YouKnowTestAccess::highPassStateAfterProcess(
    ...
    const float recovered =
        YouKnowTestAccess::highPassOutputAfterPoisonedCallHeals(
            std::numeric_limits<double>::quiet_NaN(), 1.0f, g, 0.0f, 1.0f);
...
void testBoostBranchMatchesItsNetworkAndKeepsItsCharge()
{
    ...
    for (double frequency : { 20.0, 100.0, 300.0, 1000.0, 5000.0 })
    {
        ...
        for (int index = 0; index < settle + window; ++index)
        {
            ...
            const double out = YouKnowTestAccess::boostBranchStep(
    ...
    for (int index = 0; index < static_cast<int>(rate * 0.2); ++index)
        YouKnowTestAccess::boostBranchStep(engine, 0.2f, true);
    expectNear(YouKnowTestAccess::boostBranchNodeVolts(engine), 0.2, 1.0e-4,
    ...
    {
        const double out =
            YouKnowTestAccess::boostBranchStep(engine, 0.0f, false);
        if (index == 0)
            first = out;
    ...
    YouKnowTestAccess::boostBranchStep(engine, 1.0f, true);
    expectNear(YouKnowTestAccess::boostBranchNodeVolts(engine),
               47.0 / 57.0, 0.01,
...
void testChorusOutputLoadingFollowsTheMuteTransistors()
{
    ...
    for (const float rate : { 48000.0f, 192000.0f })
    {
        ...
        for (int index = 0; index < static_cast<int>(0.9f * rate); ++index)
        {
            ...
            const auto& actual =
                YouKnowTestAccess::chorusExactOutputState(delayed);
            const auto& expected =
                YouKnowTestAccess::chorusExactOutputState(switched);
...
void testPwmDispersionSitsInsideTheServiceWindow()
{
    ...
    const auto dutiesAt = [](float calibration) {
        ...
        for (int voice = 0; voice < 6; ++voice)
            duties[static_cast<std::size_t>(
                YouKnowTestAccess::voiceCard(engine, voice))] =
                YouKnowTestAccess::voicePulseDuty(engine, voice);
        return duties;
    };
    ...
    for (std::size_t card = 1; card < 6; ++card)
    {
        expect(std::abs(dispersed[card] - dispersed[0])
                   <= YouKnowTestAccess::pwmDutyAcceptanceHalfWidth() + 0.001f,
...
void testNoiseSourceShapingFollowsItsCircuit()
{
    ...
    expectNear(YouKnowEngine::noiseSourceHighPassHz(),
               1.0 / (2.0 * pi * 1.0e-6 * 4700.0), 1.0e-3,
               "noise-source high-pass is not at C42/4.7k's corner");
    expectNear(YouKnowEngine::noiseSourceLowPassHz(),
    ...
    const double policy = YouKnowEngine::outputLevelPolicyGain;
...
void testNoiseLevelControlPrecedesC41()
{
    ...
    YouKnowEngine circuit;
    YouKnowEngine legacy;
    circuit.prepare(sampleRate, 64, false);
    legacy.prepare(sampleRate, 64, false);
    YouKnowTestAccess::setNoiseSourceSupportState(circuit, 0.0, 0.0);
    YouKnowTestAccess::setNoiseSourceSupportState(legacy, 0.0, 0.0);
    ...
    for (int sample = 0; sample < 2048; ++sample)
    {
        ...
        const float ordered = YouKnowTestAccess::processMainNoiseSource(
            circuit, raw, 0.5f, true);
        const float former = YouKnowTestAccess::processMainNoiseSource(
    ...
    const auto fixedCircuitState =
        YouKnowTestAccess::noiseSourceSupportState(circuit);
    const auto fixedLegacyState =
        YouKnowTestAccess::noiseSourceSupportState(legacy);
    ...
    YouKnowEngine endpointCircuit;
    YouKnowEngine endpointLegacy;
    ...
    for (int sample = 0; sample < 256; ++sample)
    {
        ...
        expect(YouKnowTestAccess::processMainNoiseSource(
                   endpointCircuit, raw, level, true)
                   == YouKnowTestAccess::processMainNoiseSource(
                       endpointLegacy, raw, level, false),
               "the sub-sample C41 fallback exposed an alternating tail");
    }
    expect(YouKnowTestAccess::noiseSourceSupportState(endpointCircuit)
               == YouKnowTestAccess::noiseSourceSupportState(endpointLegacy),
    ...
    YouKnowEngine integratedCircuit;
    YouKnowEngine integratedLegacy;
    ...
    const auto processBlock = [&](YouKnowEngine& engine, int frames) {
    ...
    expect(YouKnowTestAccess::noiseSourceSupportState(integratedCircuit)
               == YouKnowTestAccess::noiseSourceSupportState(integratedLegacy),
           "the integrated unity-level NOISE paths did not precharge identically");
    expect(YouKnowTestAccess::noiseHeld(integratedCircuit) > 0.999f,
    ...
    for (int sample = 0; sample < 512; ++sample)
    {
        ...
        const double circuitState =
            YouKnowTestAccess::noiseSourceSupportState(integratedCircuit)[1];
        const double legacyState =
            YouKnowTestAccess::noiseSourceSupportState(integratedLegacy)[1];
    ...
    const auto integratedCircuitState =
        YouKnowTestAccess::noiseSourceSupportState(integratedCircuit);
    ...
    expect(YouKnowTestAccess::noiseHeld(integratedCircuit) < 1.0e-6f,
...
void testNoiseSourceLowRateSafetyAndStateSemantics()
{
    ...
    const double physicalCorner = YouKnowEngine::noiseSourceLowPassHz();
    ...
    for (const auto& item : rateCases)
    {
        ...
        const double actualRate =
            YouKnowTestAccess::noiseSourceProcessingRate(engine);
        const double g =
            YouKnowTestAccess::noiseSourceLowPassG(engine);
    ...
    {
        YouKnowEngine hqEndpoint;
        hqEndpoint.prepare(endpointRate, 64, true);
        const double actual =
            YouKnowTestAccess::noiseSourceLowPassG(hqEndpoint);
    ...
    // resets it. The bounded endpoint recursion must instead decay normally.
    {
        ...
        for (int index = 0; index < 4096; ++index)
        {
            const float output = YouKnowTestAccess::processNoiseSourceSupport(
                engine, index == 0 ? 1.0f : 0.0f);
            const auto state =
                YouKnowTestAccess::noiseSourceSupportState(engine);
    ...
    // 8 kHz. This also rejects a cap accidentally based on the host rate.
    {
        ...
        const double hqG =
            YouKnowTestAccess::noiseSourceLowPassG(engine);
        YouKnowTestAccess::setNoiseSourceSupportState(engine, -0.125, 0.375);
        YouKnowTestAccess::forceNoiseSourceProcessingQuality(engine, false);
        expect(engine.getOversamplingFactor() == 1,
               "forced 8k noise-support rate did not select q1");
        expect(YouKnowTestAccess::noiseSourceSupportState(engine)
                   == std::array<double, 2> { -0.125, 0.375 },
               "q4-to-q1 rebuild reset the physical noise-support state");
        expectNear(YouKnowTestAccess::noiseSourceLowPassG(engine),
                   expectedCoefficient(endpointRate), 1.0e-5,
                   "q4-to-q1 rebuild missed the 8k endpoint cap");

        YouKnowTestAccess::forceNoiseSourceProcessingQuality(engine, true);
        ...
        expectNear(YouKnowTestAccess::noiseSourceLowPassG(engine), hqG,
...
void testCombinedBbdSupportTransitionAndStateSafety()
{
             ...
             8000.0, 44100.0, 48000.0, 176400.0, 192000.0, 768000.0 })
    {
        ...
        for (int frame = 0; frame < driveFrames + decayFrames; ++frame)
        {
            ...
            const float output = YouKnowTestAccess::processBbdExactMode(
                chorus, input, 200000.0f, connected);
            allFinite = allFinite && std::isfinite(output)
                && YouKnowTestAccess::chorusExactHistoriesAreFinite(chorus);
            for (double value : YouKnowTestAccess::chorusExactInputState(
                     chorus))
            {
                allFinite = allFinite && std::isfinite(value);
                maximumState = std::max(maximumState, std::abs(value));
            }
            for (double value : YouKnowTestAccess::chorusExactOutputState(
                     chorus))
            {
                allFinite = allFinite && std::isfinite(value);
                maximumState = std::max(maximumState, std::abs(value));
        ...
        const float nanOutput = YouKnowTestAccess::processBbdExactMode(
            chorus, std::numeric_limits<float>::quiet_NaN(), 200000.0f, true);
        const float infiniteOutput = YouKnowTestAccess::processBbdExactMode(
            chorus, std::numeric_limits<float>::infinity(), 200000.0f, false);
        const float maximumOutput = YouKnowTestAccess::processBbdExactMode(
        ...
        expect(YouKnowTestAccess::chorusExactHistoriesAreFinite(chorus),
               "combined BBD support retained a hostile non-finite input");
        for (double value : YouKnowTestAccess::chorusExactInputState(chorus))
            expect(std::isfinite(value),
                   "combined BBD input state escaped finite containment");
        for (double value : YouKnowTestAccess::chorusExactOutputState(chorus))
            expect(std::isfinite(value),
        ...
        for (int frame = 0; frame < 1024; ++frame)
            recoveryPeak = std::max(recoveryPeak, std::abs(static_cast<double>(
                YouKnowTestAccess::processBbdExactMode(
             ...
             std::pair { Chorus::minimumExactInputSupportRate, true } })
    {
        ...
        for (int frame = 0; frame < 32; ++frame)
            (void) YouKnowTestAccess::processBbdExactMode(
                chorus, frame == 0 ? 0.25f : 0.0f, 50000.0f, true);
        const bool exactStateMoved = std::any_of(
            YouKnowTestAccess::chorusExactInputState(chorus).begin(),
            YouKnowTestAccess::chorusExactInputState(chorus).end(),
            [](double value) { return value != 0.0; });
...
void testCorrectionResidualsVanishAtTheEdges()
{
    ...
    const double programmed = YouKnowEngine::dcoQuantisedFrequency(
        YouKnowEngine::dcoDivider(440.0), DcoRange::Eight);
...
void testOutputSummerIsLinearBelowItsAsymptote()
{
    ...
    const auto thirdHarmonicFraction = [](double peakVolts) {
        ...
        const double amplitude =
            peakVolts / static_cast<double>(YouKnowEngine::internalVoltsPerUnit);
        double first = 0.0;
        double third = 0.0;
        for (int index = 0; index < points; ++index)
        {
            ...
            const double output = YouKnowEngine::outputSummerClip(
    ...
    constexpr float asymptote =
        YouKnowEngine::outputSummerSwingAsymptoteVolts
        / YouKnowEngine::internalVoltsPerUnit;
             ...
             3.0f, -asymptote, 10.0f, std::numeric_limits<float>::max() })
    {
        ...
        expectNear(YouKnowEngine::outputSummerClip(drive), reference,
                   2.0e-6,
    ...
    {
        expect(std::abs(YouKnowEngine::outputSummerClip(drive)) <= asymptote,
               "the output summer passed its positive model asymptote");
        expect(std::abs(YouKnowEngine::outputSummerClip(-drive)) <= asymptote,
               "the output summer passed its negative model asymptote");
    }
    expect(std::isfinite(YouKnowEngine::outputSummerClip(
               std::numeric_limits<float>::max())),
    ...
    expect(YouKnowEngine::outputSummerClip(
               std::numeric_limits<float>::quiet_NaN()) == 0.0f,
    ...
    expectNear(YouKnowEngine::outputSummerClip(3.0f),
               -YouKnowEngine::outputSummerClip(-3.0f), 1.0e-6,
...
void testOutputSummerBandwidthFollowsNoiseGain()
{
    ...
    expectNear(YouKnowEngine::outputSummerBandwidthHz(),
               3.0e6 / noiseGain, 0.1,
...
void testOutputResistorNoiseFollowsJohnsonNyquistLaw()
{
    ...
    expectNear(YouKnowEngine::outputSummerResistorNoiseDensity(),
...
void testOutputSummerSlewMatchesDatasheetTypical()
{
    ...
    for (const auto& rate : rates)
    {
        ...
        expectNear(YouKnowTestAccess::outputSlewVoltsPerSecond(engine),
                   1.0e6, 1.0,
...
void testFilterCoreDividerMatchesSchematic()
{
    ...
    expectNear(YouKnowTestAccess::stageAttenuation(),
               560.0 / (68000.0 + 560.0), 1.0e-9,
               "the filter-core input divider left the Roland schematic");
    expectNear(YouKnowTestAccess::otaHeadroomVolts(), 2.0 * 0.026
...
void testDecimatorProtectsTheTopOfTheBand()
{
    ...
    const auto kernel = YouKnowTestAccess::halfbandKernel();
...
int main()
{
    ...
    if (failures != 0)
    {
        std::cerr << failures << " YouKnow circuit check(s) failed.\n";
        return EXIT_FAILURE;
    }
    std::cout << "All YouKnow circuit checks passed.\n";
    return EXIT_SUCCESS;
}
