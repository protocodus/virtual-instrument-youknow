...
struct YouKnowTestAccess
{
    ...
    {
        return YouKnowEngine::subMixVolts;
    }
    ...
    static int lastVoiceMidi(const YouKnowEngine& engine, int slot) noexcept
    {
    ...
    static void setDcoResetPending(YouKnowEngine& engine, int slot,
    ...
    static float stageGScale(const YouKnowEngine& engine, int slot,
                             int stage) noexcept
    {
    ...
    static float stageOffset(const YouKnowEngine& engine, int slot,
                             int stage) noexcept
    {
        return engine.voices_[static_cast<std::size_t>(slot)]
    ...
    static float cardStageOffset(const YouKnowEngine& engine, int slot,
                                 int stage) noexcept
    {
    ...
    static float lfoValue(const YouKnowEngine& engine) noexcept
    {
    ...
    static void advanceLfo(YouKnowEngine& engine,
                           const EngineParameters& parameters) noexcept
    {
        engine.advanceLfo(parameters);
    }

    static void setLfoRestartState(YouKnowEngine& engine) noexcept
    {
    ...
    static double pwmFirstPoleHeld(const YouKnowEngine& engine) noexcept
    {
        return engine.pwmVoltsFirstPole_;
    }

    static float subTarget(const YouKnowEngine& engine) noexcept
    {
        return engine.subCvTarget_;
    }

    static float noiseTarget(const YouKnowEngine& engine) noexcept
    {
        return engine.noiseCvTarget_;
    }

    static double dcoPeriodSamples(const YouKnowEngine& engine,
                                   int slot) noexcept
    {
        return engine.voices_[static_cast<std::size_t>(slot)].dco.periodSamples;
    }

    static double dcoRampValue(const YouKnowEngine& engine,
                               int slot) noexcept
    {
        ...
        const YouKnowEngine& engine, int slot) noexcept
    {
        ...
        return YouKnowEngine::dcoPositiveBaseRail(
        ...
        const YouKnowEngine& engine, int slot) noexcept
    {
        ...
        const double totalScale =
            static_cast<double>(voice.dco.renderScale)
            * static_cast<double>(voice.rampCurrentScale);
        return 0.5 * YouKnowEngine::rampAmplitudeVolts
    ...
    {
        return YouKnowEngine::Dco::mode3HalfClocks(count, outHigh);
    }

    static double eightFootClockHz() noexcept
    {
        return YouKnowEngine::rangeClockHz(DcoRange::Eight);
    }

    static double rangeClockHz(DcoRange range) noexcept
    {
        return YouKnowEngine::rangeClockHz(range);
    }

    static double masterClockHz() noexcept
    {
        return YouKnowEngine::masterClockHz;
    }
    ...
    static void setInternalRateForTimingTest(
        YouKnowEngine& engine, double rate) noexcept
    {
        engine.oversampledRate_ = rate;
        engine.inverseOversampledRate_ = static_cast<float>(1.0 / rate);
    }

    static void setMode3Running(YouKnowEngine& engine, int slot,
                                ...
                                DcoRange range = DcoRange::Eight) noexcept
    {
        ...
        dco.pitWriteState = YouKnowEngine::Dco::PitWriteState::idle;
    ...
    static void stageMode3Count(YouKnowEngine& engine, int slot,
    ...
    static int consumePitEvent(YouKnowEngine& engine, int slot) noexcept
    {
    ...
    {
        return static_cast<int>(YouKnowEngine::Dco::PitEvent::fallingEdge);
    }

    static int risingPitEvent() noexcept
    {
        return static_cast<int>(YouKnowEngine::Dco::PitEvent::risingEdge);
    }

    static void programDcoCount(YouKnowEngine& engine, int slot,
                                std::uint32_t count,
    ...
    static void setAwaitingPitMsb(YouKnowEngine& engine, int slot,
                                  std::uint32_t count,
    ...
    static void setAwaitingPitLsb(YouKnowEngine& engine, int slot,
                                  std::uint32_t count,
                                  double cpuStates) noexcept
    {
        auto& dco = engine.voices_[static_cast<std::size_t>(slot)].dco;
    ...
    static void setAwaitingPitchPrestage(
        YouKnowEngine& engine, int slot, double cpuStates) noexcept
    {
        auto& dco = engine.voices_[static_cast<std::size_t>(slot)].dco;
        dco.pitWriteState =
            YouKnowEngine::Dco::PitWriteState::awaitingPitchPrestage;
        dco.cpuStatesToWrite = cpuStates;
    }

    static void performPitchWrite(YouKnowEngine& engine, int slot,
                                  const EngineParameters& parameters) noexcept
    {
    ...
    static float dcoCv(const YouKnowEngine& engine, int slot) noexcept
    {
        return engine.voices_[static_cast<std::size_t>(slot)].dcoCv;
    }

    static float dcoCvTarget(const YouKnowEngine& engine,
                             int slot) noexcept
    {
        return engine.voices_[static_cast<std::size_t>(slot)].dcoCvTarget;
    }

    static void advanceDcoPitAndRamp(YouKnowEngine& engine, int slot,
    ...
    static void advanceDcoPitAndRamp(
        YouKnowEngine& engine, int slot, float previousDuty, float duty,
        bool corrections) noexcept
    {
        const auto& voice =
            engine.voices_[static_cast<std::size_t>(slot)];
    ...
    static void advanceDcoPitAndRampThreshold(
        YouKnowEngine& engine, int slot, DcoRange range,
    ...
    static void advanceDcoAndSharedRangeClock(
        YouKnowEngine& engine, int slot, DcoRange range) noexcept
    {
    ...
    static void setDcoRampState(YouKnowEngine& engine, int slot,
    ...
    static void setDcoResetState(YouKnowEngine& engine, int slot,
    ...
    static void setDcoRampScales(YouKnowEngine& engine, int slot,
                                 float renderScale,
    ...
    static void setPositiveRailHeld(YouKnowEngine& engine, int slot,
                                    bool held) noexcept
    {
        engine.voices_[static_cast<std::size_t>(slot)]
    ...
    static bool positiveRailHeld(const YouKnowEngine& engine,
                                 int slot) noexcept
    {
        return engine.voices_[static_cast<std::size_t>(slot)]
    ...
    static void setDcoLaunchState(YouKnowEngine& engine, int slot,
    ...
    static void setColdDcoPitchTransaction(YouKnowEngine& engine, int slot,
                                           float capturedCvCode) noexcept
    {
        ...
        voice.dco.pitWriteState = YouKnowEngine::Dco::PitWriteState::idle;
    }
    ...
    static void primePulseTrack(YouKnowEngine& engine, int slot,
    ...
    static bool pitOutHigh(const YouKnowEngine& engine, int slot) noexcept
    {
    ...
    static void setRangeClockPhase(YouKnowEngine& engine,
    ...
    static void setRangeClockTransition(
        YouKnowEngine& engine, double clocksToFalling,
    ...
    static double rangeClockClocksToNextFallingEdge(
        const YouKnowEngine& engine, double elapsedSeconds,
    ...
    static void advanceRangeClock(YouKnowEngine& engine,
                                  DcoRange range) noexcept
    {
        engine.advanceRangeClock(range);
    }
    ...
    static int pitState(const YouKnowEngine& engine, int slot) noexcept
    {
    ...
    {
        return static_cast<int>(
            YouKnowEngine::Dco::PitState::awaitingInitialLoad);
    }

    static int awaitingCountState() noexcept
    {
        return static_cast<int>(
            YouKnowEngine::Dco::PitState::awaitingCount);
    }

    static int pitWriteState(const YouKnowEngine& engine,
                             int slot) noexcept
    {
        return static_cast<int>(
            engine.voices_[static_cast<std::size_t>(slot)]
    ...
    {
        return static_cast<int>(
            YouKnowEngine::Dco::PitWriteState::idle);
    }
    ...
    {
        return static_cast<int>(
            YouKnowEngine::Dco::PitWriteState::awaitingLsb);
    }

    static int awaitingMsbWriteState() noexcept
    {
        return static_cast<int>(
            YouKnowEngine::Dco::PitWriteState::awaitingMsb);
    }
    ...
    {
        return static_cast<int>(YouKnowEngine::Dco::PitState::stopped);
    }

    static int runningPitState() noexcept
    {
        return static_cast<int>(YouKnowEngine::Dco::PitState::running);
    }

    static void setDcoLogicStates(YouKnowEngine& engine, int slot,
    ...
    static void primeDcoControlEdgeFixture(
        YouKnowEngine& engine, int slot) noexcept
    {
        auto& dco = engine.voices_[static_cast<std::size_t>(slot)].dco;
    ...
    static double chorusPhase(const YouKnowEngine& engine) noexcept
    {
    ...
    static bool pulseThresholdHistoryIsPrimed(const YouKnowEngine& engine,
                                              int slot) noexcept
    {
        return engine.voices_[static_cast<std::size_t>(slot)]
    ...
    static void primePulseDutyHistory(YouKnowEngine& engine, int slot,
                                      float duty) noexcept
    {
        auto& voice = engine.voices_[static_cast<std::size_t>(slot)];
        const float totalScale =
    ...
    static bool pulseMixEnabled(const YouKnowEngine& engine, int slot,
    ...
    static double moduleCouplingState(const YouKnowEngine& engine,
                                      int slot) noexcept
    {
        return engine.voices_[static_cast<std::size_t>(slot)]
    ...
    static double vcaControl(const YouKnowEngine& engine, int slot) noexcept
    {
        return engine.voices_[static_cast<std::size_t>(slot)].vcaControl;
    }

    static float vcaControlTarget(const YouKnowEngine& engine,
                                  int slot) noexcept
    {
        return engine.voices_[static_cast<std::size_t>(slot)].vcaControlTarget;
    }

    static float voiceVcaGain(const YouKnowEngine& engine,
                              int slot) noexcept
    {
        return engine.voices_[static_cast<std::size_t>(slot)].vca;
    }

    static double sharedVca(const YouKnowEngine& engine) noexcept
    {
        return engine.sharedVca_;
    }

    static void setSharedVca(YouKnowEngine& engine, float state) noexcept
    {
        engine.sharedVca_ = state;
    }

    static float sharedVcaTarget(const YouKnowEngine& engine) noexcept
    {
        return engine.sharedVcaTarget_;
    }

    static void setSharedVcaHold(YouKnowEngine& engine, double state,
                                 float target) noexcept
    {
        engine.sharedVca_ = state;
    ...
    static void setPwmHeld(YouKnowEngine& engine, double state) noexcept
    {
        engine.pwmVoltsFirstPole_ = state;
        engine.pwmVolts_ = state;
    }

    static void setPwmHold(YouKnowEngine& engine, double first,
        ...
        double intervalSeconds) noexcept
    {
        const auto coefficients =
            YouKnowEngine::pwmHoldCoefficients(intervalSeconds);
        const auto result = YouKnowEngine::exactPwmHoldEndpoint(
            YouKnowEngine::PwmHoldState { stateFirst, stateSecond },
    ...
    static double subHeld(const YouKnowEngine& engine) noexcept
    {
        return engine.subCv_;
    }

    static void setSubHeld(YouKnowEngine& engine, float state) noexcept
    {
        engine.subCv_ = state;
    }

    static void setSubHold(YouKnowEngine& engine, double state,
                           float target) noexcept
    {
        engine.subCv_ = state;
    ...
    static void setVoiceVcaHold(YouKnowEngine& engine, int slot,
    ...
    static float noiseHeld(const YouKnowEngine& engine) noexcept
    {
        return engine.noiseCv_;
    }

    static void setNoiseHold(YouKnowEngine& engine, float state,
                             float target) noexcept
    {
    ...
    static bool assignmentPending(const YouKnowEngine& engine) noexcept
    {
    ...
    static float filterOutputVolts(const YouKnowEngine& engine,
                                   int slot) noexcept
    {
    ...
    static float vcaInputVolts(const YouKnowEngine& engine,
                               int slot) noexcept
    {
        return engine.voices_[static_cast<std::size_t>(slot)].vcaInputVolts;
    }

    static bool voiceActive(const YouKnowEngine& engine, int slot) noexcept
    {
        return engine.voices_[static_cast<std::size_t>(slot)].active;
    }

    static void setFilterState(YouKnowEngine& engine, int slot,
    ...
    static void setMicroscopicNoiseState(YouKnowEngine& engine, int slot,
                                         std::uint32_t state) noexcept
    {
        engine.voices_[static_cast<std::size_t>(slot)].noiseState = state;
    }

    static void initialiseVoice(YouKnowEngine& engine, int slot,
    ...
    static void silenceVoice(YouKnowEngine& engine, int slot) noexcept
    {
        engine.silenceVoice(
            engine.voices_[static_cast<std::size_t>(slot)]);
    }
    ...
    static float renderSilentVoiceAtControlOffset(
        YouKnowEngine& engine, float controlOffset) noexcept
    {
    ...
    static double controlScanPhase(const YouKnowEngine& engine) noexcept
    {
    ...
    static std::size_t vcfOrdinal(int voice) noexcept
    {
        ...
        for (std::size_t ordinal = 0; ordinal < writes.size(); ++ordinal)
            if (writes[ordinal].destination
                    == YouKnowEngine::ConverterDestination::Vcf
    ...
    {
        YouKnowEngine::ConverterDestination engineDestination =
            YouKnowEngine::ConverterDestination::Resonance;
        switch (destination)
        {
            ...
            case PassiveHoldDestination::CommonVca:
                engineDestination =
                    YouKnowEngine::ConverterDestination::CommonVca;
                break;
            case PassiveHoldDestination::Sub:
                engineDestination = YouKnowEngine::ConverterDestination::Sub;
                break;
            case PassiveHoldDestination::Pwm:
                engineDestination = YouKnowEngine::ConverterDestination::Pwm;
                break;
            case PassiveHoldDestination::Vcf:
                engineDestination = YouKnowEngine::ConverterDestination::Vcf;
                break;
            case PassiveHoldDestination::VoiceVca:
                engineDestination =
                    YouKnowEngine::ConverterDestination::VoiceVca;
                break;
        }
        const auto& writes = YouKnowEngine::converterWriteOrder();
        for (std::size_t ordinal = 0; ordinal < writes.size(); ++ordinal)
        ...
        PassiveHoldDestination destination) noexcept
    {
        ...
        return ordinal < YouKnowEngine::converterWritesPerPass
            ? static_cast<int>(YouKnowEngine::converterWriteOrder()[ordinal]
                                   .destination)
            : -1;
    }
    ...
    static std::size_t pitchOrdinal(int voice) noexcept
    {
        ...
        for (std::size_t ordinal = 0; ordinal < writes.size(); ++ordinal)
            if (writes[ordinal].destination
                    == YouKnowEngine::ConverterDestination::Pitch
                && writes[ordinal].voice == voice)
                return ordinal;
        return writes.size();
    }
    ...
    static std::size_t noiseOrdinal() noexcept
    {
        ...
        for (std::size_t ordinal = 0; ordinal < writes.size(); ++ordinal)
            if (writes[ordinal].destination
                    == YouKnowEngine::ConverterDestination::Noise)
                return ordinal;
        return writes.size();
    }

    static double converterEventPhase(
        const YouKnowEngine& engine, std::size_t ordinal) noexcept
    {
    ...
    static double scanPhasePerInternalSample(
        const YouKnowEngine& engine) noexcept
    {
        return YouKnowEngine::controlScanHz / engine.oversampledRate_;
    }

    static void setConverterScheduler(
        YouKnowEngine& engine, double phase,
    ...
    static void setCutoffHold(
        YouKnowEngine& engine, int slot, float state,
        float target) noexcept
    {
        auto& voice = engine.voices_[static_cast<std::size_t>(slot)];
    ...
    static float resonanceHeld(const YouKnowEngine& engine) noexcept
    {
        return engine.resonanceCv_;
    }

    static float resonanceTarget(const YouKnowEngine& engine) noexcept
    {
        return engine.resonanceCvTarget_;
    }

    static void setResonanceHold(
        YouKnowEngine& engine, float state, float target) noexcept
    {
    ...
    {
        return YouKnowEngine::exactVcfHoldInterval(
    ...
    static float glidedVolume(const YouKnowEngine& engine) noexcept
    {
        return engine.glidedVolume_;
    }

    static float cardAgingCutoffCounts(const YouKnowEngine& engine,
                                       int card) noexcept
    {
        return engine.cards_[static_cast<std::size_t>(card)].agingCutoffCounts;
    }

    static float cardAgingWeight(const YouKnowEngine& engine,
                                 int card) noexcept
    {
        return engine.cards_[static_cast<std::size_t>(card)].agingWeight;
    }

    static float agedNoiseGain(const YouKnowEngine& engine) noexcept
    {
        return engine.agedNoiseGain_;
    }

    static double converterEventPhase(const YouKnowEngine& engine,
                                     ...
                                     float widthPointError) noexcept
    {
        ...
        return YouKnowEngine::cutoffAnalogCounts(counts, card, 1.0f, 0.0f);
    }
    ...
    struct BandlimitedTrackState
    {
        ...
        std::array<float, YouKnowEngine::correctionHalfWidth> delay {};
        int base { 0 };
        bool primed { false };
    };
    ...
    static BandlimitedTrackState expectedPulseStep(
        YouKnowEngine& engine, float initial, float height,
    ...
    static BandlimitedTrackState expectedSawSlope(
        YouKnowEngine& engine, float slopeStep,
        float samplesAgo) noexcept
    {
        YouKnowEngine::BandlimitedTrack track;
        track.prime(0.0f);
        ...
        YouKnowEngine& engine) noexcept
    {
        constexpr float epsilon = 0.25f
                                / YouKnowEngine::correctionOversample;
        YouKnowEngine::BandlimitedTrack before;
        YouKnowEngine::BandlimitedTrack at;
        YouKnowEngine::BandlimitedTrack after;
        engine.addStep(before, 1.0f, 1.0f - epsilon);
        engine.addStep(at, 1.0f, 0.0f);
        engine.addStep(after, 1.0f, epsilon);
        constexpr int centre = YouKnowEngine::correctionHalfWidth;
        return {
    ...
    static double thermalWarmupSeconds(const YouKnowEngine& engine) noexcept
    {
    ...
    static std::array<float, 2> thermalOmegaPair(
        const YouKnowEngine& engine, int cardIndex,
        float baseOmegaStep) noexcept
    {
        ...
        const float cached = static_cast<float>(
            YouKnowEngine::OtaCascade::clampOmegaStep(
    ...
    static double internalRate(const YouKnowEngine& engine) noexcept
    {
    ...
    static float otaHeadroomVolts(const YouKnowEngine& engine,
    ...
    {
        return YouKnowEngine::sanitise(parameters);
    }

    static bool usesCubicEarly(const YouKnowEngine& engine) noexcept
    {
    ...
    static bool sameOtaCascadePersistentBits(
        const YouKnowEngine::OtaCascade& first,
        const YouKnowEngine::OtaCascade& second) noexcept
    {
        ...
        std::size_t fixtureIndex) noexcept
    {
        ...
        const auto makeCascade = [](const LaneFixture& fixture) {
            YouKnowEngine::OtaCascade cascade;
            ...
            cascade.previousOmegaStep =
                YouKnowEngine::OtaCascade::clampOmegaStep(fixture.omega);
        ...
        const bool accepted =
            YouKnowEngine::OtaCascade::tryProcessSettledRk4Pair(
        ...
        std::size_t fixtureIndex) noexcept
    {
        ...
        float pairSecondOutput = 97.25f;
        const bool accepted =
            YouKnowEngine::OtaCascade::tryProcessSettledMersonPair(
                pairFirst, fixture.first.input, fixture.first.omega,
                fixture.first.feedback, fixture.first.headroom,
                pairSecond, fixture.second.input, fixture.second.omega,
                fixture.second.feedback, fixture.second.headroom,
    ...
    static OtaQuadComparison compareSettledMersonQuad() noexcept
    {
        ...
        std::array<YouKnowEngine::OtaCascade, 4> initial;
        ...
        const auto pointers = [](auto& cascades) {
            std::array<YouKnowEngine::OtaCascade*, 4> result;
        ...
        const bool accepted =
            YouKnowEngine::OtaCascade::tryProcessSettledMersonQuad(
        ...
        std::array<YouKnowEngine::OtaCascade, 4> stayedQuad {
    ...
    {
        YouKnowEngine::OtaCascade first;
        YouKnowEngine::OtaCascade second;
        ...
        bool mixedTableaux) noexcept
    {
        ...
        first.previousOmegaStep =
            YouKnowEngine::OtaCascade::clampOmegaStep(mersonOmega);
...
#if defined(YOUKNOW_SANITIZER_BUILD)
constexpr bool sanitizerBuild = YOUKNOW_SANITIZER_BUILD != 0;
    ...
    long double duration)
{
    const long double firstTime =
        static_cast<long double>(YouKnowTestAccess::pwmFirstPoleSeconds());
    const long double secondTime =
        static_cast<long double>(YouKnowTestAccess::pwmSecondPoleSeconds());
...
void testSelfOscillationLandsOnTheServiceAnchor()
{
    ...
    const double law = YouKnowEngine::vcfCutoffHz(
        YouKnowEngine::vcfPanelCounts(parameters.cutoff));
    ...
    const auto scales = [](float calibration) {
        ...
        for (int stage = 0; stage < 4; ++stage)
            result[static_cast<std::size_t>(stage)] =
                YouKnowTestAccess::stageGScale(engine, 0, stage);
        return result;
    };
...
void testAliasFloor()
{
    ...
    for (double sampleRate : { 44100.0, 48000.0 })
    {
        for (int note : { 60, 84 })
        {
            ...
            const double f0 = YouKnowEngine::dcoQuantisedFrequency(
                YouKnowEngine::dcoDivider(wanted), DcoRange::Eight);
            ...
            if (std::getenv("YOUKNOW_AUDIT_ALIAS") != nullptr)
...
{
    YouKnowEngine engine;
    const auto sides = YouKnowTestAccess::stepCorrectionEventSides(engine);
...
void testRampHasARampSpectrum()
{
    ...
    const auto rendered = render(engine, static_cast<int>(sampleRate));
    const double f0 = YouKnowEngine::dcoQuantisedFrequency(
        YouKnowEngine::dcoDivider(110.0), DcoRange::Eight);
    const std::size_t start = static_cast<std::size_t>(sampleRate * 0.5);
...
void testRescanPreservesVoiceCpuPitchHistory()
{
    ...
    YouKnowEngine cleared;
    YouKnowEngine transposedEquivalent;
    auto parameters = plainPatch();
...
void testHeldTransposeUpdatesVoiceCpuPitchHistory()
{
    ...
    engine.setParameters(parameters);
    renderExact(engine, scanPeriod * 2);
    expect(YouKnowTestAccess::lastVoiceMidi(engine, 0) == 72,
    ...
    expect(!YouKnowTestAccess::dcoResetPending(engine, 0),
    ...
    YouKnowTestAccess::updateVoiceScan(releasing, 0, releaseParameters);
    expect(YouKnowTestAccess::dcoResetPending(releasing, 0),
...
void testMode3CountStagingAndControlPolarity()
{
    ...
    expect(YouKnowTestAccess::mode3HalfClocks(8u, true) == 4u
               && YouKnowTestAccess::mode3HalfClocks(8u, false) == 4u,
           "an even Mode-3 count did not split equally");
    expect(YouKnowTestAccess::mode3HalfClocks(9u, true) == 5u
               && YouKnowTestAccess::mode3HalfClocks(9u, false) == 4u,
           "an odd Mode-3 count did not give its extra clock to OUT high");
    expect(YouKnowTestAccess::mode3HalfClocks(65535u, true) == 32768u
               && YouKnowTestAccess::mode3HalfClocks(65535u, false)
                      == 32767u,
    ...
    YouKnowTestAccess::setMode3Running(engine, 0, 8u, true, 1.0);
    YouKnowTestAccess::stageMode3Count(engine, 0, 9u);
    expect(YouKnowTestAccess::activeDcoDivider(engine, 0) == 8u
               && YouKnowTestAccess::pendingDcoDividerValid(engine, 0),
           "a count-only write changed CE before the current half-cycle ended");
    expect(YouKnowTestAccess::consumePitEvent(engine, 0)
               == YouKnowTestAccess::fallingPitEvent(),
           "the running high half did not end with a falling OUT edge");
    expect(!YouKnowTestAccess::pitOutHigh(engine, 0)
               && YouKnowTestAccess::activeDcoDivider(engine, 0) == 9u
               && YouKnowTestAccess::pitClocksToEvent(engine, 0) == 4.0
               && !YouKnowTestAccess::pendingDcoDividerValid(engine, 0),
    ...
    YouKnowTestAccess::setMode3Running(engine, 0, 9u, false, 1.0);
    YouKnowTestAccess::stageMode3Count(engine, 0, 8u);
    expect(YouKnowTestAccess::consumePitEvent(engine, 0)
               == YouKnowTestAccess::risingPitEvent(),
           "the running low half did not end with a rising OUT edge");
    expect(YouKnowTestAccess::pitOutHigh(engine, 0)
               && YouKnowTestAccess::activeDcoDivider(engine, 0) == 8u
               && YouKnowTestAccess::pitClocksToEvent(engine, 0) == 4.0,
    ...
    YouKnowTestAccess::setMode3Running(engine, 0, 10u, true, 1.0);
    YouKnowTestAccess::stageMode3Count(engine, 0, 11u);
    YouKnowTestAccess::stageMode3Count(engine, 0, 13u);
    (void) YouKnowTestAccess::consumePitEvent(engine, 0);
    expect(YouKnowTestAccess::activeDcoDivider(engine, 0) == 13u,
           "a later complete count did not replace the pending count register");
    YouKnowTestAccess::setMode3Running(engine, 0, 10u, true, 1.0);
    YouKnowTestAccess::stageMode3Count(engine, 0, 11u);
    YouKnowTestAccess::stageMode3Count(engine, 0, 10u);
    (void) YouKnowTestAccess::consumePitEvent(engine, 0);
    expect(YouKnowTestAccess::activeDcoDivider(engine, 0) == 10u,
    ...
    YouKnowEngine programmedEdge;
    YouKnowEngine runningEdge;
    ...
    YouKnowTestAccess::primeDcoControlEdgeFixture(programmedEdge, 0);
    YouKnowTestAccess::primeDcoControlEdgeFixture(runningEdge, 0);
    YouKnowTestAccess::programDcoCount(
        programmedEdge, 0, 10000u, true);
    YouKnowTestAccess::advanceDcoPitAndRamp(runningEdge, 0, true);
    const auto programmedSaw =
        YouKnowTestAccess::sawTrackState(programmedEdge, 0);
    const auto runningSaw =
        YouKnowTestAccess::sawTrackState(runningEdge, 0);
    const auto programmedSub =
        YouKnowTestAccess::subTrackState(programmedEdge, 0);
    const auto runningSub =
        YouKnowTestAccess::subTrackState(runningEdge, 0);
    ...
    YouKnowTestAccess::setMode3Running(engine, 0, 9u, false, 3.25);
    YouKnowTestAccess::primeDcoControlEdgeFixture(engine, 0);
    YouKnowTestAccess::programDcoCount(engine, 0, 11u, true);
    expect(YouKnowTestAccess::pitOutHigh(engine, 0)
               && YouKnowTestAccess::pitState(engine, 0)
                      == YouKnowTestAccess::awaitingCountState()
               && YouKnowTestAccess::pitClocksToEvent(engine, 0) == 0.25
               && YouKnowTestAccess::pitWriteState(engine, 0)
                      == YouKnowTestAccess::awaitingLsbWriteState()
               && !YouKnowTestAccess::pendingDcoDividerValid(engine, 0)
               && YouKnowTestAccess::dcoLogicStates(engine, 0)[0] == 1.0f
               && YouKnowTestAccess::dcoLogicStates(engine, 0)[1] == 1.0f,
           "forcing a low OUT high did not make exactly the C54/sub edge");

    const auto sawAfter = YouKnowTestAccess::sawTrackState(engine, 0);
    const auto pulseAfter = YouKnowTestAccess::pulseTrackState(engine, 0);
    const auto subAfter = YouKnowTestAccess::subTrackState(engine, 0);
    ...
    const auto transition =
        YouKnowTestAccess::subTrackAfterControlEdge(engine, 0);
    ...
    for (int frame = 0; frame < 4; ++frame)
        YouKnowTestAccess::advanceDcoPitAndRamp(engine, 0, true);
    expect(YouKnowTestAccess::activeDcoDivider(engine, 0) == 11u
               && YouKnowTestAccess::pitState(engine, 0)
                      != YouKnowTestAccess::awaitingInitialLoadState()
               && YouKnowTestAccess::pitWriteState(engine, 0)
                      == YouKnowTestAccess::idlePitWriteState()
               && !YouKnowTestAccess::pendingDcoDividerValid(engine, 0)
               && YouKnowTestAccess::pitClocksToEvent(engine, 0) > 0.0
               && YouKnowTestAccess::pitClocksToEvent(engine, 0) <= 6.0,
           "the odd initial Mode-3 count did not enter CE");

    YouKnowTestAccess::setMode3Running(engine, 0, 9u, true, 2.75);
    YouKnowTestAccess::setDcoLogicStates(engine, 0, -1.0f, -1.0f);
    YouKnowTestAccess::programDcoCount(engine, 0, 8u, true);
    expect(YouKnowTestAccess::pitClocksToEvent(engine, 0) == 0.75
               && YouKnowTestAccess::dcoLogicStates(engine, 0)[0] == -1.0f
               && YouKnowTestAccess::dcoLogicStates(engine, 0)[1] == -1.0f,
    ...
    const double warmRail =
        YouKnowTestAccess::dcoPositiveBaseRail(warm, 0);
    YouKnowTestAccess::setDcoRampState(warm, 0, warmRail, 0.0);
    YouKnowTestAccess::setPositiveRailHeld(warm, 0, true);
    YouKnowTestAccess::setDcoLogicStates(warm, 0, 1.0f, -1.0f);
    YouKnowTestAccess::programDcoCount(warm, 0, 10000u, true);
    for (int frame = 0; frame < 4; ++frame)
        YouKnowTestAccess::advanceDcoPitAndRamp(warm, 0, true);
    expect(YouKnowTestAccess::activeDcoDivider(warm, 0) == 10000u
               && YouKnowTestAccess::dcoRampState(warm, 0)[0] == warmRail
               && YouKnowTestAccess::dcoRampState(warm, 0)[1] == 0.0
               && YouKnowTestAccess::dcoRampState(warm, 0)[2] == 0.0
               && YouKnowTestAccess::dcoLogicStates(warm, 0)[1] == -1.0f,
    ...
    {
        YouKnowTestAccess::advanceDcoPitAndRamp(warm, 0, true);
        if (YouKnowTestAccess::dcoLogicStates(warm, 0)[1] == -1.0f)
        {
            expect(YouKnowTestAccess::dcoRampState(warm, 0)[0] == warmRail
                       && YouKnowTestAccess::dcoRampState(warm, 0)[1] == 0.0,
        ...
        else
        {
            ...
            expect(YouKnowTestAccess::pitOutHigh(warm, 0)
                       && YouKnowTestAccess::dcoRampValue(warm, 0)
                              < warmRail,
    ...
    YouKnowTestAccess::programDcoCount(cold, 0, 10000u, true);
    for (int frame = 0; frame < 4; ++frame)
        YouKnowTestAccess::advanceDcoPitAndRamp(cold, 0, true);
    expect(YouKnowTestAccess::activeDcoDivider(cold, 0) == 10000u
               && YouKnowTestAccess::dcoRampValue(cold, 0) < -0.99
               && YouKnowTestAccess::dcoRampState(cold, 0)[1] > 0.0,
           "cold initial load no longer held the low rail before first charge");

    engine.reset();
    expect(YouKnowTestAccess::pitState(engine, 0)
               == YouKnowTestAccess::stoppedPitState()
               && YouKnowTestAccess::pitOutHigh(engine, 0)
               && !YouKnowTestAccess::pendingDcoDividerValid(engine, 0),
...
void testPitByteTransactionsFollowRecoveredCpuTiming()
{
    ...
    const auto advance = [](YouKnowEngine& engine, DcoRange range,
                            int frames = 1) {
        for (int frame = 0; frame < frames; ++frame)
            YouKnowTestAccess::advanceDcoAndSharedRangeClock(
                engine, 0, range);
    };
    ...
    for (const auto& phase : phases)
    {
        ...
        expect(YouKnowTestAccess::pitState(engine, 0)
                   ...
                   && YouKnowTestAccess::pitWriteState(engine, 0)
                       == YouKnowTestAccess::idlePitWriteState()
                   && YouKnowTestAccess::pitWriteDivider(engine, 0)
                       == YouKnowTestAccess::activeDcoDivider(engine, 0)
        ...
        YouKnowTestAccess::stageMode3Count(engine, 0, 0x2345u);
        YouKnowTestAccess::programDcoCount(engine, 0, requested, true);
        ...
        expect(YouKnowTestAccess::pitState(engine, 0)
                   ...
                   && YouKnowTestAccess::pitWriteState(engine, 0)
                       == YouKnowTestAccess::awaitingLsbWriteState()
                   && YouKnowTestAccess::cpuStatesToWrite(engine, 0)
                       == 55.0,
        ...
        expect(YouKnowTestAccess::pitWriteState(engine, 0)
                       == YouKnowTestAccess::awaitingLsbWriteState()
                   && std::abs(
                       YouKnowTestAccess::cpuStatesToWrite(engine, 0)
                       - 11.0) < 1.0e-10
        ...
        expect(YouKnowTestAccess::pitWriteState(engine, 0)
                       == YouKnowTestAccess::awaitingMsbWriteState()
                   && std::abs(
                       YouKnowTestAccess::cpuStatesToWrite(engine, 0)
                       - 11.0) < 1.0e-10
                   && !YouKnowTestAccess::pendingDcoDividerValid(engine, 0),
        ...
        expect(YouKnowTestAccess::pitWriteState(engine, 0)
                   ...
                   && YouKnowTestAccess::pitState(engine, 0)
                       == YouKnowTestAccess::awaitingInitialLoadState()
                   && YouKnowTestAccess::pendingDcoDividerValid(engine, 0)
                   && YouKnowTestAccess::pendingDcoDivider(engine, 0)
                       == requested,
               "the 16.50 us MSB write did not complete exactly one count");
        expectNear(YouKnowTestAccess::pitClocksToEvent(engine, 0),
                   phase.clocksAfterMsb, 1.0e-10,
                   "the control transaction lost the selected PIT-clock phase");
        advance(engine, phase.range);
        expect(YouKnowTestAccess::activeDcoDivider(engine, 0) == requested
                   && !YouKnowTestAccess::pendingDcoDividerValid(engine, 0),
    ...
    for (const auto& loadCase : firstLoadCases)
    {
        const double selectedClockHz =
            YouKnowTestAccess::rangeClockHz(loadCase.range);
        for (const auto count : firstLoadDividers)
        {
            ...
            YouKnowEngine engine;
            engine.prepare(192000.0, blockSize, false);
            YouKnowTestAccess::setInternalRateForTimingTest(
                engine, rawClockRate);
            YouKnowTestAccess::setRangeClockPhase(engine, 1.0);
            YouKnowTestAccess::programDcoCount(
                engine, 0, count, true);

            int writeFrames = 0;
            while (YouKnowTestAccess::pitWriteState(engine, 0)
                       != YouKnowTestAccess::idlePitWriteState()
                   && writeFrames < 200)
            {
            ...
            int loadFrames = 0;
            while (YouKnowTestAccess::pitState(engine, 0)
                       == YouKnowTestAccess::awaitingInitialLoadState()
                   && loadFrames < 16)
            {
                advance(engine, loadCase.range);
            ...
            const auto highClocks =
                YouKnowTestAccess::mode3HalfClocks(count, true);
            const auto lowClocks =
                YouKnowTestAccess::mode3HalfClocks(count, false);
            expect(YouKnowTestAccess::activeDcoDivider(engine, 0) == count
                       && YouKnowTestAccess::pitOutHigh(engine, 0)
                       && !YouKnowTestAccess::pendingDcoDividerValid(engine, 0),
            ...
            const double legacyReloadSeconds = correctedLoadSeconds
                + 1.0 / YouKnowTestAccess::masterClockHz();
    ...
    YouKnowEngine realCollision;
    realCollision.prepare(elevenCpuStatesRate, blockSize, false);
    YouKnowTestAccess::setMode3Running(
        realCollision, 0, active, true, 5.5, DcoRange::Eight);
    YouKnowTestAccess::stageMode3Count(
        realCollision, 0, oldPending);
    YouKnowTestAccess::programDcoCount(
        realCollision, 0, requested, false);
    expect(YouKnowTestAccess::activeDcoDivider(realCollision, 0) == active
               && YouKnowTestAccess::pendingDcoDivider(realCollision, 0)
                      == oldPending
    ...
    expect(YouKnowTestAccess::activeDcoDivider(realCollision, 0)
                   == oldPending
               && !YouKnowTestAccess::pitOutHigh(realCollision, 0)
               && YouKnowTestAccess::pendingDcoDivider(realCollision, 0)
                      == requested
               && YouKnowTestAccess::pendingDcoDividerValid(
                   realCollision, 0),
    ...
    for (const auto range : ranges)
    {
        ...
        for (const double offsetInPeriods : writeOffsetsInPeriods)
        {
            for (int repetition = 0; repetition < 2; ++repetition)
            {
                ...
                YouKnowTestAccess::setAwaitingPitMsb(
                ...
                expect(YouKnowTestAccess::activeDcoDivider(
                               collision, 0)
                           == (writeBeforeFalling ? requested : oldPending)
                           && !YouKnowTestAccess::pitOutHigh(collision, 0)
                           && YouKnowTestAccess::pitWriteState(
                                  collision, 0)
                                  == YouKnowTestAccess::idlePitWriteState()
                           && YouKnowTestAccess::pendingDcoDividerValid(
                                  collision, 0)
    ...
    YouKnowEngine resetTransaction;
    resetTransaction.prepare(192000.0, blockSize, false);
    YouKnowTestAccess::setAwaitingPitMsb(
        resetTransaction, 0, requested, 7.0);
    resetTransaction.reset();
    expect(YouKnowTestAccess::pitState(resetTransaction, 0)
                   == YouKnowTestAccess::stoppedPitState()
               && YouKnowTestAccess::pitWriteState(resetTransaction, 0)
                      == YouKnowTestAccess::idlePitWriteState()
               && YouKnowTestAccess::pitWriteDivider(resetTransaction, 0)
                      == YouKnowTestAccess::activeDcoDivider(
                          resetTransaction, 0)
               && YouKnowTestAccess::cpuStatesToWrite(
...
{
    YouKnowEngine at48k;
    YouKnowEngine at48kHq;
    YouKnowEngine at192k;
    ...
    const auto advance = [](YouKnowEngine& engine, int frames) {
        for (int frame = 0; frame < frames; ++frame)
            YouKnowTestAccess::advanceDcoAndSharedRangeClock(
    ...
    const auto sameLogicalState = [](const YouKnowEngine& left,
                                     const YouKnowEngine& right) {
        return YouKnowTestAccess::activeDcoDivider(left, 0)
                   == YouKnowTestAccess::activeDcoDivider(right, 0)
            && YouKnowTestAccess::pendingDcoDivider(left, 0)
                   == YouKnowTestAccess::pendingDcoDivider(right, 0)
            && YouKnowTestAccess::pendingDcoDividerValid(left, 0)
                   == YouKnowTestAccess::pendingDcoDividerValid(right, 0)
            && YouKnowTestAccess::pitState(left, 0)
                   == YouKnowTestAccess::pitState(right, 0)
            && YouKnowTestAccess::pitOutHigh(left, 0)
                   == YouKnowTestAccess::pitOutHigh(right, 0)
            && YouKnowTestAccess::pitWriteState(left, 0)
                   == YouKnowTestAccess::pitWriteState(right, 0)
            && YouKnowTestAccess::pitWriteDivider(left, 0)
                   == YouKnowTestAccess::pitWriteDivider(right, 0)
            && std::abs(YouKnowTestAccess::cpuStatesToWrite(left, 0)
                        - YouKnowTestAccess::cpuStatesToWrite(right, 0))
                   < 1.0e-10
            && std::abs(YouKnowTestAccess::pitClocksToEvent(left, 0)
                        - YouKnowTestAccess::pitClocksToEvent(right, 0))
                   < 1.0e-9;
    };
...
void testSerialVoiceCommandsRestartScanWithoutSplittingPitWrites()
{
    ...
    const auto fixture = [=] {
        ...
        YouKnowTestAccess::setConverterScheduler(*engine, 0.75, 17u);
        YouKnowTestAccess::setColdDcoPitchTransaction(*engine, 0, 0.75f);
        return engine;
    };
    const auto expectRestarted = [](const YouKnowEngine& engine,
                                    const std::string& context) {
        const auto latch = YouKnowTestAccess::passiveHoldLatch(engine);
        expect(YouKnowTestAccess::controlScanPhase(engine) == 0.0
                   && YouKnowTestAccess::nextConverterWrite(engine) == 0u
                   && !latch.valid
                   && !YouKnowTestAccess::dcoPitchTransactionValid(
                          engine, 0)
                   && !YouKnowTestAccess::dcoPitchTransactionColdStart(
                          engine, 0)
                   && YouKnowTestAccess::pitWriteState(engine, 0)
                          == YouKnowTestAccess::idlePitWriteState()
                   && YouKnowTestAccess::cpuStatesToWrite(engine, 0) == 0.0,
    ...
    YouKnowTestAccess::setAwaitingPitLsb(
        *beforeDi, 0, requested, 42.5);
    beforeDi->noteOn(60, 1.0f);
    expectRestarted(*beforeDi, "a pre-DI Voice On");
    expect(YouKnowTestAccess::pitState(*beforeDi, 0)
                   == YouKnowTestAccess::runningPitState()
               && YouKnowTestAccess::activeDcoDivider(*beforeDi, 0)
                      == active
               && !YouKnowTestAccess::pendingDcoDividerValid(*beforeDi, 0),
    ...
    auto atDi = fixture();
    YouKnowTestAccess::setAwaitingPitLsb(*atDi, 0, requested, 42.0);
    atDi->noteOn(60, 1.0f);
    expectRestarted(*atDi, "a DI-boundary Voice On");
    expect(YouKnowTestAccess::pendingDcoDividerValid(*atDi, 0)
               && YouKnowTestAccess::pendingDcoDivider(*atDi, 0)
                      == requested,
    ...
    expect(YouKnowTestAccess::pendingDcoDividerValid(*afterLsb, 0)
               && YouKnowTestAccess::pendingDcoDivider(*afterLsb, 0)
                      == requested,
    ...
    YouKnowTestAccess::stageMode3Count(*sharedBoard, 4, completed);
    YouKnowTestAccess::setMode3Running(
    ...
    expect(YouKnowTestAccess::pendingDcoDividerValid(*sharedBoard, 5)
               && YouKnowTestAccess::pendingDcoDivider(*sharedBoard, 5)
                      == requested
               && YouKnowTestAccess::pitWriteState(*sharedBoard, 5)
                      == YouKnowTestAccess::idlePitWriteState()
               && YouKnowTestAccess::pendingDcoDividerValid(*sharedBoard, 4)
               && YouKnowTestAccess::pendingDcoDivider(*sharedBoard, 4)
                      == completed,
    ...
    const auto resetPrestageFixture = [&](int oldNote, double cpuStates,
        ...
        YouKnowTestAccess::setMode3Running(
            *engine, 0, active, true, 10000.25);
        YouKnowTestAccess::setConverterScheduler(*engine, 0.75, 17u);
        YouKnowTestAccess::setDcoResetPending(
            *engine, 0, resetPending);
        YouKnowTestAccess::setAwaitingPitchPrestage(
            *engine, 0, cpuStates);
        return engine;
    };
    auto oldPitchReference = resetPrestageFixture(36, 4.0);
    const std::uint32_t oldPitchCount = YouKnowTestAccess::updateVoiceScan(
        *oldPitchReference, 0, resetParameters);
    auto newPitchReference = resetPrestageFixture(96, 4.0);
    const std::uint32_t newPitchCount = YouKnowTestAccess::updateVoiceScan(
    ...
    for (const double protectedStates : { 4.0, 3.5 })
    {
        ...
        expect(YouKnowTestAccess::pitState(*protectedReset, 0)
                       == YouKnowTestAccess::awaitingInitialLoadState()
                   && YouKnowTestAccess::pendingDcoDividerValid(
                          *protectedReset, 0)
                   && YouKnowTestAccess::pendingDcoDivider(
    ...
    expect(YouKnowTestAccess::pitState(*beforeResetDi, 0)
                   == YouKnowTestAccess::runningPitState()
               && !YouKnowTestAccess::pendingDcoDividerValid(
                      *beforeResetDi, 0),
    ...
    const auto latentResetPrestageFixture =
        [&](double cpuStates, LatentResetRunState runState) {
            ...
            (void) YouKnowTestAccess::updateVoiceScan(
            ...
            YouKnowTestAccess::setMode3Running(
                *engine, 0, active, true, 10000.25);
            YouKnowTestAccess::setConverterScheduler(
                *engine, 0.75, 17u);
            YouKnowTestAccess::setDcoResetPending(
                *engine, 0, false);
            YouKnowTestAccess::setAwaitingPitchPrestage(
    ...
    for (const double protectedStates : { 4.0, 3.5 })
    {
        ...
        expect(YouKnowTestAccess::pitState(*protectedLatent, 0)
                       == YouKnowTestAccess::awaitingInitialLoadState()
                   && YouKnowTestAccess::pendingDcoDividerValid(
                          *protectedLatent, 0)
                   && YouKnowTestAccess::pendingDcoDivider(
    ...
    expect(YouKnowTestAccess::pitState(*beforeLatentResetDi, 0)
                   == YouKnowTestAccess::runningPitState()
               && !YouKnowTestAccess::pendingDcoDividerValid(
                                 ...
                                 LatentResetRunState::Sustained })
    {
        ...
        expect(YouKnowTestAccess::pitState(*runningLatent, 0)
                       == YouKnowTestAccess::runningPitState()
                   && !YouKnowTestAccess::pendingDcoDividerValid(
                          *runningLatent, 0),
    ...
    expect(YouKnowTestAccess::pitState(*runningPrestage, 0)
                   == YouKnowTestAccess::runningPitState()
               && !YouKnowTestAccess::pendingDcoDividerValid(
                      *runningPrestage, 0),
    ...
    expect(YouKnowTestAccess::pitState(*resetPath, 0)
                   == YouKnowTestAccess::awaitingInitialLoadState()
               && YouKnowTestAccess::pendingDcoDividerValid(*resetPath, 0)
               && YouKnowTestAccess::pendingDcoDivider(*resetPath, 0)
                      == requested,
    ...
    YouKnowTestAccess::setConverterScheduler(*releaseAll, 0.75, 17u);
    YouKnowTestAccess::setColdDcoPitchTransaction(*releaseAll, 0, 0.75f);
    YouKnowTestAccess::setAwaitingPitLsb(
        *releaseAll, 0, requested, 42.0);
    releaseAll->releaseAllNotes();
    expectRestarted(*releaseAll, "an All Notes Off Voice Off");
    expect(YouKnowTestAccess::pendingDcoDividerValid(*releaseAll, 0)
               && YouKnowTestAccess::pendingDcoDivider(*releaseAll, 0)
                      == requested,
    ...
    completedHold.setParameters(holdParameters);
    const std::size_t vcfOrdinal = YouKnowTestAccess::vcfOrdinal(0);
    const double scanDelta =
        YouKnowTestAccess::scanPhasePerInternalSample(completedHold);
    YouKnowTestAccess::setConverterScheduler(
        completedHold,
        YouKnowTestAccess::converterEventPhase(
    ...
    const auto completedLatch =
        YouKnowTestAccess::passiveHoldLatch(completedHold);
    const float heldAfterWrite =
        YouKnowTestAccess::cutoffHeld(completedHold, 0);
    expect(completedLatch.valid && completedLatch.ordinal == vcfOrdinal
               && YouKnowTestAccess::cutoffTarget(completedHold, 0)
                      != completedLatch.target,
           "the serial-restart fixture did not complete a fractional VCF write");
    completedHold.noteOn(67, 1.0f);
    expect(!YouKnowTestAccess::passiveHoldLatch(completedHold).valid
               && YouKnowTestAccess::cutoffTarget(completedHold, 0)
                      == completedLatch.target
               && YouKnowTestAccess::cutoffHeld(completedHold, 0)
                      == heldAfterWrite
               && YouKnowTestAccess::controlScanPhase(completedHold) == 0.0
               && YouKnowTestAccess::nextConverterWrite(completedHold)
                      == 0u,
    ...
    const auto lfoFixture = [&](double phase) {
        ...
        engine->noteOn(60, 1.0f);
        YouKnowTestAccess::setLfoRestartState(*engine);
        YouKnowTestAccess::setConverterScheduler(
            *engine, phase, phase >= 1.0
                ? YouKnowTestAccess::converterWriteCount() : 17u);
        engine->setParameters(restartedLfoParameters);
        return engine;
    };

    auto midPassLfo = lfoFixture(0.75);
    const float midPassLfoValue = YouKnowTestAccess::lfoValue(*midPassLfo);
    const std::uint16_t midPassPwm =
        YouKnowTestAccess::converterPassPwmDacCode(*midPassLfo);
    const std::int32_t midPassPitchWord =
        YouKnowTestAccess::dcoLfoPitchWord(*midPassLfo);
    midPassLfo->noteOn(64, 1.0f);
    expect(YouKnowTestAccess::controlScanPhase(*midPassLfo) == 0.0
               && YouKnowTestAccess::lfoDelayHoldoff(*midPassLfo) == 0x4000u
               && YouKnowTestAccess::lfoDelayFade(*midPassLfo) == 0x4100u
               && YouKnowTestAccess::lfoDelayByte(*midPassLfo) == 0x41u
               && YouKnowTestAccess::lfoValue(*midPassLfo)
                      == midPassLfoValue
               && YouKnowTestAccess::converterPassPwmDacCode(*midPassLfo)
                      == midPassPwm
               && YouKnowTestAccess::dcoLfoPitchWord(*midPassLfo)
    ...
    const float dueValue = YouKnowTestAccess::lfoValue(*dueLfo);
    const std::uint16_t duePwm =
        YouKnowTestAccess::converterPassPwmDacCode(*dueLfo);
    dueLfo->noteOn(64, 1.0f);
    expect(YouKnowTestAccess::controlScanPhase(*dueLfo) == 1.0
               && YouKnowTestAccess::lfoDelayHoldoff(*dueLfo) == 0x4000u
               && YouKnowTestAccess::lfoDelayFade(*dueLfo) == 0x4000u
    ...
    expect(YouKnowTestAccess::lfoDelayHoldoff(*dueLfo) == 0x4000u
               && YouKnowTestAccess::lfoDelayFade(*dueLfo) == 0x4100u
               && YouKnowTestAccess::lfoDelayByte(*dueLfo) == 0x41u
               && YouKnowTestAccess::lfoValue(*dueLfo) != dueValue,
...
void testConverterAnchoredPitchPrestageTimelineAndAtomicCommit()
{
    ...
    for (const bool coldReset : { false, true })
    {
        ...
        if (!coldReset)
        {
            YouKnowTestAccess::setMode3Running(
                engine, 0, active, true, 10000.25);
            YouKnowTestAccess::setDcoResetPending(engine, 0, false);
        }

        const std::size_t ordinal = YouKnowTestAccess::pitchOrdinal(0);
        const double phaseStep =
            YouKnowTestAccess::scanPhasePerInternalSample(engine);
        ...
        const float oldCv = YouKnowTestAccess::dcoCv(engine, 0);
        const std::uint16_t oldEnvelope =
            YouKnowTestAccess::envelopeLevel(engine, 0);
        processOne();
        expect(!YouKnowTestAccess::dcoPitchTransactionValid(engine, 0)
                   && YouKnowTestAccess::pitWriteState(engine, 0)
                          == YouKnowTestAccess::idlePitWriteState()
                   && YouKnowTestAccess::envelopeLevel(engine, 0)
                          == oldEnvelope,
        ...
        YouKnowTestAccess::scheduleUpcomingDcoPitchPrestages(engine);
        expect(YouKnowTestAccess::pitWriteState(engine, 0)
                   == YouKnowTestAccess::awaitingPitchPrestageWriteState()
                   && std::abs(
                       YouKnowTestAccess::cpuStatesToWrite(engine, 0)
                       - 41.0 / 6.0) < 1.0e-9,
        ...
        auto resetBeforePrestage =
            std::make_unique<YouKnowEngine>(engine);
        resetBeforePrestage->reset();
        expect(YouKnowTestAccess::pitWriteState(
        ...
        if (!coldReset)
            YouKnowTestAccess::stageMode3Count(engine, 0, oldPending);
        processOne();
        const std::uint32_t capturedDivider =
            YouKnowTestAccess::pitWriteDivider(engine, 0);
        const float capturedCv =
            YouKnowTestAccess::dcoPitchTransactionCvTarget(engine, 0);
        ...
        expect(YouKnowTestAccess::dcoPitchTransactionValid(engine, 0)
                   ...
                   && std::abs(
                       YouKnowTestAccess::cpuStatesToWrite(engine, 0)
                       - 41.0) < 1.0e-9
                   && !YouKnowTestAccess::dcoResetPending(engine, 0),
        ...
        auto resetAfterPrestage = std::make_unique<YouKnowEngine>(engine);
        resetAfterPrestage->reset();
        expect(!YouKnowTestAccess::dcoPitchTransactionValid(
        ...
        engine.setParameters(parameters);
        YouKnowTestAccess::setDcoResetPending(engine, 0, true);
        ...
        for (int frame = 0; frame < 15; ++frame)
            processOne();
        expect(YouKnowTestAccess::nextConverterWrite(engine) == ordinal
                   && YouKnowTestAccess::dcoPitchTransactionValid(engine, 0)
                   && YouKnowTestAccess::dcoCvTarget(engine, 0)
                          == oldCvTarget
                   && YouKnowTestAccess::dcoPitchTransactionCvTarget(
                          engine, 0) == capturedCv
                   && YouKnowTestAccess::envelopeLevel(engine, 0)
                          == capturedEnvelope
                   && YouKnowTestAccess::dcoResetPending(engine, 0),
        ...
        processOne();
        expect(YouKnowTestAccess::nextConverterWrite(engine)
                       == ordinal + 1u
                   && !YouKnowTestAccess::dcoPitchTransactionValid(
                          engine, 0)
                   && YouKnowTestAccess::dcoCvTarget(engine, 0)
               ...
               context + " T recomputed or failed to commit its captured "
                         "pitch payload");
        if (coldReset)
            expect(YouKnowTestAccess::dcoCv(engine, 0) == capturedCv
                       && oldCv != capturedCv,
...
void testConverterAnchoredPitchPrestageIsWallClockInvariant()
{
    ...
    std::array<Fixture, 3> fixtures {{
        { std::make_unique<YouKnowEngine>(), 1 },
        { std::make_unique<YouKnowEngine>(), 1 },
        { std::make_unique<YouKnowEngine>(), 4 },
    }};
    ...
    for (auto& fixture : fixtures)
    {
        ...
        YouKnowTestAccess::setDcoResetPending(*fixture.engine, 0, false);
        const std::size_t ordinal =
            YouKnowTestAccess::pitchOrdinal(0);
        const double phaseStep =
            YouKnowTestAccess::scanPhasePerInternalSample(*fixture.engine);
        const double internalFramesToT = 20.0
            / (48000.0 * YouKnowTestAccess::internalIntervalSeconds(
                *fixture.engine));
        YouKnowTestAccess::setConverterScheduler(
            *fixture.engine,
            YouKnowTestAccess::converterEventPhase(
    ...
    expect(all([](const YouKnowEngine& engine) {
               return !YouKnowTestAccess::dcoPitchTransactionValid(
                          engine, 0)
                   && YouKnowTestAccess::pitWriteState(engine, 0)
                          == YouKnowTestAccess::idlePitWriteState();
           }),
    ...
    advanceWallFrames(1);
    const std::uint32_t capturedDivider =
        YouKnowTestAccess::pitWriteDivider(*fixtures[0].engine, 0);
    const float capturedCv = YouKnowTestAccess::
        dcoPitchTransactionCvTarget(*fixtures[0].engine, 0);
    const std::uint16_t capturedEnvelope =
        YouKnowTestAccess::envelopeLevel(*fixtures[0].engine, 0);
    expect(all([capturedDivider, capturedCv,
                capturedEnvelope](const YouKnowEngine& engine) {
               return YouKnowTestAccess::dcoPitchTransactionValid(
                          engine, 0)
                   && YouKnowTestAccess::pitWriteState(engine, 0)
                          == YouKnowTestAccess::awaitingMsbWriteState()
    ...
    expect(all([](const YouKnowEngine& engine) {
               return YouKnowTestAccess::dcoPitchTransactionValid(
                          engine, 0)
                   && YouKnowTestAccess::nextConverterWrite(engine)
                          == YouKnowTestAccess::pitchOrdinal(0);
           }),
           "one processing grid committed the captured CV before T");
    advanceWallFrames(1);
    expect(all([capturedCv](const YouKnowEngine& engine) {
               return !YouKnowTestAccess::dcoPitchTransactionValid(
                          engine, 0)
                   && YouKnowTestAccess::dcoCvTarget(engine, 0)
                          == capturedCv
                   && YouKnowTestAccess::nextConverterWrite(engine)
                          == YouKnowTestAccess::pitchOrdinal(0) + 1u;
           }),
...
void testPitchPrestageConsumesResetDiscoveredByItsOwnScan()
{
    ...
    engine.setParameters(parameters);
    engine.noteOn(60, 1.0f);
    YouKnowTestAccess::updateVoiceScan(engine, 0, parameters);
    YouKnowTestAccess::setDcoResetPending(engine, 0, false);
    engine.noteOff(60);
    parameters.keyTranspose = 12;
    engine.setParameters(parameters);
    ...
    YouKnowTestAccess::setRangeClockTransition(engine, 1.5, 0.75);
    YouKnowTestAccess::setDcoResetPending(engine, 0, false);
    const std::size_t ordinal = YouKnowTestAccess::pitchOrdinal(0);
    const double phaseStep =
        YouKnowTestAccess::scanPhasePerInternalSample(engine);
    ...
    expect(YouKnowTestAccess::lastVoiceMidi(engine, 0) == 72
               ...
               && std::abs(
                      YouKnowTestAccess::pitClocksToEvent(engine, 0)
                      - YouKnowTestAccess::rangeClockFallingClocks(engine))
                      < 1.0e-10
               && !YouKnowTestAccess::dcoResetPending(engine, 0),
...
void testQualitySwitchWaitsForConverterAnchoredPitchTransaction()
{
    ...
    expect(!YouKnowTestAccess::forceQualitySwitchAtZero(engine, 1)
...
void testPhaseZeroPrestagesAllSixPhysicalCardsButNoExtension()
{
    ...
    for (int slot = 0; slot < YouKnowEngine::hardwareVoices; ++slot)
    {
        ...
        YouKnowTestAccess::setDcoResetPending(engine, slot, false);
    }
    ...
    YouKnowTestAccess::setConverterScheduler(
        ...
        YouKnowTestAccess::converterWriteCount());
    float left = 0.0f;
    float right = 0.0f;
    engine.process(&left, &right, 1);
    ...
    for (int slot = 0; slot < YouKnowEngine::hardwareVoices; ++slot)
        allPhysicalCapturedTogether = allPhysicalCapturedTogether
            && YouKnowTestAccess::dcoPitchTransactionValid(engine, slot)
            && YouKnowTestAccess::pitWriteState(engine, slot)
                   == YouKnowTestAccess::awaitingLsbWriteState()
            && std::abs(YouKnowTestAccess::cpuStatesToWrite(engine, slot)
                        - 41.0) < 1.0e-9;
    constexpr int extensionSlot = YouKnowEngine::hardwareVoices;
    expect(allPhysicalCapturedTogether,
           "Phase Zero did not prestage all six physical cards together");
    expect(!YouKnowTestAccess::dcoPitchTransactionValid(
                   engine, extensionSlot)
               && YouKnowTestAccess::pitWriteState(engine, extensionSlot)
                      == YouKnowTestAccess::idlePitWriteState()
               && YouKnowTestAccess::cpuStatesToWrite(
...
void testRangeDoesNotRetargetDcoCompensationCv()
{
    ...
    engine.setParameters(parameters);
    engine.noteOn(69, 1.0f);
    YouKnowTestAccess::performPitchWrite(engine, 0, parameters);

    const std::uint32_t count =
        YouKnowTestAccess::pitWriteDivider(engine, 0);
    const float fixedCv = YouKnowTestAccess::dcoCvTarget(engine, 0);
    YouKnowTestAccess::setMode3Running(
        engine, 0, count, true, 103.25, DcoRange::Eight);
    const double eightFootPeriod =
        YouKnowTestAccess::dcoPeriodSamples(engine, 0);
    ...
    engine.setParameters(parameters);
    expect(YouKnowTestAccess::rangeClockTransitionPending(engine)
               && YouKnowTestAccess::pitClocksToEvent(engine, 0)
                      != countdownBefore,
    ...
    while (YouKnowTestAccess::dcoPitchTransactionValid(engine, 0)
           && framesToCommit < 32)
    {
        engine.process(&left, &right, 1);
        ++framesToCommit;
    ...
    expect(framesToCommit < 32
               && YouKnowTestAccess::dcoCvTarget(engine, 0) == fixedCv
               && YouKnowTestAccess::dcoCv(engine, 0) == fixedCv,
           "a RANGE-only pitch scan committed or slewed a new DCO CV");
    expect(YouKnowTestAccess::dcoPeriodSamples(engine, 0)
               == eightFootPeriod / 2.0,
...
void testDcoPitchPairKeepsSettledRampProductAndCvSaturation()
{
    ...
    const auto scaleFor = [&](std::uint16_t pitchWord) {
        const auto pair = YouKnowEngine::dcoPitchPair(pitchWord);
        YouKnowTestAccess::setMode3Running(
        ...
        YouKnowTestAccess::setDcoLaunchState(
            ...
            static_cast<float>(pair.cvCode));
        return YouKnowTestAccess::dcoLaunchScale(engine, 0);
    };
...
void testDcoAndNoiseHoldsAcquireAtTheWrite()
{
    ...
    const float unitCode = YouKnowTestAccess::dcoCvCodeForScale(
        engine, 0, 1.0f);
    YouKnowTestAccess::setDcoLaunchState(
    ...
    expect(YouKnowTestAccess::dcoCv(engine, 0) == 4095.0f,
...
void testResonanceHoldStepsAtTheWrite()
{
    ...
    YouKnowTestAccess::setResonanceHold(
        engine, staleHold, YouKnowTestAccess::resonanceTarget(engine));
    const float heldBefore = YouKnowTestAccess::resonanceHeld(engine);
    ...
    expect(YouKnowTestAccess::resonanceHeld(engine) == latch.target,
           "the RESO CV hold did not step to its target at the write");
    const auto stepped = YouKnowTestAccess::resonanceVcfHoldNodes(engine);
    bool sawOld = false;
    bool sawNew = false;
...
{
    expect(YouKnowEngine::masterTunePitchWordOffset(-50.0) == -128
               && YouKnowEngine::masterTunePitchWordOffset(0.0) == 0
               && YouKnowEngine::masterTunePitchWordOffset(50.0) == 127,
           "MASTER TUNE left the B-2 signed-byte endpoint law");
    expect(YouKnowEngine::masterTunePitchWordOffset(-900.0) == -128
               && YouKnowEngine::masterTunePitchWordOffset(900.0) == 127
    ...
    const auto renderedPair = [](float cents) {
        ...
        engine.noteOn(60, 1.0f);
        YouKnowTestAccess::performPitchWrite(engine, 0, parameters);
        return YouKnowEngine::DcoPitchPair {
            YouKnowTestAccess::pitWriteDivider(engine, 0),
    ...
    const auto expectedLow = YouKnowEngine::dcoPitchPair(0x5398u);
    const auto expectedNeutral = YouKnowEngine::dcoPitchPair(0x5418u);
    const auto expectedHigh = YouKnowEngine::dcoPitchPair(0x5497u);
    ...
    const auto oldSymmetricEndpoint = YouKnowEngine::dcoPitchPair(0x5498u);
...
void testPitchBendUsesRecoveredIntegerWord()
{
    ...
    const auto bendWord = [&](int wheel, float depth = 1.0f) {
        return YouKnowEngine::dcoPitchBendWordOffset(
    ...
    expect(YouKnowEngine::dcoPitchBendWordOffset(
               std::numeric_limits<float>::quiet_NaN(), 1.0f) == 0
               && YouKnowEngine::dcoPitchBendWordOffset(
    ...
    const auto expectedLow = YouKnowEngine::dcoPitchPair(0x4821u);
    const auto expectedCentre = YouKnowEngine::dcoPitchPair(0x5418u);
    const auto expectedHigh = YouKnowEngine::dcoPitchPair(0x600fu);
    expect(low.divider == expectedLow.divider
               && low.cvCode == expectedLow.cvCode
    ...
    const auto oldIdealEndpoint = YouKnowEngine::dcoPitchPair(0x6018u);
    ...
    YouKnowTestAccess::performPitchWrite(held, 0, zeroDepth);
    expect(YouKnowTestAccess::pitWriteDivider(held, 0)
...
void testDcoLfoUsesRecoveredIntegerWord()
{
    ...
    {
        depthHash ^= YouKnowEngine::dcoLfoDepthScale(
            static_cast<std::uint8_t>(code));
        depthHash *= 16777619u;
    }
    expect(depthHash == 0x5c5f010cu,
           "the generated B-2 DCO-LFO depth table changed");
    expect(YouKnowEngine::dcoLfoDepthScale(2u) == 0u
               && YouKnowEngine::dcoLfoDepthScale(3u) == 1u
               && YouKnowEngine::dcoLfoDepthScale(64u) == 62u
               && YouKnowEngine::dcoLfoDepthScale(65u) == 64u
               && YouKnowEngine::dcoLfoDepthScale(95u) == 124u
               && YouKnowEngine::dcoLfoDepthScale(96u) == 128u
               && YouKnowEngine::dcoLfoDepthScale(124u) == 240u
               && YouKnowEngine::dcoLfoDepthScale(125u) == 248u
               && YouKnowEngine::dcoLfoDepthScale(126u) == 255u
               && YouKnowEngine::dcoLfoDepthScale(127u) == 255u
               && YouKnowEngine::dcoLfoDepthScale(255u) == 255u,
    ...
    const auto word = [](std::uint8_t delay, std::uint8_t panel,
                         ...
                         bool positive = true) {
        return YouKnowEngine::dcoLfoPitchWordOffset(
    ...
    const auto base = YouKnowEngine::dcoPitchPair(0x5418u);
    const auto controller = YouKnowEngine::dcoPitchPair(0x5612u);
...
void testPwmUsesRecoveredIntegerDacWord()
{
    ...
    const float panelMaximumVolts = YouKnowEngine::pwmDacVolts(code(101));
    expectNear(panelMaximumVolts, 0.634725275, 1.0e-6,
               "the loaded PWM panel maximum missed its B-2 DAC code");
    expectNear(YouKnowEngine::pwmDutyCycle(panelMaximumVolts),
               0.947106227, 1.0e-6,
               "the loaded PWM panel maximum left Roland's 93-97% window");
    expect(YouKnowEngine::pwmDutyCycle(
               YouKnowEngine::pwmDacVolts(code(127))) == 1.0f,
    ...
    held.noteOn(60, 1.0f);
    renderExact(held, 1);
    expect(YouKnowTestAccess::converterPassPwmDacCode(held) == 0x0360u,
           "the converter pass did not latch B-2's PWM word");

    auto zeroDepth = physicalMaximum;
    zeroDepth.pwmDepth = 0.0f;
    held.setParameters(zeroDepth);
    YouKnowTestAccess::performPwmWrite(held, zeroDepth);
    expect(YouKnowTestAccess::pwmTarget(held) == panelMaximumVolts,
           "mid-pass automation recomputed the pending PWM word");

    YouKnowTestAccess::setConverterScheduler(
        held, 1.0, YouKnowTestAccess::converterWriteCount());
    renderExact(held, 1);
    expect(YouKnowTestAccess::converterPassPwmDacCode(held) == 0x0fffu,
           "the following pass did not adopt the new PWM byte");
    YouKnowTestAccess::performPwmWrite(held, zeroDepth);
    expect(YouKnowTestAccess::pwmTarget(held)
               == YouKnowEngine::pwmDacVolts(0x0fffu),
           "the following PWM write did not consume its new pass latch");
}

void testLfoDelayStartsFadeOnHoldoffCrossingPass()
{
    YouKnowEngine fastest;
    fastest.prepare(192000.0, blockSize, false);
    auto fast = plainPatch();
    fast.lfoDelay = 0.0f;
    fastest.setParameters(fast);
    fastest.noteOn(60, 1.0f);
    YouKnowTestAccess::advanceLfo(fastest, fast);
    expect(YouKnowTestAccess::lfoDelayHoldoff(fastest) == 0x4000u
               && YouKnowTestAccess::lfoDelayFade(fastest) == 0xffffu
               && YouKnowTestAccess::lfoDelayByte(fastest) == 0xffu
               && YouKnowTestAccess::lfoDelayLevel(fastest) == 1.0f,
           "fast LFO delay did not fade on its holdoff-crossing pass");
    YouKnowTestAccess::advanceLfo(fastest, fast);
    expect(YouKnowTestAccess::lfoDelayFade(fastest) == 0x10000u
               && YouKnowTestAccess::lfoDelayByte(fastest) == 0xffu,
           "fast LFO delay did not complete on its second pass");

    YouKnowEngine slowest;
    ...
    for (int pass = 0; pass < 780; ++pass)
        YouKnowTestAccess::advanceLfo(slowest, slow);
    expect(YouKnowTestAccess::lfoDelayHoldoff(slowest) == 16380u
               && YouKnowTestAccess::lfoDelayFade(slowest) == 0u
               && YouKnowTestAccess::lfoDelayByte(slowest) == 0u,
           "slow LFO delay began fading before its hold threshold");
    YouKnowTestAccess::advanceLfo(slowest, slow);
    expect(YouKnowTestAccess::lfoDelayHoldoff(slowest) == 0x4000u
               && YouKnowTestAccess::lfoDelayFade(slowest) == 256u
               && YouKnowTestAccess::lfoDelayByte(slowest) == 1u,
...
void testRangeDividerCompletesItsCurrentSynchronousCount()
{
    ...
    expect(!YouKnowTestAccess::rangeClockTransitionPending(cold)
               && YouKnowTestAccess::rangeClockFallingClocks(cold) == 1.0
               && YouKnowTestAccess::rangeClockReloadClocks(cold) == 0.0,
           "a startup RANGE snapshot manufactured an IC35 handoff");
    YouKnowTestAccess::programDcoCount(cold, 0, 128u, true);
    expect(YouKnowTestAccess::pitClocksToEvent(cold, 0) == 1.0,
    ...
    for (const auto& handoff : handoffs)
    {
        ...
        const double actualReload =
            YouKnowTestAccess::rangeClockReloadClocks(engine);
        expect(YouKnowTestAccess::rangeClockTransitionPending(engine)
                   && YouKnowTestAccess::activeDcoDivider(engine, 0)
                          == 128u
                   && YouKnowTestAccess::pitOutHigh(engine, 0)
        ...
        const double clockHz =
            YouKnowTestAccess::rangeClockHz(handoff.next);
        const double rawTickClocks =
            clockHz / YouKnowTestAccess::masterClockHz();
        expectNear(
            YouKnowTestAccess::rangeClockClocksToNextFallingEdge(
    ...
    YouKnowTestAccess::setMode3Running(
        ...
        DcoRange::Eight);
    YouKnowTestAccess::setRangeClockPhase(exactFalling, 1.0);
    parameters.range = DcoRange::Four;
    exactFalling.setParameters(parameters);
    expectNear(YouKnowTestAccess::rangeClockFallingClocks(exactFalling),
               1.0, 1.0e-12,
               "an exact falling-edge PF write moved the next PIT count");
    expectNear(YouKnowTestAccess::rangeClockReloadClocks(exactFalling),
               0.5, 1.0e-12,
               "an exact falling-edge PF write missed the upcoming reload");
    expectNear(YouKnowTestAccess::pitClocksToEvent(exactFalling, 0),
               10.0, 1.0e-12,
    ...
    parameters.range = DcoRange::Four;
    shared.setParameters(parameters);
    expectNear(YouKnowTestAccess::rangeClockFallingClocks(shared),
               0.8, 1.0e-12,
               "six-card handoff lost the shared TP5 falling phase");
    expectNear(YouKnowTestAccess::rangeClockReloadClocks(shared),
               1.3, 1.0e-12,
               "six-card handoff collapsed reload onto TP5 falling");
    for (int slot = 0; slot < 6; ++slot)
        expectNear(
            YouKnowTestAccess::pitClocksToEvent(shared, slot),
    ...
    for (int slot = 0; slot < 6; ++slot)
        YouKnowTestAccess::advanceDcoPitAndRampThreshold(
            ...
            false, false, false);
    YouKnowTestAccess::advanceRangeClock(shared, DcoRange::Four);
    expect(!YouKnowTestAccess::rangeClockTransitionPending(shared),
           "the shared reload survived one physical chassis-clock advance");
    for (int slot = 0; slot < 6; ++slot)
    {
        double phase = std::fmod(
            YouKnowTestAccess::pitClocksToEvent(shared, slot), 1.0);
        if (phase <= 1.0e-12)
            phase = 1.0;
        expectNear(phase,
                   YouKnowTestAccess::rangeClockFallingClocks(shared),
                   1.0e-10,
    ...
    parameters.range = DcoRange::Four;
    repeated.setParameters(parameters);
    expectNear(YouKnowTestAccess::rangeClockFallingClocks(repeated),
               3.0, 1.0e-12,
               "a slow-to-fast handoff reduced its first falling deadline");
    expectNear(YouKnowTestAccess::rangeClockReloadClocks(repeated),
               3.5, 1.0e-12,
               "a slow-to-fast handoff lost its later reload deadline");
    expectNear(YouKnowTestAccess::pitClocksToEvent(repeated, 0),
               5.0, 1.0e-12,
               "a slow-to-fast handoff lost a PIT edge ordinal");
    const double fourClockHz =
        YouKnowTestAccess::rangeClockHz(DcoRange::Four);
    const auto fallingAfter = [&](double clocks) {
        return YouKnowTestAccess::rangeClockClocksToNextFallingEdge(
    ...
    YouKnowEngine loadBetweenEdges;
    ...
    YouKnowTestAccess::setRangeClockTransition(
        loadBetweenEdges, 1.5, 1.75);
    YouKnowTestAccess::programDcoCount(
        loadBetweenEdges, 0, replacement, true);
    expect(YouKnowTestAccess::pitState(loadBetweenEdges, 0)
                   == YouKnowTestAccess::awaitingCountState()
               && YouKnowTestAccess::pitClocksToEvent(
    ...
    expect(YouKnowTestAccess::rangeClockTransitionPending(
                   loadBetweenEdges)
               && YouKnowTestAccess::rangeClockReloadClocks(
                      loadBetweenEdges) > 0.0
               && YouKnowTestAccess::activeDcoDivider(
                      loadBetweenEdges, 0)
                      == replacement,
           "the completed count did not load between falling and reload: "
               + std::to_string(YouKnowTestAccess::activeDcoDivider(
                   loadBetweenEdges, 0)) + ", "
               + std::to_string(YouKnowTestAccess::rangeClockReloadClocks(
                   loadBetweenEdges)) + ", "
               + std::to_string(YouKnowTestAccess::rangeClockTransitionPending(
                   loadBetweenEdges)));
    ...
    const double oldPeriod =
        YouKnowTestAccess::dcoPeriodSamples(periodGuard, 0);
    parameters.range = DcoRange::Four;
    periodGuard.setParameters(parameters);
    YouKnowTestAccess::advanceDcoAndSharedRangeClock(
        periodGuard, 0, DcoRange::Four);
    expect(!YouKnowTestAccess::rangeClockTransitionPending(periodGuard)
               && YouKnowTestAccess::pitOutHigh(periodGuard, 0)
               && YouKnowTestAccess::dcoPeriodSamples(periodGuard, 0)
                      == oldPeriod,
           "IC35 reload retuned an in-progress PIT half-cycle: "
               + std::to_string(YouKnowTestAccess::dcoPeriodSamples(
                   periodGuard, 0)) + ", "
               + std::to_string(oldPeriod) + ", "
               + std::to_string(YouKnowTestAccess::pitOutHigh(
                   periodGuard, 0)) + ", "
               + std::to_string(YouKnowTestAccess::rangeClockTransitionPending(
                   periodGuard)));
    for (int frame = 0;
         frame < 16 && YouKnowTestAccess::pitOutHigh(periodGuard, 0);
         ++frame)
    {
        YouKnowTestAccess::advanceDcoAndSharedRangeClock(
            periodGuard, 0, DcoRange::Four);
    }
    expect(!YouKnowTestAccess::pitOutHigh(periodGuard, 0),
           "the period guard never reached its next PIT OUT transition");
    expectNear(YouKnowTestAccess::dcoPeriodSamples(periodGuard, 0),
    ...
    YouKnowEngine reloadInside;
    YouKnowEngine reloadOutside;
    reloadInside.prepare(192000.0, blockSize, false);
    reloadOutside.prepare(192000.0, blockSize, false);
    const double frameClocks =
        YouKnowTestAccess::rangeClockHz(DcoRange::Four)
        * YouKnowTestAccess::internalIntervalSeconds(reloadInside);
    const double aperture = frameClocks * 1.0e-9;
    const auto armReloadBoundary = [&](YouKnowEngine& timed,
                                       double reload) {
        parameters.range = DcoRange::Four;
    ...
    expect(!YouKnowTestAccess::rangeClockTransitionPending(reloadInside)
               && YouKnowTestAccess::pitOutHigh(reloadInside, 0)
               && YouKnowTestAccess::rangeClockFallingClocks(reloadInside)
                      == 0.5,
           "a coalesced IC35 reload was treated as a PIT count");
    expect(YouKnowTestAccess::rangeClockTransitionPending(reloadOutside)
               && YouKnowTestAccess::pitOutHigh(reloadOutside, 0)
               && YouKnowTestAccess::rangeClockReloadClocks(reloadOutside)
                      > aperture,
    ...
    for (const double apertureMultiple : { -0.5, 0.5, 2.0 })
    {
        ...
        expect(YouKnowTestAccess::pitOutHigh(stable, 0)
                       == !edgeConsumed
                   && (edgeConsumed
                       ? YouKnowTestAccess::rangeClockFallingClocks(stable)
                              == 1.0
                       : YouKnowTestAccess::rangeClockFallingClocks(stable)
                              > aperture),
    ...
    const auto advance = [](YouKnowEngine& timed, int frames) {
        for (int frame = 0; frame < frames; ++frame)
            YouKnowTestAccess::advanceDcoAndSharedRangeClock(
                timed, 0, DcoRange::Four);
    };
    ...
    advance(at48kHq, 4);
    advance(at192k, 4);
    const auto sameCountdowns = [](const YouKnowEngine& left,
                                   const YouKnowEngine& right) {
        return std::abs(
                   YouKnowTestAccess::pitClocksToEvent(left, 0)
                   - YouKnowTestAccess::pitClocksToEvent(right, 0))
                   < 1.0e-9
            && std::abs(
                   YouKnowTestAccess::rangeClockFallingClocks(left)
                   - YouKnowTestAccess::rangeClockFallingClocks(right))
                   < 1.0e-9;
    };
    ...
    const double periodBeforeRebuild =
        YouKnowTestAccess::dcoPeriodSamples(rebuilt, 0);
    parameters.range = DcoRange::Four;
    rebuilt.setParameters(parameters);
    const double pitBeforeRebuild =
        YouKnowTestAccess::pitClocksToEvent(rebuilt, 0);
    const double fallingBeforeRebuild =
        YouKnowTestAccess::rangeClockFallingClocks(rebuilt);
    const double reloadBeforeRebuild =
        YouKnowTestAccess::rangeClockReloadClocks(rebuilt);
    YouKnowTestAccess::rebuildAtOversamplingFactor(rebuilt, 1);
    expectNear(YouKnowTestAccess::dcoPeriodSamples(rebuilt, 0),
               periodBeforeRebuild / 4.0, 1.0e-12,
               "a quality rebuild adopted RANGE before PIT OUT");
    expect(YouKnowTestAccess::pitClocksToEvent(rebuilt, 0)
                   == pitBeforeRebuild
               && YouKnowTestAccess::rangeClockFallingClocks(rebuilt)
    ...
    expect(YouKnowTestAccess::rangeClockTransitionPending(repeated)
               && YouKnowTestAccess::rangeClockReloadClocks(repeated)
                      == 1.75,
           "the reset fixture no longer held an in-flight reload");
    repeated.reset();
    expect(!YouKnowTestAccess::rangeClockTransitionPending(repeated)
               && YouKnowTestAccess::rangeClockFallingClocks(repeated)
                      == 1.0
               && YouKnowTestAccess::rangeClockReloadClocks(repeated)
                      == 0.0,
...
void testControlWordConverterWritePreservesCardState()
{
    ...
    const double periodBefore =
        YouKnowTestAccess::dcoPeriodSamples(engine, 0);
    const float cvBefore = YouKnowTestAccess::dcoCv(engine, 0);
    const float cvTargetBefore =
        YouKnowTestAccess::dcoCvTarget(engine, 0);
    const auto filterBefore = YouKnowTestAccess::filterState(engine, 0);
    const auto noiseBefore =
        YouKnowTestAccess::microscopicNoiseState(engine, 0);
    const auto sawBefore = YouKnowTestAccess::sawTrackState(engine, 0);
    const auto pulseBefore = YouKnowTestAccess::pulseTrackState(engine, 0);
    const auto subBefore = YouKnowTestAccess::subTrackState(engine, 0);
    ...
    expect(YouKnowTestAccess::dcoCv(engine, 0) == cvBefore,
           "the control-word write bypassed the physical compensation hold");
    expect(YouKnowTestAccess::dcoCvTarget(engine, 0) != cvTargetBefore,
           "the converter did not update the compensation target");
    expect(YouKnowTestAccess::filterState(engine, 0) == filterBefore,
           "the control-word write reset the continuously powered filter");
    expect(YouKnowTestAccess::microscopicNoiseState(engine, 0) == noiseBefore,
    ...
    expect(historyPreserved(
               sawBefore, YouKnowTestAccess::sawTrackState(engine, 0))
               && historyPreserved(
                   pulseBefore,
                   YouKnowTestAccess::pulseTrackState(engine, 0))
               && historyPreserved(
                   subBefore, YouKnowTestAccess::subTrackState(engine, 0)),
    ...
    engine.reset();
    expect(!YouKnowTestAccess::sawTrackState(engine, 0).primed
               && !YouKnowTestAccess::pulseTrackState(engine, 0).primed
               && !YouKnowTestAccess::subTrackState(engine, 0).primed,
...
void testWideDownwardRetargetStaysOnRampRails()
{
    ...
    for (const auto& fixture : cases)
    {
        ...
        const float subBefore =
            YouKnowTestAccess::dcoLogicStates(engine, 0)[1];
        YouKnowTestAccess::performPitchWrite(engine, 0, parameters);
        const std::uint32_t requested =
            YouKnowTestAccess::pitWriteDivider(engine, 0);
        ...
        if (fixture.writesControlWord && !fixture.startsOutHigh)
        {
            expect(subReference != subBefore
                       && YouKnowTestAccess::dcoRampState(engine, 0)[1] < 0.0,
        ...
        const auto processOne = [&] {
            ...
            const double rampVolts =
                YouKnowTestAccess::dcoRampVolts(engine, 0);
        ...
        {
            processOne();
            expect(YouKnowTestAccess::activeDcoDivider(engine, 0)
                           == activeBefore
                       && YouKnowTestAccess::pendingDcoDividerValid(
                           engine, 0)
                       && YouKnowTestAccess::pendingDcoDivider(engine, 0)
            ...
            {
                expect(YouKnowTestAccess::dcoLogicStates(engine, 0)[1]
                           == subReference,
        ...
        {
            processOne();
            const auto ramp = YouKnowTestAccess::dcoRampState(engine, 0);
            const float sub =
                YouKnowTestAccess::dcoLogicStates(engine, 0)[1];
            if (sub != subReference)
            {
                ...
                expect(YouKnowTestAccess::pitOutHigh(engine, 0)
                           && (ramp[0]
                                   < YouKnowTestAccess::dcoPositiveBaseRail(
                                       engine, 0)
                               || ramp[1] < 0.0),
                       std::string(fixture.name)
...
void testPhysicalRampSupplyBoundUsesTotalScaleAndCoalesces()
{
    ...
    const double positiveRail =
        YouKnowTestAccess::dcoPositiveBaseRail(held, 0);
    const double expectedBaseRail = 2.5 / totalScale - 1.0;
    const double interval =
        YouKnowTestAccess::internalIntervalSeconds(held);
    const double risingSlope = 0.4 / interval;
    YouKnowTestAccess::setDcoRampState(
        held, 0, positiveRail - 0.2, risingSlope);
    YouKnowTestAccess::primeSawTrack(held, 0);
    YouKnowTestAccess::primePulseTrack(held, 0, -1.0f);
    YouKnowTestAccess::advanceDcoPitAndRampThreshold(
        held, 0, DcoRange::Eight, 20.0f, 20.0f, false, false, true);

    const auto heldRamp = YouKnowTestAccess::dcoRampState(held, 0);
    const auto heldSaw = YouKnowTestAccess::sawTrackState(held, 0);
    const auto heldPulse = YouKnowTestAccess::pulseTrackState(held, 0);
    const auto ringEnergy = [](const auto& ring) {
        double energy = 0.0;
        for (const float value : ring)
            energy += static_cast<double>(value) * value;
    ...
    expect(YouKnowTestAccess::positiveRailHeld(held, 0),
           "a genuine +15 V charge hit did not latch the supply hold");
    expectNear(YouKnowTestAccess::dcoRampVolts(held, 0), 15.0, 1.0e-12,
    ...
    expect(ringEnergy(heldPulse.ring) == 0.0
               && YouKnowTestAccess::dcoLogicStates(held, 0)[0] == -1.0f,
           "the supply plateau fabricated a pinned-comparator edge");
    YouKnowTestAccess::advanceDcoPitAndRampThreshold(
        held, 0, DcoRange::Eight, 20.0f, 20.0f, false, false, true);
    expect(YouKnowTestAccess::dcoRampState(held, 0)[0] == positiveRail
               && YouKnowTestAccess::dcoRampState(held, 0)[1] == 0.0
               && YouKnowTestAccess::dcoRampVolts(held, 0)
                      <= 15.0 + 1.0e-12,
    ...
    const auto heldResidual =
        YouKnowTestAccess::sawTrackState(held, 0).ring;
    YouKnowTestAccess::setDcoRampScales(
    ...
    expect(YouKnowTestAccess::dcoRampState(held, 0)[0]
                   == increasedScaleRail
               && YouKnowTestAccess::positiveRailHeld(held, 0)
               && YouKnowTestAccess::sawTrackState(held, 0).ring
                      == heldResidual,
    ...
    YouKnowEngine editedControlEdge;
    editedControlEdge.prepare(192000.0, blockSize, false);
    YouKnowTestAccess::setRampCurrentError(
    ...
    const double oldEditedRail =
        YouKnowTestAccess::dcoPositiveBaseRail(editedControlEdge, 0);
    YouKnowTestAccess::setDcoRampState(
        editedControlEdge, 0, oldEditedRail, 0.0);
    YouKnowTestAccess::setPositiveRailHeld(
        editedControlEdge, 0, true);
    YouKnowTestAccess::primeSawTrack(editedControlEdge, 0);
    editedParameters.calibration = 1.0f;
    editedControlEdge.setParameters(editedParameters);
    const double newEditedRail =
        YouKnowTestAccess::dcoPositiveBaseRail(editedControlEdge, 0);
    expect(newEditedRail != oldEditedRail
               && YouKnowTestAccess::dcoRampState(editedControlEdge, 0)[0]
                      == newEditedRail
               && YouKnowTestAccess::positiveRailHeld(
                      editedControlEdge, 0),
           "a calibration edit left the held ramp in its old coordinate");
    YouKnowTestAccess::programDcoCount(
        editedControlEdge, 0, 10000u, true);
    const auto editedDischarge =
        YouKnowTestAccess::dcoRampState(editedControlEdge, 0);
    ...
    expectNear(YouKnowTestAccess::dcoRampVolts(editedControlEdge, 0),
               15.0, 1.0e-12,
    ...
    const double fallingInterval =
        YouKnowTestAccess::internalIntervalSeconds(falling);
    ...
    YouKnowTestAccess::primeSawTrack(falling, 0);
    YouKnowTestAccess::setDcoRampScales(falling, 0, 1.0f, 1.25f);
    const double fallingRail =
        YouKnowTestAccess::dcoPositiveBaseRail(falling, 0);
    YouKnowTestAccess::advanceDcoPitAndRampThreshold(
        falling, 0, DcoRange::Eight, 20.0f, 20.0f, false, false, true);
    const auto fallingAfter =
        YouKnowTestAccess::dcoRampState(falling, 0);
    ...
    expect(fallingAfter[0] >= -1.0 && fallingAfter[0] < fallingRail
               ...
               && !YouKnowTestAccess::positiveRailHeld(falling, 0)
               && YouKnowTestAccess::dcoRampVolts(falling, 0) < 15.0
               && YouKnowTestAccess::sawTrackState(falling, 0).ring
                      == expectedClamp.ring,
    ...
    expect(YouKnowTestAccess::pitOutHigh(coincident, 0)
               && YouKnowTestAccess::dcoLogicStates(coincident, 0)[1] == 1.0f
               && after[1] < 0.0 && after[2] > 0.0
               && YouKnowTestAccess::dcoRampVolts(coincident, 0) < 15.0,
...
void testComparatorEndpointPinsAndTransitionTiming()
{
    ...
    // opposite edge and an end-of-sample repair on every period.
    {
        ...
        YouKnowTestAccess::primeDcoControlEdgeFixture(engine, 0);
        YouKnowTestAccess::primePulseTrack(engine, 0, 1.0f);
        for (int sample = 0; sample < 300; ++sample)
            YouKnowTestAccess::advanceDcoPitAndRampThreshold(
                engine, 0, DcoRange::Eight,
                -0.8f, -0.8f, true, true, true);

        const auto track = YouKnowTestAccess::pulseTrackState(engine, 0);
        expect(YouKnowTestAccess::dcoLogicStates(engine, 0)[0] == 1.0f
                   && track.primed && ringIsClean(track.ring)
                   && YouKnowTestAccess::nextPulseTrackSample(
                          engine, 0, 1.0f) == 1.0f,
    ...
    YouKnowEngine lowScale;
    lowScale.prepare(192000.0, blockSize, false);
    YouKnowTestAccess::setMode3Running(
        lowScale, 0, 100u, true, 1.0);
    YouKnowTestAccess::stageMode3Count(lowScale, 0, 10000u);
    YouKnowTestAccess::setDcoRampScales(lowScale, 0, 0.5f, 0.5f);
    const double lowScaleInterval =
        YouKnowTestAccess::internalIntervalSeconds(lowScale);
    YouKnowTestAccess::setDcoRampState(
        lowScale, 0, 2.5, 1.0 / lowScaleInterval);
    YouKnowTestAccess::primePulseTrack(lowScale, 0, -1.0f);
    YouKnowTestAccess::advanceDcoPitAndRampThreshold(
        lowScale, 0, DcoRange::Eight,
        6.0f, 6.0f, false, false, true);
    const auto lowScalePulse =
        YouKnowTestAccess::pulseTrackState(lowScale, 0);
    const auto expectedLowScalePulse =
        YouKnowTestAccess::expectedPulseStep(
            lowScale, -1.0f, 2.0f, 0.5f);
    expect(!YouKnowTestAccess::pitOutHigh(lowScale, 0)
               && YouKnowTestAccess::activeDcoDivider(lowScale, 0)
                      == 10000u
               && YouKnowTestAccess::dcoLogicStates(lowScale, 0)[0]
                      == 1.0f
               && YouKnowTestAccess::dcoPositiveBaseRail(lowScale, 0)
                      == 9.0
    ...
    YouKnowTestAccess::setDcoRampScales(scaleFlip, 0, 1.0f, 0.99f);
    YouKnowTestAccess::setDcoRampState(scaleFlip, 0, 0.0, 0.0);
    YouKnowTestAccess::primePulseTrack(scaleFlip, 0, -1.0f);
    YouKnowTestAccess::setDcoRampScales(scaleFlip, 0, 1.0f, 1.01f);
    YouKnowTestAccess::advanceDcoPitAndRampThreshold(
        scaleFlip, 0, DcoRange::Eight,
        6.0f, 6.0f, false, false, true);
    const auto expectedScaleFlip = YouKnowTestAccess::expectedPulseStep(
        scaleFlip, -1.0f, 2.0f, 1.0f);
    expect(YouKnowTestAccess::dcoLogicStates(scaleFlip, 0)[0] == 1.0f
               && YouKnowTestAccess::pulseTrackState(scaleFlip, 0).ring
    ...
    expect(YouKnowTestAccess::dcoRampState(rescaledSuffix, 0)[3] == 2.0
           ...
           "a mid-interval launch used the previous cycle's comparator scale: scale="
               + std::to_string(
                   YouKnowTestAccess::dcoRampState(rescaledSuffix, 0)[3])
               + " logic=" + std::to_string(
                   YouKnowTestAccess::dcoLogicStates(rescaledSuffix, 0)[0])
    ...
    for (const auto& transition : transitions)
    {
        ...
        YouKnowTestAccess::advanceDcoPitAndRamp(
            engine, 0, transition.previousDuty, transition.duty, true);

        const auto actual = YouKnowTestAccess::pulseTrackState(engine, 0);
        const auto expected = YouKnowTestAccess::expectedPulseStep(
            engine, transition.initialState,
            transition.finalState - transition.initialState,
            transition.samplesAgo);
        expect(YouKnowTestAccess::dcoLogicStates(engine, 0)[0]
    ...
    YouKnowEngine offExit;
    offExit.prepare(192000.0, blockSize, false);
    YouKnowTestAccess::setDcoRampState(offExit, 0, -1.0, 0.0);
    YouKnowTestAccess::primePulseTrack(offExit, 0, 1.0f);
    YouKnowTestAccess::advanceDcoPitAndRampThreshold(
        offExit, 0, DcoRange::Eight,
        -0.8f, 2.4f, true, false, true);
    const auto expectedOffExit = YouKnowTestAccess::expectedPulseStep(
        offExit, 1.0f, -2.0f, 0.75f);
    expect(YouKnowTestAccess::dcoLogicStates(offExit, 0)[0] == -1.0f
               && YouKnowTestAccess::pulseTrackState(offExit, 0).ring
...
{
    YouKnowEngine rendered;
    YouKnowEngine freewheeled;
    ...
    YouKnowTestAccess::setRangeClockTransition(rendered, 20.0, 20.25);
    YouKnowTestAccess::setRangeClockTransition(freewheeled, 20.0, 20.25);
    const auto armPhysicalPrestage = [](YouKnowEngine& engine) {
        const std::size_t ordinal = YouKnowTestAccess::pitchOrdinal(0);
        const double phaseStep =
            YouKnowTestAccess::scanPhasePerInternalSample(engine);
        YouKnowTestAccess::setConverterScheduler(
    ...
    {
        YouKnowTestAccess::advanceDcoPitAndRamp(rendered, 0, true);
        YouKnowTestAccess::advanceDcoPitAndRamp(freewheeled, 0, false);
        YouKnowTestAccess::advanceRangeClock(
            rendered, DcoRange::Eight);
        YouKnowTestAccess::advanceRangeClock(
            freewheeled, DcoRange::Eight);
        observedPendingReload = observedPendingReload
            || (YouKnowTestAccess::rangeClockTransitionPending(rendered)
                && YouKnowTestAccess::rangeClockReloadClocks(rendered)
                       > 0.0);
        transactionMatched = transactionMatched
            && YouKnowTestAccess::activeDcoDivider(rendered, 0)
                   == YouKnowTestAccess::activeDcoDivider(freewheeled, 0)
            && YouKnowTestAccess::pendingDcoDivider(rendered, 0)
                   == YouKnowTestAccess::pendingDcoDivider(freewheeled, 0)
            && YouKnowTestAccess::pitState(rendered, 0)
                   == YouKnowTestAccess::pitState(freewheeled, 0)
            && YouKnowTestAccess::pitOutHigh(rendered, 0)
                   == YouKnowTestAccess::pitOutHigh(freewheeled, 0)
            && YouKnowTestAccess::pitClocksToEvent(rendered, 0)
                   == YouKnowTestAccess::pitClocksToEvent(freewheeled, 0)
            && YouKnowTestAccess::pitWriteState(rendered, 0)
                   == YouKnowTestAccess::pitWriteState(freewheeled, 0)
            && YouKnowTestAccess::pitWriteDivider(rendered, 0)
                   == YouKnowTestAccess::pitWriteDivider(freewheeled, 0)
            && YouKnowTestAccess::cpuStatesToWrite(rendered, 0)
                   == YouKnowTestAccess::cpuStatesToWrite(freewheeled, 0)
            && YouKnowTestAccess::dcoPitchTransactionValid(rendered, 0)
                   == YouKnowTestAccess::dcoPitchTransactionValid(
                       freewheeled, 0)
            && YouKnowTestAccess::dcoPitchTransactionColdStart(rendered, 0)
                   == YouKnowTestAccess::dcoPitchTransactionColdStart(
                       freewheeled, 0)
            && YouKnowTestAccess::dcoPitchTransactionCvTarget(rendered, 0)
                   == YouKnowTestAccess::dcoPitchTransactionCvTarget(
                       freewheeled, 0)
            && YouKnowTestAccess::pendingDcoDividerValid(rendered, 0)
                   == YouKnowTestAccess::pendingDcoDividerValid(
                       freewheeled, 0)
            && YouKnowTestAccess::rangeClockFallingClocks(rendered)
                   == YouKnowTestAccess::rangeClockFallingClocks(
                       freewheeled)
            && YouKnowTestAccess::rangeClockReloadClocks(rendered)
                   == YouKnowTestAccess::rangeClockReloadClocks(
                       freewheeled)
            && YouKnowTestAccess::rangeClockTransitionPending(rendered)
                   == YouKnowTestAccess::rangeClockTransitionPending(
                       freewheeled);
    }
    ...
    expect(transactionMatched && observedPendingReload
               ...
               && YouKnowTestAccess::pitOutHigh(rendered, 0)
                      == YouKnowTestAccess::pitOutHigh(freewheeled, 0)
               && std::abs(YouKnowTestAccess::pitClocksToEvent(rendered, 0)
                           - YouKnowTestAccess::pitClocksToEvent(
                               freewheeled, 0)) < 1.0e-12
               && YouKnowTestAccess::dcoRampState(rendered, 0)
                      == YouKnowTestAccess::dcoRampState(freewheeled, 0)
               && YouKnowTestAccess::dcoLogicStates(rendered, 0)[1]
                      == YouKnowTestAccess::dcoLogicStates(freewheeled, 0)[1],
    ...
    YouKnowTestAccess::setRampCurrentError(exact, 0, 1.0f);
    YouKnowTestAccess::setRampCurrentError(fast, 0, 1.0f);
    ...
    const double launchRail =
        YouKnowTestAccess::dcoPositiveBaseRail(exact, 0);
    const double launchInterval =
        YouKnowTestAccess::internalIntervalSeconds(exact);
    for (auto* engine : { &exact, &fast })
    {
        YouKnowTestAccess::setMode3Running(
            *engine, 0, 100u, true, 1.0);
        YouKnowTestAccess::stageMode3Count(*engine, 0, 10000u);
        YouKnowTestAccess::setDcoRampState(
            *engine, 0, launchRail - 0.1, 0.3 / launchInterval);
        YouKnowTestAccess::setDcoLogicStates(*engine, 0, -1.0f, -1.0f);
        YouKnowTestAccess::primeSawTrack(*engine, 0);
        YouKnowTestAccess::primePulseTrack(*engine, 0, -1.0f);
    }
    ...
    {
        YouKnowTestAccess::renderRetiredVoice(
            exact, 0, exactParameters);
        YouKnowTestAccess::renderRetiredVoice(
            fast, 0, fastParameters);
        expect(YouKnowTestAccess::activeDcoDivider(exact, 0)
                   == YouKnowTestAccess::activeDcoDivider(fast, 0)
                   && YouKnowTestAccess::pitOutHigh(exact, 0)
                          == YouKnowTestAccess::pitOutHigh(fast, 0)
                   && YouKnowTestAccess::pitClocksToEvent(exact, 0)
                          == YouKnowTestAccess::pitClocksToEvent(fast, 0)
                   && YouKnowTestAccess::dcoRampState(exact, 0)
                          == YouKnowTestAccess::dcoRampState(fast, 0)
                   && YouKnowTestAccess::dcoLogicStates(exact, 0)[1]
                          == YouKnowTestAccess::dcoLogicStates(fast, 0)[1]
                   && YouKnowTestAccess::positiveRailHeld(exact, 0)
                          == YouKnowTestAccess::positiveRailHeld(fast, 0),
               "fast retired calibration diverged from Exact PIT/ramp state");
        sawHeldRail = sawHeldRail
            || YouKnowTestAccess::positiveRailHeld(exact, 0);
        sawNextRising =
            YouKnowTestAccess::dcoLogicStates(exact, 0)[1] == 1.0f;
    }
...
void testRescanGateOffReachesTheVoiceCpu()
{
    ...
    for (; elapsed < scanPeriod + 3
           && YouKnowTestAccess::assignmentPending(engine);
         ++elapsed)
        renderExact(engine, 1);
...
void testMidNoteSustainSnapsUpAndDecaysDown()
{
    ...
    const float lowLevel = YouKnowEngine::envelopeDacFraction(
        YouKnowEngine::storedControlAlignedWord(sustainLow));
    const float highLevel = YouKnowEngine::envelopeDacFraction(
        YouKnowEngine::storedControlAlignedWord(sustainHigh));
    ...
    const auto decayMultiplier =
        YouKnowEngine::envelopeDecayReleaseMultiplier(decayPosition);
    const auto releaseMultiplier =
        YouKnowEngine::envelopeDecayReleaseMultiplier(releasePosition);
    const auto sustainWord =
        YouKnowEngine::storedControlAlignedWord(sustainLow);
    std::uint16_t decayed = YouKnowEngine::storedControlAlignedWord(
        sustainHigh);
    ...
    {
        decayed = YouKnowEngine::envelopeDecayLevel(decayed, sustainWord,
                                                       decayMultiplier);
        released = YouKnowEngine::envelopeReleaseLevel(released,
                                                          releaseMultiplier);
        ...
        if (pass == fallPasses - 2)
            slowestDecayed = YouKnowEngine::envelopeDacFraction(decayed);
        if (pass == fallPasses + 2)
            fastestDecayed = YouKnowEngine::envelopeDacFraction(decayed);
    }
    const float releasedLevel = YouKnowEngine::envelopeDacFraction(released);
...
void testReassignedVoiceGlidesFromItsOwnPreviousPitch()
{
    ...
    engine.noteOn(36, 1.0f);
    renderExact(engine, scanPeriod * 400);
    expect(YouKnowTestAccess::lastVoiceMidi(engine, 0) == 36,
    ...
    renderExact(engine, scanPeriod * 2);
    expect(YouKnowTestAccess::lastVoiceMidi(engine, 0) == 84,
    ...
    const float departure = YouKnowTestAccess::currentMidi(engine, 0);
    ...
    expectNear(YouKnowTestAccess::currentMidi(engine, 0), 84.0, 1.0e-3,
...
void testIdleSnapshotPrimesEverySharedHold()
{
    ...
    const auto converterFraction = [](float value) {
        return static_cast<float>(
            YouKnowEngine::storedControlDacCode(value)) / 4064.0f;
    };
    const auto expectPrimed = [&](const YouKnowEngine& engine,
        ...
        const std::uint16_t pwmCode = snapshot.pulseEnabled
            ...
            : 0u;
        const double pwm = YouKnowEngine::pwmDacVolts(pwmCode);
        expect(YouKnowTestAccess::resonanceTarget(engine) == resonance
                   && YouKnowTestAccess::resonanceHeld(engine) == resonance,
               context + " did not prime resonance");
        expect(YouKnowTestAccess::sharedVcaTarget(engine) == level
                   && YouKnowTestAccess::sharedVca(engine) == level,
               context + " did not prime common VCA LEVEL");
        expect(YouKnowTestAccess::subTarget(engine) == sub
                   && YouKnowTestAccess::subHeld(engine) == sub,
               context + " did not prime SUB");
        expect(YouKnowTestAccess::noiseTarget(engine) == noise
                   && YouKnowTestAccess::noiseHeld(engine) == noise,
               context + " did not prime NOISE");
        expect(YouKnowTestAccess::pwmTarget(engine) == pwm
                   && YouKnowTestAccess::pwmFirstPoleHeld(engine) == pwm
                   && YouKnowTestAccess::pwmHeld(engine) == pwm,
               context + " did not prime both PWM hold poles");
    };

    YouKnowEngine hostOrder;
    ...
    expect(YouKnowTestAccess::controlScanPhase(hostOrder) == 1.0
               && YouKnowTestAccess::nextConverterWrite(hostOrder) == 0u,
           "an invalid startup process call advanced the converter");

    YouKnowEngine preloaded;
...
void testStartedIdleEditsUseTheOrderedConverterScan()
{
    ...
    const auto observe = [](const YouKnowEngine& value) {
        return std::array<double, 11> {
            YouKnowTestAccess::resonanceTarget(value),
            YouKnowTestAccess::resonanceHeld(value),
            YouKnowTestAccess::sharedVcaTarget(value),
            YouKnowTestAccess::sharedVca(value),
            YouKnowTestAccess::subTarget(value),
            YouKnowTestAccess::subHeld(value),
            YouKnowTestAccess::pwmTarget(value),
            YouKnowTestAccess::pwmFirstPoleHeld(value),
            YouKnowTestAccess::pwmHeld(value),
            YouKnowTestAccess::noiseTarget(value),
            YouKnowTestAccess::noiseHeld(value)
        };
    };
    ...
    const double phaseBefore = YouKnowTestAccess::controlScanPhase(engine);
    const std::size_t cursorBefore =
        YouKnowTestAccess::nextConverterWrite(engine);
    const auto latchBefore = YouKnowTestAccess::passiveHoldLatch(engine);
    auto edited = initial;
    ...
    expect(YouKnowTestAccess::controlScanPhase(engine) == phaseBefore
               && YouKnowTestAccess::nextConverterWrite(engine)
                      == cursorBefore
               && sameLatch(YouKnowTestAccess::passiveHoldLatch(engine),
                            latchBefore),
           "an idle edit after audio time changed the scanner state");

    engine.noteOn(60, 1.0f);
    expect(observe(engine) == before
               && YouKnowTestAccess::controlScanPhase(engine) == 0.0
               && YouKnowTestAccess::nextConverterWrite(engine) == 0u
               && !YouKnowTestAccess::passiveHoldLatch(engine).valid,
    ...
    YouKnowEngine contiguous = engine;
    YouKnowEngine partitioned = engine;
    ...
    expect(maximumDifference(contiguousLeft, partitionedLeft) == 0.0
               ...
               && YouKnowTestAccess::controlScanPhase(contiguous)
                      == YouKnowTestAccess::controlScanPhase(partitioned)
               && YouKnowTestAccess::nextConverterWrite(contiguous)
                      == YouKnowTestAccess::nextConverterWrite(partitioned),
    ...
    using Destination = YouKnowTestAccess::PassiveHoldDestination;
    const std::size_t ordinal = YouKnowTestAccess::passiveHoldOrdinal(
    ...
    const float newTarget = static_cast<float>(
        YouKnowEngine::storedControlDacCode(edited.vcaLevel)) / 4064.0f;
    float left = 0.0f;
    float right = 0.0f;
    engine.process(&left, &right, 1);
    const auto eventLatch = YouKnowTestAccess::passiveHoldLatch(engine);
    const auto expectedEventState = independentOnePoleEndpoint(
        oldState, oldTarget, true, 0.5, newTarget, 1.0 / sampleRate,
        YouKnowEngine::commonVcaHoldTimeConstantSeconds());
    expect(eventLatch.valid && eventLatch.ordinal == ordinal
               && eventLatch.target == newTarget
               && YouKnowTestAccess::sharedVcaTarget(engine) == oldTarget
               && YouKnowTestAccess::nextConverterWrite(engine) == ordinal,
           "the common-VCA physical event consumed its ordinal too early");
    expectNear(YouKnowTestAccess::sharedVca(engine),
    ...
    const auto expectedCommittedState = independentOnePoleEndpoint(
        ...
        1.0 / sampleRate,
        YouKnowEngine::commonVcaHoldTimeConstantSeconds());
    expect(YouKnowTestAccess::sharedVcaTarget(engine) == newTarget
               && YouKnowTestAccess::nextConverterWrite(engine)
                      == ordinal + 1u,
    ...
    YouKnowEngine panic = partitioned;
    panic.allNotesOff();
    const auto beforePanicEdit = observe(panic);
    const double panicPhase = YouKnowTestAccess::controlScanPhase(panic);
    const std::size_t panicCursor =
        YouKnowTestAccess::nextConverterWrite(panic);
    ...
    expect(observe(panic) == beforePanicEdit
               && YouKnowTestAccess::controlScanPhase(panic) == panicPhase
               && YouKnowTestAccess::nextConverterWrite(panic)
                      == panicCursor,
    ...
    const float resetLevel = static_cast<float>(
        YouKnowEngine::storedControlDacCode(afterReset.vcaLevel)) / 4064.0f;
    expect(YouKnowTestAccess::sharedVcaTarget(panic) == resetLevel
               && YouKnowTestAccess::sharedVca(panic) == resetLevel
               && YouKnowTestAccess::controlScanPhase(panic) == 1.0
               && YouKnowTestAccess::nextConverterWrite(panic) == 0u,
...
void testPhysicalFiltersKeepRunningBehindClosedVcas()
{
    ...
    for (int slot = 0; slot < YouKnowEngine::hardwareVoices; ++slot)
    {
        const auto state = YouKnowTestAccess::filterState(engine, slot);
    ...
    const auto extension = YouKnowTestAccess::filterState(
        engine, YouKnowEngine::hardwareVoices);
...
void testPhysicalCardStateSurvivesVoiceAssignments()
{
    ...
    std::array<std::uint32_t, YouKnowEngine::hardwareVoices> seeds {};
    for (int slot = 0; slot < YouKnowEngine::hardwareVoices; ++slot)
        seeds[static_cast<std::size_t>(slot)] =
            YouKnowTestAccess::microscopicNoiseState(engine, slot);
    ...
    engine.reset();
    for (int slot = 0; slot < YouKnowEngine::hardwareVoices; ++slot)
        expect(YouKnowTestAccess::microscopicNoiseState(engine, slot)
                   == seeds[static_cast<std::size_t>(slot)],
    ...
    YouKnowTestAccess::setFilterState(engine, slot, filterMarker);
    YouKnowTestAccess::setMicroscopicNoiseState(engine, slot, noiseMarker);

    YouKnowTestAccess::initialiseVoice(engine, slot, 60);
    expect(YouKnowTestAccess::filterState(engine, slot) == filterMarker,
           "note activation reset the continuously powered filter state");
    expect(YouKnowTestAccess::microscopicNoiseState(engine, slot)
               == noiseMarker,
           "note activation reseeded the card's microscopic excitation");

    YouKnowTestAccess::silenceVoice(engine, slot);
    expect(YouKnowTestAccess::filterState(engine, slot) == filterMarker,
    ...
    YouKnowTestAccess::initialiseVoice(engine, slot, 67);
    expect(YouKnowTestAccess::filterState(engine, slot) == filterMarker,
    ...
    constexpr int extensionSlot = YouKnowEngine::hardwareVoices;
    const auto extensionSeed = YouKnowTestAccess::microscopicNoiseState(
        engine, extensionSlot);
    YouKnowTestAccess::setFilterState(
        engine, extensionSlot, filterMarker);
    YouKnowTestAccess::setMode3Running(
        engine, extensionSlot, 123u, false, 17.5);
    YouKnowTestAccess::setMicroscopicNoiseState(
        engine, extensionSlot, noiseMarker);
    YouKnowTestAccess::primePulseDutyHistory(engine, extensionSlot, 0.9f);
    YouKnowTestAccess::initialiseVoice(engine, extensionSlot, 72);
    YouKnowTestAccess::silenceVoice(engine, extensionSlot);
    const auto clearedExtension = YouKnowTestAccess::filterState(
        engine, extensionSlot);
    ...
    expect(YouKnowTestAccess::pitState(engine, extensionSlot)
                   == YouKnowTestAccess::stoppedPitState()
               && YouKnowTestAccess::pitOutHigh(engine, extensionSlot)
               && YouKnowTestAccess::pitClocksToEvent(
                      engine, extensionSlot) == 0.0
               && YouKnowTestAccess::dcoRampValue(
                      engine, extensionSlot) == -1.0,
           "an idle extension slot retained frozen PIT/ramp state");
    expect(!YouKnowTestAccess::pulseThresholdHistoryIsPrimed(
               engine, extensionSlot),
           "an idle extension slot retained a stale PWM comparator timeline");
    expect(YouKnowTestAccess::microscopicNoiseState(engine, extensionSlot)
               == extensionSeed,
           "an idle extension slot retained a frozen noise timeline");
    expect(YouKnowTestAccess::currentMidi(engine, extensionSlot) == 72.0f,
           "clearing virtual analogue state discarded extension portamento memory");
    expectNear(YouKnowTestAccess::moduleCouplingState(
                   engine, extensionSlot),
               0.0, 1.0e-12,
    ...
    YouKnowTestAccess::initialiseVoice(engine, extensionSlot, 74);
    expectNear(YouKnowTestAccess::moduleCouplingState(
                   engine, extensionSlot),
               6.0, 1.0e-6,
...
void testConverterSchedulerPreservesFractionalScanPeriod()
{
    ...
    double previousPhase = YouKnowTestAccess::controlScanPhase(engine);
    for (int sample = 0; sample < 24000 && boundaries.size() < 101; ++sample)
    {
        float left = 0.0f;
        float right = 0.0f;
        engine.process(&left, &right, 1);
        const double phase = YouKnowTestAccess::controlScanPhase(engine);
...
void testFractionalVcfWritesPeekWithoutConsumingTheScheduler()
{
    ...
    engine.setParameters(eventParameters);
    YouKnowTestAccess::setResonanceHold(engine, 0.13f, 0.74f);
    const std::size_t ordinal = YouKnowTestAccess::vcfOrdinal(slot);
    const double delta =
        YouKnowTestAccess::scanPhasePerInternalSample(engine);
    const double eventPhase =
        YouKnowTestAccess::converterEventPhase(engine, ordinal);
    ...
    const float heldBefore = YouKnowTestAccess::cutoffHeld(engine, slot);
    ...
    expect(YouKnowTestAccess::cutoffTarget(engine, slot) == targetBefore,
    ...
    const float expectedEndpoint =
        YouKnowTestAccess::exactVcfHoldEndpoint(
            heldBefore, targetBefore, latch.eventPosition, latch.target,
            1.0 / sampleRate);
    expectNear(YouKnowTestAccess::cutoffHeld(engine, slot),
    ...
    expect(YouKnowTestAccess::resonanceHeld(engine) == resonanceHeldBefore,
           "a VCF event interval moved the held RESO CV");
    const auto cutoffNodes =
        YouKnowTestAccess::cutoffVcfHoldNodes(engine, slot);
    const auto resonanceNodes =
        YouKnowTestAccess::resonanceVcfHoldNodes(engine);
    expect(YouKnowTestAccess::exactVcfControlInterval(engine, slot),
    ...
    expect(YouKnowTestAccess::passiveHoldLatch(engine).valid,
           "live automation discarded a pending fractional VCF payload");
    engine.process(&left, &right, 1);
    const auto followingLatch = YouKnowTestAccess::passiveHoldLatch(engine);
    ...
    expect(YouKnowTestAccess::nextConverterWrite(engine) == ordinal + 1u,
           "the normal poll did not consume exactly one converter write");
    expect(YouKnowTestAccess::cutoffTarget(engine, slot) == latch.target,
...
void testFractionalResonancePeekCrossesThePassBoundary()
{
    ...
    eventParameters.resonance = 0.68f;
    engine.setParameters(eventParameters);
    YouKnowTestAccess::setCutoffHold(engine, 0, 1400.0f, 11000.0f);
    const double delta =
        YouKnowTestAccess::scanPhasePerInternalSample(engine);
    constexpr double requestedPosition = 0.27;
    YouKnowTestAccess::setConverterScheduler(
        ...
        YouKnowTestAccess::converterWriteCount());
    const float heldBefore = YouKnowTestAccess::resonanceHeld(engine);
    const float targetBefore = YouKnowTestAccess::resonanceTarget(engine);
    const float cutoffHeldBefore =
        YouKnowTestAccess::cutoffHeld(engine, 0);
    ...
    const auto resonanceNodes =
        YouKnowTestAccess::resonanceVcfHoldNodes(engine);
    const auto cutoffNodes =
        YouKnowTestAccess::cutoffVcfHoldNodes(engine, 0);
    expect(YouKnowTestAccess::exactVcfControlInterval(engine, 0),
    ...
    expect(!followingLatch.valid || followingLatch.ordinal != latch.ordinal,
           ...
           "peeking the following passive hold");
    expect(YouKnowTestAccess::nextConverterWrite(engine) == 1u,
           "pass-wrap polling consumed more than resonance ordinal zero");
    expect(YouKnowTestAccess::resonanceTarget(engine) == latch.target,
...
void testFractionalPassiveHoldsUseTheirPhysicalWriteTime()
{
    ...
    for (const auto& fixture : fixtures)
    {
        ...
        {
            case Destination::CommonVca:
                YouKnowTestAccess::setSharedVcaHold(
                    engine, fixture.initialFirst, fixture.oldTarget);
                break;
            case Destination::Sub:
                YouKnowTestAccess::setSubHold(
                    engine, fixture.initialFirst, fixture.oldTarget);
                break;
            case Destination::Pwm:
                YouKnowTestAccess::setPwmHold(
                    engine, fixture.initialFirst, fixture.initialSecond,
                    fixture.oldTarget);
                break;
            case Destination::VoiceVca:
                YouKnowTestAccess::setVoiceVcaHold(
        ...
        else
        {
            const double timeConstant = fixture.destination
                ...
                ? YouKnowTestAccess::voiceVcaHoldSeconds()
                : (fixture.destination == Destination::CommonVca
                       ? YouKnowEngine::commonVcaHoldTimeConstantSeconds()
                       : YouKnowTestAccess::subSlewSeconds());
            ...
            {
                actual = YouKnowTestAccess::sharedVca(engine);
                officialTarget = YouKnowTestAccess::sharedVcaTarget(engine);
            }
            else if (fixture.destination == Destination::Sub)
            {
                actual = YouKnowTestAccess::subHeld(engine);
                officialTarget = YouKnowTestAccess::subTarget(engine);
            }
            else
            {
                actual = YouKnowTestAccess::vcaControl(engine, 0);
                officialTarget =
                    YouKnowTestAccess::vcaControlTarget(engine, 0);
            }
        ...
        const auto followingLatch =
            YouKnowTestAccess::passiveHoldLatch(engine);
        ...
        switch (fixture.destination)
        {
            case Destination::CommonVca:
                committedTarget =
                    YouKnowTestAccess::sharedVcaTarget(engine);
                break;
            case Destination::Sub:
                committedTarget = YouKnowTestAccess::subTarget(engine);
                break;
            case Destination::Pwm:
                committedTarget = YouKnowTestAccess::pwmTarget(engine);
                break;
            case Destination::VoiceVca:
                committedTarget =
                    YouKnowTestAccess::vcaControlTarget(engine, 0);
...
void testPassiveHoldEventEndpointsRespectIntervalOwnership()
{
    ...
    for (const auto destination : { Destination::CommonVca, Destination::Pwm })
    {
        for (const double eventPosition : { 0.0, 1.0 })
        {
            ...
            const std::size_t ordinal =
                YouKnowTestAccess::passiveHoldOrdinal(destination);
            const double delta =
                YouKnowTestAccess::scanPhasePerInternalSample(engine);
            const double eventPhase =
                YouKnowTestAccess::converterEventPhase(engine, ordinal);
            ...
            const float newTarget = destination == Destination::CommonVca
                ? (eventPosition == 0.0
                       ...
                       : latch.target)
                : (eventPosition == 0.0
                       ? YouKnowTestAccess::pwmTarget(engine)
                       : latch.target);
            const std::string context =
...
void testFractionalPwmHoldIsHostBlockPartitionInvariant()
{
    ...
    contiguous.setParameters(parameters);
    YouKnowTestAccess::setPwmHold(contiguous, 1.2, 2.4, 3.3f);
    const std::size_t ordinal = YouKnowTestAccess::passiveHoldOrdinal(
        Destination::Pwm);
    const double delta =
        YouKnowTestAccess::scanPhasePerInternalSample(contiguous);
    YouKnowTestAccess::setConverterScheduler(
        contiguous,
        YouKnowTestAccess::converterEventPhase(contiguous, ordinal)
            - 0.55 * delta,
        ordinal);
    YouKnowEngine split = contiguous;
    ...
    expect(YouKnowTestAccess::pwmFirstPoleHeld(split)
               == YouKnowTestAccess::pwmFirstPoleHeld(contiguous)
               && YouKnowTestAccess::pwmHeld(split)
                      == YouKnowTestAccess::pwmHeld(contiguous)
               && YouKnowTestAccess::pwmTarget(split)
                      == YouKnowTestAccess::pwmTarget(contiguous),
           "fractional PWM circuit state changed with host block partitioning");
    expect(YouKnowTestAccess::controlScanPhase(split)
               == YouKnowTestAccess::controlScanPhase(contiguous)
               && YouKnowTestAccess::nextConverterWrite(split)
                      == YouKnowTestAccess::nextConverterWrite(contiguous),
           "fractional PWM scheduling changed with host block partitioning");
    const auto contiguousLatch =
        YouKnowTestAccess::passiveHoldLatch(contiguous);
    const auto splitLatch = YouKnowTestAccess::passiveHoldLatch(split);
...
void testPitchAndNoiseRemainSampleGridWrites()
{
    ...
    YouKnowTestAccess::setMode3Running(
        pitch, 0, YouKnowTestAccess::activeDcoDivider(pitch, 0),
        true, 1000.0);
    YouKnowTestAccess::setDcoResetPending(pitch, 0, false);
    const double oldPeriod = YouKnowTestAccess::dcoPeriodSamples(pitch, 0);
    pitchParameters.masterTuneCents = 47.0f;
    pitch.setParameters(pitchParameters);
    const std::size_t pitchOrdinal = YouKnowTestAccess::pitchOrdinal(0);
    const double pitchDelta =
        YouKnowTestAccess::scanPhasePerInternalSample(pitch);
    YouKnowTestAccess::setConverterScheduler(
        pitch,
        YouKnowTestAccess::converterEventPhase(pitch, pitchOrdinal)
            - eventPosition * pitchDelta,
        pitchOrdinal);
    float left = 0.0f;
    float right = 0.0f;
    pitch.process(&left, &right, 1);
    expect(!YouKnowTestAccess::passiveHoldLatch(pitch).valid,
           "a DCO pitch write entered the passive-hold latch");
    expect(YouKnowTestAccess::nextConverterWrite(pitch) == pitchOrdinal
               && YouKnowTestAccess::dcoPeriodSamples(pitch, 0)
                      == oldPeriod,
           "a DCO pitch write moved from its established sample-grid poll");
    pitch.process(&left, &right, 1);
    expect(YouKnowTestAccess::nextConverterWrite(pitch)
               == pitchOrdinal + 1u
               && YouKnowTestAccess::dcoPeriodSamples(pitch, 0)
                      == oldPeriod
               && YouKnowTestAccess::pendingDcoDividerValid(pitch, 0)
               && YouKnowTestAccess::pendingDcoDivider(pitch, 0)
                      != YouKnowTestAccess::activeDcoDivider(pitch, 0),
           "the ordinary DCO pitch poll did not stage exactly one count write");
    for (int sample = 0; sample < 4
         && YouKnowTestAccess::dcoPeriodSamples(pitch, 0) == oldPeriod;
         ++sample)
        pitch.process(&left, &right, 1);
    expect(YouKnowTestAccess::dcoPeriodSamples(pitch, 0) != oldPeriod
               && !YouKnowTestAccess::pendingDcoDividerValid(pitch, 0),
    ...
    noise.setParameters(noiseParameters);
    YouKnowTestAccess::setNoiseHold(
        noise, oldNoiseTarget, oldNoiseTarget);
    const std::size_t noiseOrdinal = YouKnowTestAccess::noiseOrdinal();
    const double noiseDelta =
        YouKnowTestAccess::scanPhasePerInternalSample(noise);
    YouKnowTestAccess::setConverterScheduler(
        noise,
        YouKnowTestAccess::converterEventPhase(noise, noiseOrdinal)
            - eventPosition * noiseDelta,
        noiseOrdinal);
    noise.process(&left, &right, 1);
    expect(!YouKnowTestAccess::passiveHoldLatch(noise).valid,
           "the unmeasured noise hold entered the passive-hold latch");
    expect(YouKnowTestAccess::nextConverterWrite(noise) == noiseOrdinal
               && YouKnowTestAccess::noiseTarget(noise) == oldNoiseTarget
               && YouKnowTestAccess::noiseHeld(noise) == oldNoiseTarget,
           "the noise destination moved from its established sample-grid poll");
    noise.process(&left, &right, 1);
    expect(YouKnowTestAccess::nextConverterWrite(noise)
               == noiseOrdinal + 1u
               && YouKnowTestAccess::noiseTarget(noise)
                      != oldNoiseTarget,
...
void testDoublePassiveHoldStatesDoNotStallAtHighRate()
{
    ...
    YouKnowTestAccess::setSharedVcaHold(engine, 1.0, nearOne);
    YouKnowTestAccess::setSubHold(engine, 1.0, nearOne);
    YouKnowTestAccess::setVoiceVcaHold(engine, 0, 1.0, nearOne);
    YouKnowTestAccess::setPwmHold(engine, 6.0, 6.0, nearSix);
    ...
    YouKnowTestAccess::setConverterScheduler(engine, 0.5, 12u);

    float left = 0.0f;
    float right = 0.0f;
    engine.process(&left, &right, 1);
    expect(YouKnowTestAccess::sharedVca(engine) < 1.0
               && YouKnowTestAccess::sharedVca(engine) > nearOne,
           "the double common-VCA state stalled at 768 kHz");
    expect(YouKnowTestAccess::subHeld(engine) < 1.0
               && YouKnowTestAccess::subHeld(engine) > nearOne,
           "the double SUB state stalled at 768 kHz");
    expect(YouKnowTestAccess::vcaControl(engine, 0) < 1.0
               && YouKnowTestAccess::vcaControl(engine, 0) > nearOne,
           "the double voice-VCA state stalled at 768 kHz");
    expect(YouKnowTestAccess::pwmFirstPoleHeld(engine) < 6.0
               && YouKnowTestAccess::pwmFirstPoleHeld(engine) > nearSix,
           "the double PWM first node stalled at 768 kHz");
    expect(YouKnowTestAccess::pwmHeld(engine) < 6.0
               && YouKnowTestAccess::pwmHeld(engine) > nearSix,
...
void testPulseOffPinsComparatorWithoutResettingTheDco()
{
    ...
    parameters.enableVcfStageOffsets = false;
    engine.setParameters(parameters);
    expectNear(YouKnowTestAccess::pwmTarget(engine), -0.8, 1.0e-6,
               "Pulse Off did not write the documented -0.8 V shared control");
    expectNear(YouKnowTestAccess::moduleCouplingState(engine, 0),
               6.0, 1.0e-6,
    ...
    const bool outBefore = YouKnowTestAccess::pitOutHigh(engine, 0);
    const double clocksBefore =
        YouKnowTestAccess::pitClocksToEvent(engine, 0);
    const double rampBefore = YouKnowTestAccess::dcoRampValue(engine, 0);
    renderExact(engine, 1);
    expect(YouKnowTestAccess::pitOutHigh(engine, 0) != outBefore
               || YouKnowTestAccess::pitClocksToEvent(engine, 0)
                      != clocksBefore
               || YouKnowTestAccess::dcoRampValue(engine, 0) != rampBefore,
    ...
    {
        renderExact(engine, 1);
        if (YouKnowTestAccess::pwmHeld(engine) > 0.0f
            && YouKnowTestAccess::pulseDuty(engine, 1) >= 1.0f)
        {
            sawPerCardOffsetWindow = true;
            expect(YouKnowTestAccess::pulseMixEnabled(engine, 1, true),
...
void testPulseOffCouplingSettlesWhileFastCardsFreewheel()
{
    ...
    const double fastState = YouKnowTestAccess::moduleCouplingState(fast, 0);
    const double exactState = YouKnowTestAccess::moduleCouplingState(exact, 0);
    ...
    expectNear(YouKnowTestAccess::pulseDuty(fast, 0),
               1.0, 1.0e-6,
    ...
    fast.noteOn(0, 1.0f);
    expectNear(YouKnowTestAccess::moduleCouplingState(fast, 0),
               fastState, 1.0e-12,
    ...
    YouKnowEngine fastHigh;
    YouKnowEngine exactHigh;
    ...
    const double highDifference = std::abs(
        YouKnowTestAccess::moduleCouplingState(fastHigh, 0)
        - YouKnowTestAccess::moduleCouplingState(exactHigh, 0));
...
void testMovingPwmComparatorDoesNotMissThresholdCrossings()
{
    ...
    for (const bool highQuality : { false, true })
    {
        ...
        int edgeCount = 0;
        float previousState = YouKnowTestAccess::pulseLogicState(engine, 0);
        for (int sample = 0; sample < static_cast<int>(sampleRate * 3.0); ++sample)
        {
            ...
            const float state = YouKnowTestAccess::pulseLogicState(engine, 0);
            const float threshold =
                YouKnowTestAccess::pulseThresholdVolts(engine, 0);
            const double rampVolts =
                YouKnowTestAccess::dcoRampVolts(engine, 0);
        ...
        const double hostPeriod = YouKnowTestAccess::dcoPeriodSamples(engine, 0)
                                / (highQuality ? 4.0 : 1.0);
...
void testConverterTimingProfileSelectionAppliesAtReset()
{
    ...
    YouKnowEngine engine;
    engine.prepare(48000.0, blockSize, false);
    expect(YouKnowTestAccess::converterEventPhase(engine, 9) == 9.0 / 23.0,
    ...
    engine.reset();
    expectNear(YouKnowTestAccess::converterEventPhase(engine, 9),
               905.0 / 1886.5, 0.0,
...
void testAgedUnitExtensionStaysInertByDefault()
{
    ...
    for (int card = 0; card < 6; ++card)
        expect(YouKnowTestAccess::cardAgingCutoffCounts(engine, card)
                   == 0.0f,
               "an aged cutoff shift leaked into the freshly calibrated state");
    expect(YouKnowTestAccess::agedNoiseGain(engine) == 1.0f,
    ...
    {
        const float counts =
            YouKnowTestAccess::cardAgingCutoffCounts(engine, card);
        const float weight = YouKnowTestAccess::cardAgingWeight(engine, card);
    ...
    expectNear(YouKnowTestAccess::agedNoiseGain(engine),
...
void testPortamentoStateUsesEightEightGrid()
{
    ...
    const float state = YouKnowTestAccess::currentMidi(engine, 0);
...
void testUnisonUsesEveryVoiceWithoutDetuning()
{
    ...
    const bool firstOut = YouKnowTestAccess::pitOutHigh(engine, 0);
    const bool sixthOut = YouKnowTestAccess::pitOutHigh(engine, 5);
    const double countdownDistance = std::abs(
        YouKnowTestAccess::pitClocksToEvent(engine, 0)
        - YouKnowTestAccess::pitClocksToEvent(engine, 5));
...
void testUnisonDoesNotResurrectPolyphonicTails()
{
    ...
    engine.noteOff(65);
    render(engine, static_cast<int>(sampleRate
        * (YouKnowEngine::envelopeReleaseSeconds(parameters.release) + 0.25f)));
...
void testMainVolumeLoadedLinearPotLaw()
{
    ...
    const double expectedHalf =
        YouKnowEngine::outputCouplingHighGain(0.5f)
        / YouKnowEngine::outputCouplingHighGain(1.0f);
    const double expectedQuarter =
        YouKnowEngine::outputCouplingHighGain(0.25f)
        / YouKnowEngine::outputCouplingHighGain(1.0f);
    ...
    for (std::size_t rateIndex = 0; rateIndex < rates.size(); ++rateIndex)
    {
        ...
        float right = 0.0f;
        engine.process(&left, &right, 1);
        expect(YouKnowTestAccess::glidedVolume(engine) == 0.0f,
               "the initial VOLUME position was not primed without a glide");

        parameters.volume = 1.0f;
        engine.setParameters(parameters);
        int samples = 0;
        while (YouKnowTestAccess::glidedVolume(engine) < oneTau
        ...
        stateAtTenMilliseconds[rateIndex] =
            YouKnowTestAccess::glidedVolume(engine);
...
void testFixedOutputBoundaryCorpus()
{
    ...
    constexpr std::array baselines {
        ...
        // on the owner's decision (see YouKnowEngine::subMixVolts). Only
        ...
        // (YouKnowEngine::outputLevelPolicyDb). Every floating figure below
    ...
    const bool printAudit = std::getenv("YOUKNOW_AUDIT_OUTPUT") != nullptr;
    ...
    const float lowerReference =
        YouKnowEngine::compatibilityOutputReferenceRmsVolts * 0.5f;
    const float higherReference =
        YouKnowEngine::compatibilityOutputReferenceRmsVolts * 2.0f;
    const double lowerGain = YouKnowEngine::outputReferenceGain(lowerReference);
    const double higherGain = YouKnowEngine::outputReferenceGain(higherReference);
...
void testNotesWaitForTheSharedConverterScan()
{
    ...
    expect(YouKnowTestAccess::controlScanPhase(engine) > 0.2
               && YouKnowTestAccess::nextConverterWrite(engine) > 0u,
...
void testRetriggerDoesNotTouchVcaHoldBeforeConverterScan()
{
    ...
    engine.noteOff(60);
    const double heldBefore = YouKnowTestAccess::vcaControl(engine, 0);
    const double phaseBefore = YouKnowTestAccess::controlScanPhase(engine);
    expect(std::isfinite(heldBefore) && heldBefore > 0.0f,
           "the retrigger fixture has no live analogue VCA hold");

    engine.noteOn(60, 1.0f);
    expect(YouKnowTestAccess::vcaControl(engine, 0) == heldBefore,
...
void testNoteOnPlayingLatencyAcrossConverterPhases()
{
    ...
    const float outputOnsetThreshold =
        1.0e-4f * YouKnowEngine::outputBoundaryGain();
    ...
    for (int quality = 0; quality < 2; ++quality)
    {
        ...
        for (int measuredSlot = 0; measuredSlot < 6; ++measuredSlot)
        {
            ...
            const double cycleStartPhase =
                YouKnowTestAccess::controlScanPhase(timeline);
            auto probe = std::make_unique<YouKnowEngine>();
            for (int eventPhase = 0; eventPhase < phaseCycleSamples; ++eventPhase)
            {
                *probe = timeline;
                probe->noteOn(48 + measuredSlot, 1.0f);
                expect(YouKnowTestAccess::voiceActive(*probe, measuredSlot)
                           && YouKnowTestAccess::lastVoiceMidi(
                                  *probe, measuredSlot) == 60,
                       "the latency probe did not reach its requested card");
                expect(YouKnowTestAccess::dcoResetPending(
                           *probe, measuredSlot),
                       "the latency probe did not arm its pitch write");
                expect(YouKnowTestAccess::vcaControlTarget(
                           *probe, measuredSlot) == 0.0f
                           && YouKnowTestAccess::vcaControl(
                                  *probe, measuredSlot) == 0.0f
                           && YouKnowTestAccess::voiceVcaGain(
                                  *probe, measuredSlot) == 0.0f,
                       "Note On bypassed the scheduled voice-VCA path");

                Observation measured;
                const float pitchCvBefore =
                    YouKnowTestAccess::dcoCvTarget(
                        *probe, measuredSlot);
                const std::size_t voiceVcaOrdinal =
                    YouKnowTestAccess::passiveHoldOrdinal(
                ...
                for (int sample = 0; sample < maximumObservedLatency; ++sample)
                {
                    ...
                    if (measured.pitPrestage < 0
                        && !YouKnowTestAccess::dcoResetPending(
                            *probe, measuredSlot))
                    ...
                    const auto passiveLatch =
                        YouKnowTestAccess::passiveHoldLatch(*probe);
                    ...
                    if (measured.vcaTargetWrite < 0
                        && YouKnowTestAccess::vcaControlTarget(
            ...
            expectNear(YouKnowTestAccess::controlScanPhase(timeline),
    ...
    for (int quality = 0; quality < 2; ++quality)
    {
        ...
        if (std::getenv("YOUKNOW_AUDIT_LATENCY") != nullptr)
        {
...
void testSilentVoiceDoesNotInventUnmeasuredVcaFeedthrough()
{
    ...
    for (const float controlOffset : { -1.0f, 0.0f, 1.0f })
    {
        YouKnowEngine engine;
        engine.prepare(48000.0, blockSize, true);
        const float output = YouKnowTestAccess::renderSilentVoiceAtControlOffset(
...
void testCommonVcaHoldUsesJackBoardC7TimeConstant()
{
    ...
    const int hostSamples = static_cast<int>(std::lround(
        YouKnowEngine::commonVcaHoldTimeConstantSeconds() * sampleRate));
    ...
    for (const bool oversampling : { false, true })
    {
        ...
        parameters.vcaLevel = 1.0f;
        engine.setParameters(parameters);
        YouKnowTestAccess::setSharedVca(engine, 0.0f);

        renderExact(engine, hostSamples);
        const double elapsed = hostSamples / sampleRate;
        const double expected = 1.0 - std::exp(
            -elapsed / YouKnowEngine::commonVcaHoldTimeConstantSeconds());
        expectNear(YouKnowTestAccess::sharedVca(engine), expected, 2.0e-5,
                   oversampling
...
void testPwmHoldCrossesItsTwoSmoothingPoles()
{
    ...
    constexpr double sampleRate = 48000.0;
    const double tauFirst = YouKnowTestAccess::pwmFirstPoleSeconds();
    const double tauSecond = YouKnowTestAccess::pwmSecondPoleSeconds();
    ...
    for (const bool oversampling : { false, true })
    {
        ...
        engine.setParameters(parameters);
        const double target = YouKnowTestAccess::pwmTarget(engine);
        expect(target > 0.0, "manual PWM fixture has no positive target");
        YouKnowTestAccess::setPwmHeld(engine, 0.0f);
            ...
            * (1.0
               - (tauFirst * std::exp(-elapsed / tauFirst)
                  - tauSecond * std::exp(-elapsed / tauSecond))
                     / (tauFirst - tauSecond));
        expectNear(YouKnowTestAccess::pwmHeld(engine), expected,
                   5.0e-3 * target,
                   oversampling
...
void testExactPwmHoldEndpointNonFiniteGuards()
{
    ...
    // elapsed time. Its only production call site (YouKnowEngine::process)
...
void testSubHoldUsesItsR11C1TimeConstant()
{
    ...
    constexpr double sampleRate = 48000.0;
    const double oneTau = 1.0 - std::exp(-1.0);
    const double tau = YouKnowTestAccess::subSlewSeconds();
    ...
    for (const bool oversampling : { false, true })
    {
        ...
        parameters.subLevel = 1.0f;
        engine.setParameters(parameters);
        const double target = YouKnowTestAccess::subTarget(engine);
        expect(target > 0.0, "sub fixture has no positive target");
        YouKnowTestAccess::setSubHeld(engine, 0.0f);

        renderExact(engine, hostSamples);
        const double elapsed = hostSamples / sampleRate;
        const double expected = target * (1.0 - std::exp(-elapsed / tau));
        expectNear(YouKnowTestAccess::subHeld(engine), expected, 2.0e-5,
...
void testStartupPrimesC56WithTheSubMean()
{
    ...
    const double subTarget = YouKnowTestAccess::subTarget(engine);
    ...
    expectNear(YouKnowTestAccess::moduleCouplingState(engine, 0),
               6.0 + YouKnowTestAccess::subMixVolts() * subTarget, 1.0e-6,
...
std::vector<float> probeVcaInput(YouKnowEngine& engine, int hostSamples)
{
...
void testSubLevelJumpCouplesThroughC56AndC59()
{
    ...
    const double stepVolts = YouKnowTestAccess::subMixVolts() * 0.40;
...
void testPwmUsesRawLfoOutsideDelayEnvelope()
{
    ...
    {
        YouKnowEngine immediate;
        YouKnowEngine delayed;
        ...
        const float centreTarget = YouKnowEngine::pwmDacVolts(
            YouKnowEngine::pwmDacCode(
                1.0f, PwmSource::Lfo, 0u, true));
        const float positiveTarget = YouKnowEngine::pwmDacVolts(
            YouKnowEngine::pwmDacCode(
                1.0f, PwmSource::Lfo, 0x1fffu, true));
        const float negativeTarget = YouKnowEngine::pwmDacVolts(
            YouKnowEngine::pwmDacCode(
                1.0f, PwmSource::Lfo, 0x1fffu, false));
        expectNear(YouKnowTestAccess::pwmHeld(immediate),
                   centreTarget, 1.0e-6,
                   "raw-LFO PWM startup did not begin at its centre code");
        expect(YouKnowTestAccess::pwmHeld(immediate)
                   == YouKnowTestAccess::pwmHeld(delayed),
        ...
        for (int sample = 0; sample < windowSamples; ++sample)
        {
            ...
            pwmStatesMatch = pwmStatesMatch
                && YouKnowTestAccess::lfoValue(immediate)
                       == YouKnowTestAccess::lfoValue(delayed)
                && YouKnowTestAccess::pwmTarget(immediate)
                       == YouKnowTestAccess::pwmTarget(delayed)
                && YouKnowTestAccess::pwmFirstPoleHeld(immediate)
                       == YouKnowTestAccess::pwmFirstPoleHeld(delayed)
                && YouKnowTestAccess::pwmHeld(immediate)
                       == YouKnowTestAccess::pwmHeld(delayed)
                && YouKnowTestAccess::pulseDuty(immediate, 0)
                       == YouKnowTestAccess::pulseDuty(delayed, 0);
            delayLevelsDiverged = delayLevelsDiverged
                || YouKnowTestAccess::lfoDelayLevel(immediate)
                       != YouKnowTestAccess::lfoDelayLevel(delayed);
            cutoffTargetsDiverged = cutoffTargetsDiverged
                || YouKnowTestAccess::cutoffTarget(immediate, 0)
                       != YouKnowTestAccess::cutoffTarget(delayed, 0);
            minimumTarget = std::min(
                minimumTarget, YouKnowTestAccess::pwmTarget(immediate));
            maximumTarget = std::max(
                maximumTarget, YouKnowTestAccess::pwmTarget(immediate));
            positivePeakMapped = positivePeakMapped
                || (YouKnowTestAccess::lfoValue(immediate) == 1.0f
                    && YouKnowTestAccess::pwmTarget(immediate)
                           == positiveTarget);
...
void testQualityChangeWaitsForTheOutputPathToEmpty()
{
    ...
    const auto supportBuilds =
        YouKnowTestAccess::chorusSupportBuildCount(engine);
    ...
    expect(YouKnowTestAccess::chorusSupportBuildCount(engine) == supportBuilds,
...
void testQualityChangeFadesRateDependentOutputPath()
{
    ...
    const float gainBeforeControlChange =
        YouKnowTestAccess::rateTransitionGain(controlChanged);
    parameters.chorus = ChorusMode::Off;
    parameters.chorusNoise = 0.0f;
    controlChanged.setParameters(parameters);
    ...
    expect(YouKnowTestAccess::rateTransitionGain(controlChanged)
...
void testQualityChangePreservesOutputCouplingTail()
{
    ...
    expect(peakOf(unchanged.left, compareFrom)
               > 0.002 * YouKnowEngine::outputBoundaryGain(),
...
void testQualityChangePreservesFreeRunningClocks()
{
    ...
    const double deepPeriod =
        YouKnowTestAccess::dcoPeriodSamples(switched, 0);
    expect(!switched.setOversamplingEnabled(false),
    ...
    expect(YouKnowTestAccess::activeDcoDivider(switched, 0)
               == YouKnowTestAccess::activeDcoDivider(reference, 0)
               && YouKnowTestAccess::pitOutHigh(switched, 0)
                      == YouKnowTestAccess::pitOutHigh(reference, 0)
               && std::abs(YouKnowTestAccess::pitClocksToEvent(switched, 0)
                           - YouKnowTestAccess::pitClocksToEvent(reference, 0))
                      < 1.0e-7,
           "an HQ rebuild changed the physical PIT count, OUT or half-cycle");
    expect(circularError(YouKnowTestAccess::chorusPhase(switched),
                         YouKnowTestAccess::chorusPhase(reference))
               < 2.0e-8,
           "an HQ rebuild restarted the free-running chorus oscillator");
    expect(circularError(YouKnowTestAccess::controlScanPhase(switched),
                         YouKnowTestAccess::controlScanPhase(reference))
               < 2.0e-8,
           "an HQ rebuild restarted the firmware converter scan");
    expectNear(YouKnowTestAccess::dcoPeriodSamples(switched, 0),
...
void testModuleInputCouplingKeepsMixerDcOutOfTheVoiceVca()
{
    ...
    constexpr double sampleRate = 48000.0;
    expectNear(YouKnowEngine::moduleCouplingCornerHz(), 0.482288, 1.0e-5,
    ...
    const auto measure = [&](float pwmPanel)
    {
        ...
        for (int offset = 0; offset < total; offset += probeBlock)
        {
            ...
            if (seconds >= 1.5 && seconds <= 3.0)
            {
                ...
                weightedSum +=
                    hann * YouKnowTestAccess::vcaInputVolts(engine, 0);
                weight += hann;
                sustained = sustained
                         && YouKnowTestAccess::voiceActive(engine, 0);
            }
        }
        expect(sustained,
...
void testFinalOutputCouplingRemovesManualPwmDc()
{
    ...
    expect(std::abs(YouKnowTestAccess::outputCouplingState(silent)) > 1.0e-3,
           "VOLUME zero prevented the upstream C17 state from charging");

    const double chargedAtZero =
        YouKnowTestAccess::outputCouplingState(silent);
    silentParameters.volume = 1.0f;
    silent.setParameters(silentParameters);
    expect(YouKnowTestAccess::outputCouplingState(silent) == chargedAtZero,
...
void testThermalWarmupClockRunsToCompletionAtEveryRate()
{
    ...
    for (std::size_t index = 0; index < configurationCount; ++index)
    {
        ...
        const double internalRate =
            YouKnowTestAccess::internalRate(engine);
    ...
    // driven, compared bit for bit -- both the timer and the exponential.
    {
        ...
        const long long internalSamples = static_cast<long long>(
            std::llround(hostSamples
                         * (YouKnowTestAccess::internalRate(driven)
                            / 48000.0)));
        YouKnowTestAccess::advanceThermalWarmup(driven, internalSamples);

        expect(YouKnowTestAccess::thermalWarmupSeconds(rendered)
                   == YouKnowTestAccess::thermalWarmupSeconds(driven),
               "the driven warm-up clock is not the one the render loop "
               "advances (rendered "
                   + std::to_string(
                         YouKnowTestAccess::thermalWarmupSeconds(rendered))
                   + " s, driven "
                   + std::to_string(
                         YouKnowTestAccess::thermalWarmupSeconds(driven))
                   + " s)");
        expect(YouKnowTestAccess::thermalWarmupFraction(rendered)
                   == YouKnowTestAccess::thermalWarmupFraction(driven),
...
void testSampleRateAndOversamplingConsistency()
{
    ...
    const auto expectAlignedCentre = [&](const YouKnowEngine& current) {
        const double effective = YouKnowTestAccess::numericalLatencyCentre(
            current.getOversamplingFactor())
            + YouKnowTestAccess::latencyPadSamples(current);
...
void testResonanceDoesNotMoveTheRenderedCorner()
{
    ...
    const auto corner = [&](float cutoffPanel, float resonance) {
        ...
        return static_cast<double>(
                   YouKnowTestAccess::filterOmegaStep(engine, 0))
             * internalRate / (2.0 * pi);
    };
...
void testSelfOscillationMatchesTheServiceTrim()
{
    ...
    const auto oscillate = [&](int note, float keyFollow,
        ...
        YouKnowTestAccess::setFilterState(engine, 0, { 1.0f, 1.0f, 1.0f, 1.0f });
        ...
        {
            engine.process(&left, &right, 1);
            const float volts = YouKnowTestAccess::filterOutputVolts(engine, 0);
    ...
    const auto expectSeeded = [&](const char* when) {
        for (int slot = 0; slot < YouKnowEngine::maxVoices; ++slot)
        {
            for (int stage = 0; stage < 4; ++stage)
            {
                ...
                expectNear(YouKnowTestAccess::stageOffset(engine, slot, stage),
                           YouKnowTestAccess::cardStageOffset(engine, slot, stage)
...
void testSanitiseFallbackDefaultsMatchDeclaredDefaults()
{
    ...
    for (const auto& field : zeroToOneFields)
    {
        for (const float poison : { nan, inf, -inf })
        {
            ...
            const auto sanitised = YouKnowTestAccess::sanitise(hostile);
        ...
        expect(YouKnowTestAccess::sanitise(low).*field.member == 0.0f,
               std::string(field.name)
                   + " finite below-range input did not clamp to 0");

        EngineParameters high;
        high.*field.member = 5.0f;
        expect(YouKnowTestAccess::sanitise(high).*field.member == 1.0f,
               std::string(field.name)
    ...
    for (const float poison : { nan, inf, -inf })
    {
        ...
        expect(YouKnowTestAccess::sanitise(hostile).calibration
    ...
    expect(YouKnowTestAccess::sanitise(calibrationHigh).calibration
    ...
    expect(YouKnowTestAccess::sanitise(calibrationLow).calibration
               == 0.0f,
    ...
    for (const float poison : { nan, inf, -inf })
    {
        ...
        expect(YouKnowTestAccess::sanitise(hostile).masterTuneCents
    ...
    expect(YouKnowTestAccess::sanitise(tuneHigh).masterTuneCents
               == 50.0f,
           "masterTuneCents above range did not clamp to +50");
    EngineParameters tuneLow;
    tuneLow.masterTuneCents = -900.0f;
    expect(YouKnowTestAccess::sanitise(tuneLow).masterTuneCents
               == -50.0f,
    ...
    expect(YouKnowTestAccess::sanitise(invalidTanhMode).vcfTanhMode
...
void testExactOnePoleHoldEndpointNonFiniteGuards()
{
    ...
    {
        const double endpoint = YouKnowTestAccess::exactOnePoleHoldEndpoint(
...
void testCompactKeyLegendsFitAtEverySupportedSize()
{
    ...
    for (int step = 0; step <= steps; ++step)
    {
        ...
        for (const auto& key : keys)
        {
            ...
            const float available =
                panel::compactLegendWidth(width, height, true, scale);
    ...
    // Larger editors keep the legend proportional to the key.
    const float maximumPointSize = panel::compactLegendPointSizeMax
                                 * maximumScale / panel::defaultEditorScale;
    expect(std::abs(panel::compactLegendPointSize(1000.0f, maximumScale)
                    - maximumPointSize) <= 1.0e-5f,
           "the compact legend ceiling did not follow the enlarged panel");
...
void testSettledRk4PairMatchesScalar()
{
    ...
    {
        const auto compared =
            YouKnowTestAccess::compareSettledRk4Pair(fixture);
    ...
    {
        const auto compared =
            YouKnowTestAccess::compareSettledMersonPair(fixture);
    ...
    const auto quad = YouKnowTestAccess::compareSettledMersonQuad();
    ...
    {
        const auto rejected =
            YouKnowTestAccess::compareRejectedOtaPair(mixedTableaux);
    ...
    {
        const auto rejected =
            YouKnowTestAccess::compareRejectedMersonPair(mixedTableaux);
        const std::string context = mixedTableaux
...
int main()
{
    ...
    if (std::getenv("YOUKNOW_PIT_TESTS_ONLY") != nullptr)
    {
    ...
    if (std::getenv("YOUKNOW_STARTUP_HOLD_TESTS_ONLY") != nullptr)
    {
    ...
    if (std::getenv("YOUKNOW_LATENCY_TEST_ONLY") != nullptr)
    {
    ...
    if (std::getenv("YOUKNOW_PASSIVE_HOLD_TESTS_ONLY") != nullptr)
    {
        testFractionalVcfWritesPeekWithoutConsumingTheScheduler();
    ...
    if (failures != 0)
    {
        std::cerr << failures << " YouKnow engine check(s) failed.\n";
        return EXIT_FAILURE;
    }
    std::cout << "All YouKnow engine checks passed.\n";
    return EXIT_SUCCESS;
}
