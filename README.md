...
- One 12-bit converter scans 23 sample-and-hold destinations — 18 per-card,
  ...
  ideal acquisition at the write. Actual acquisition remains unmeasured:
...
- Mains ripple remains uncalibrated. The conditional ≈0.011-cent cutoff
  estimate is not an installed upper bound or an audibility guarantee.
...
| --- | --- | ---: | ---: | ---: |
| `01-chorus-pad.wav` | Saw and sub through the mode-I chorus: the classic pad, hiss and all | 21.9 s | −11.7 dBFS | +8.7 dB |
| `02-pwm-strings.wav` | Pulse-width-modulated strings in the faster mode-II chorus | 15.3 s | −12.2 dBFS | +9.2 dB |
| `03-sixteen-foot-bass.wav` | A 16' bassline: the exponential envelope segments doing the punch | 13.8 s | −25.1 dBFS | +22.1 dB |
| `04-filter-brass.wav` | Resonant filter-envelope stabs, ending on a full bender push | 10.3 s | −22.7 dBFS | +19.7 dB |
...
| `08-delayed-vibrato.wav` | The modulator's two-stage delay fading vibrato onto a held chord | 9.6 s | −19.2 dBFS | +16.2 dB |
| `09-high-pass-ladder.wav` | One bright chord through all four high-pass switch positions | 10.6 s | −11.3 dBFS | +8.3 dB |
...
| ---: | --- | --- | ---: | ---: | ---: |
| 1 | `A48` Synth Bass I (unison) | bass | 1 | +0.0 dB | −3.9 dBFS |
| 2 | `A67` Shaker | shaker | 1 | +9.0 dB | −19.6 dBFS |
| 3 | `A64` Snare Drum (unison) | snare | 1 | +6.0 dB | −8.5 dBFS |
| 4 | `A54` Funky II | stabs | 4 | +0.0 dB | −8.3 dBFS |
| 5 | `A28` Elect. Piano II | electric piano | 1 | +0.0 dB | −18.8 dBFS |
| 6 | `A11` Brass Set 1 | brass | 3 | +0.0 dB | −11.3 dBFS |
| 7 | `B11` Strings | strings | 3 | +0.0 dB | −13.6 dBFS |
| 8 | `A17` Choir | choir | 3 | +0.0 dB | −11.1 dBFS |
| 9 | `B13` Chorus Vibes | chorus vibes | 4 | +0.0 dB | −26.3 dBFS |
| 10 | `A53` Lead III | lead | 1 | +0.0 dB | −24.5 dBFS |
...
- **No calibrated mains-ripple or shared-rail transient model.** A conditional
  estimate using the M5230L's typical 68 dB rejection at 120 Hz gives about
  0.30 mVpp at the card rail and ~0.011 cents through the existing voiced
  rail-to-cutoff transfer. This is not an installed upper bound or an
  audibility guarantee: minimum rejection, load transients, ground impedance
  and coupling to other nodes remain unmeasured. The current audio-energy
  load proxy is not a physical rail-current model. Master-reference supply
  sensitivity needs a separate common-clock measurement.
