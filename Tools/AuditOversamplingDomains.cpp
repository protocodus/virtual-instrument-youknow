...
using youknow::VcfFastEarlyMode;
using youknow::VcfTanhMode;
using youknow::VcfSolverMode;
using youknow::oversampling_audit::DomainWorkCounters;
...
void renderBlocks(YouKnowEngine& engine, int blocks,
...
std::uint64_t renderFingerprint(const PreparedSnapshot& snapshot, int blocks)
{
    ...
    YouKnowEngine engine = snapshot.engine;
...
[[maybe_unused]] int printTimingReport()
{
    ...
    for (const auto& scenario : scenarios)
    {
        ...
        constexpr std::size_t rungs = YouKnowEngine::oversampleFactors.size();
...
CounterRun runCounterCase(const CounterCase& testCase)
{
    ...
    CounterRun result;
    {
        youknow::oversampling_audit::ScopedDomainWorkCounterSink sink(
            result.counters);
    ...
    for (std::size_t index = 0; index < counterCases.size(); ++index)
    {
        ...
        if (!selfTestOnly)
        {
            ...
            for (const auto& descriptor :
                 youknow::oversampling_audit::counterDescriptors)
...
int main(int argc, char** argv)
{
    try
    {
        ...
        {
#if defined(YOUKNOW_WORK_AUDIT)
            std::cerr << "--tanh-benchmark requires YouKnowOversamplingAudit\n";
            return 2;
#else
...
#else
            std::cerr << "--self-test requires YouKnowOversamplingWorkAudit\n";
            return 2;
#endif
        }
        if (argc != 1)
