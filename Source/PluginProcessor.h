...
class YouKnowAudioProcessorEditor;

class YouKnowAudioProcessor final : public juce::AudioProcessor,
...
public:
    ...
    ~YouKnowAudioProcessor() override;
    ...
    youknow::sysex::Patch programPatch (int index) const;
    ...
    void applyPatch (const youknow::sysex::Patch& patch);
...
private:
    ...
    // Consume each changed automation axis once. An unchanged host parameter
    // must not overwrite a later MIDI wheel event, or the other axis.
    std::atomic<int> pendingPitchBend { -1 };
    std::atomic<int> pendingModulation { -1 };
    ...
    void queuePitchBend (float value) noexcept;
    void queueModulation (float value) noexcept;
    ...
    {
        explicit ScopedParameterWrite (YouKnowAudioProcessor&) noexcept;
    ...
    static youknow::sysex::Patch patchFromEngineParameters (
        const youknow::EngineParameters&) noexcept;
    static void applyPatchToEngineParameters (
        youknow::EngineParameters&, const youknow::sysex::Patch&) noexcept;
    ...
    void applyPatchValues (const youknow::sysex::Patch& patch);
    ...
    std::atomic<int> displayVoiceLimit { youknow::YouKnowEngine::hardwareVoices };
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (YouKnowAudioProcessor)
};
