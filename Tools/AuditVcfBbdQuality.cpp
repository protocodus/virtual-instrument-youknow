...
struct YouKnowTestAccess
{
        ...
        int factor) noexcept
    {
        constexpr double half =
            (YouKnowEngine::halfbandTaps - 1) / 2.0;
        if (factor == 2)
            return half / 2.0;
        if (factor == 4)
...
double vcfHotDriveAmplitude()
{
    return vcfHotAmplitude
         * YouKnowTestAccess::inputCompensation(vcfHotFeedback);
}
...
VcfMetrics auditVcf(double hostRate, int factor, const VcfOracle& oracle)
{
    ...
    const double delay =
        YouKnowTestAccess::shippingDecimatorBoundaryDelayHostFrames(factor);
    ...
    {
        YouKnowTestAccess::Cascade production;
        production.reset();
        ...
        for (std::size_t index = 0u; index < internalFrames; ++index)
        {
            ...
            actualInternal[index] = production.process(
                ...
                YouKnowTestAccess::otaHeadroom(), false, 0.0f);
        }
    ...
    const std::size_t ringInternalFrames = vcfRingHostFrames
        * static_cast<std::size_t>(factor);
    YouKnowTestAccess::Cascade ringProduction;
    ...
    const auto ringBoundary = YouKnowTestAccess::decimate(
        ringInternal, factor);
    ...
    const int inputHarmonics = static_cast<int>(
        auditPassbandHz / vcfHotFundamentalHz);
    YouKnowTestAccess::Cascade hotCascade;
    hotCascade.reset();
    ...
    const auto hotBoundary = YouKnowTestAccess::decimate(
        hotInternal, factor);
                    ...
                    bool retainRawInternalBits = false)
{
    ...
    for (const auto& reference : oracle.cases)
    {
        ...
        for (std::size_t index = 0u; index < internalFrames; ++index)
        {
            ...
            actualInternal[index] = YouKnowTestAccess::processBbdLine(
...
struct ShippingBbdResult
{
    ...
    BbdAuditWindow window;
    YouKnowTestAccess::ProcessingRate engineRate;
    ...
    const ShippingBbdDefinition& definition)
{
    ...
    result.engineRate = YouKnowTestAccess::shippingProcessingRate(
...
void selfTest(const AuditResult& audit)
{
    ...
    for (std::size_t index = 0; index < results.size(); ++index)
    {
        ...
        const double declaredDelay =
            YouKnowTestAccess::shippingDecimatorBoundaryDelayHostFrames(
                cell.factor);
...
int main(int argc, char** argv)
{
    try
    {
        ...
        {
            std::cout
                << "Usage: YouKnowVcfBbdQualityAudit [--self-test|"
