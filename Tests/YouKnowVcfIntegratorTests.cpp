...
struct YouKnowTestAccess
{
    ...
    using HoldInterval = YouKnowEngine::VcfHoldInterval;
    ...
    static Tableau planTableau(youknow::VcfSolverMode mode, double poleStep,
    ...
    {
        return youknow::EngineParameters::calibrationCeiling;
    }

    static double thermalCutoffSpread(int cardIndex,
                                      double calibration) noexcept
    {
        return 1.0 + YouKnowEngine::vcfCutoffTempcoPerCelsius
            * calibration
            * (YouKnowEngine::chassisGradientCelsius(cardIndex)
               - YouKnowEngine::chassisGradientMeanCelsius());
    }
        ...
        int cardIndex) noexcept
    {
        ...
        for (std::size_t stage = 0; stage < result.size(); ++stage)
            result[stage] = 1.0f
                + YouKnowEngine::hashBipolar(
                    seed + 20u + static_cast<std::uint32_t>(stage))
                    * YouKnowEngine::vcfStageCapacitorTolerance
        ...
        for (std::size_t stage = 0; stage < result.size(); ++stage)
            result[stage] = 0.0015f
                * YouKnowEngine::hashBipolar(
                ...
                / YouKnowEngine::stageAttenuation
                * static_cast<float>(calibrationCeiling());
        return result;
    }
...
float processSelected(
    ...
    youknow::VcfTanhMode tanhMode,
    youknow::VcfFastEarlyMode earlyMode,
    youknow::VcfSolverMode solverMode =
        youknow::VcfSolverMode::MersonHalfSteps) noexcept
{
    if (tanhMode == youknow::VcfTanhMode::ZonedHermite
...
{
    expect(youknow::EngineParameters {}.vcfTanhMode
               == youknow::VcfTanhMode::Exact,
           "EngineParameters no longer defaults the VCF tanh mode to exact");
    expect(youknow::EngineParameters {}.vcfFastEarlyMode
    ...
    for (int sample = 0; sample < 2048; ++sample)
    {
        ...
        exact.process(input, 1.7f, 4.5f,
                      ...
                      nullptr, youknow::VcfTanhMode::Exact);
...
void testCubicEarlyTanhKernelAndDispatch()
{
    ...
    const auto render = [](youknow::VcfTanhMode tanhMode,
                           youknow::VcfFastEarlyMode earlyMode,
    ...
    const auto exactHermite = render(
        youknow::VcfTanhMode::Exact,
        youknow::VcfFastEarlyMode::Hermite, true, 1.0f);
    const auto exactCubic = render(
        youknow::VcfTanhMode::Exact,
        youknow::VcfFastEarlyMode::Cubic, true, 1.0f);
    ...
    expect(render(youknow::VcfTanhMode::ZonedHermite,
                  youknow::VcfFastEarlyMode::Hermite, true, 0.0f)
               == render(youknow::VcfTanhMode::ZonedHermite,
                         youknow::VcfFastEarlyMode::Cubic, true, 0.0f)
               && render(youknow::VcfTanhMode::ZonedHermite,
                         youknow::VcfFastEarlyMode::Hermite, false, 1.0f)
               == render(youknow::VcfTanhMode::ZonedHermite,
                         youknow::VcfFastEarlyMode::Cubic, false, 1.0f),
           "Fast Early selector changed a bypassed Early path");

    const auto renderMovingBypass = [](youknow::VcfFastEarlyMode earlyMode,
                                       bool enableEarly,
                                       float calibration) {
        Cascade cascade;
        cascade.reset();
        ...
        for (int sample = 0; sample < 2048; ++sample)
        {
            ...
            output.push_back(processSelected(
                ...
                youknow::VcfTanhMode::ZonedHermite, earlyMode));
        }
...
const char* solverName(youknow::VcfSolverMode mode)
{
    switch (mode)
    {
        case youknow::VcfSolverMode::Rk4HalfSteps: return "RK4 x2";
        case youknow::VcfSolverMode::Rk4Single:    return "RK4 x1";
        case youknow::VcfSolverMode::MersonHalfSteps: break;
    }
    return "Merson x2";
}
...
void testSolverLadderReadsOnlyItsOwnControlNodes()
{
    ...
    const auto render = [&](const Access::ControlTrajectory& trajectory,
        ...
        Access::setState(cascade, { 0.31, -0.19, 0.11, -0.07 });
        cascade.process(input, omega, feedback, headroom, true, 0.70f,
                        &trajectory, youknow::VcfTanhMode::Exact, solver);
        return Access::state(cascade);
    };
        ...
        for (const auto& rung : rungs)
        {
            ...
            for (std::size_t index = 0; index < length; ++index)
            {
                ...
                cascade.process(
                    ...
                    youknow::VcfTanhMode::Exact, rung.mode);
                const double actual = Access::state(cascade)[3];
                const double expected = reference.process(
                    input, omega, feedback, headroom, true, calibration);
                if (index >= 512)
    ...
    const auto limitCycle = [](VcfSolverMode solver) {
        ...
        for (int sample = 0; sample < 200000; ++sample)
            cascade.process(0.0f, omega, feedback, headroom, true, 1.0f,
                            nullptr, youknow::VcfTanhMode::Exact, solver);
                             ...
                             VcfSolverMode::Rk4Single })
    {
        ...
        for (std::size_t index = 0; index < length; ++index)
        {
            ...
            const double output = cascade.process(
                ...
                youknow::VcfTanhMode::Exact, solver);
...
void testMersonRungIsTheUnchangedDefault()
{
    ...
    for (int sample = 0; sample < 4096; ++sample)
    {
        ...
        const float first = historical.process(
            ...
            1.0f, trajectoryFor, youknow::VcfTanhMode::Exact);
...
void testHighGStabilityAndRecovery(youknow::VcfTanhMode tanhMode,
                                   const char* modeName,
                                   youknow::VcfFastEarlyMode earlyMode =
                                       youknow::VcfFastEarlyMode::Hermite,
                                   youknow::VcfSolverMode solverMode =
                                       youknow::VcfSolverMode::
                                           MersonHalfSteps)
{
...
int main()
{
    ...
    testHighGStabilityAndRecovery(
        youknow::VcfTanhMode::Exact, "exact");
    testHighGStabilityAndRecovery(
        youknow::VcfTanhMode::ZonedHermite, "zoned Hermite");
    testHighGStabilityAndRecovery(
        youknow::VcfTanhMode::ZonedHermite, "zoned Hermite/cubic Early",
        youknow::VcfFastEarlyMode::Cubic);
    ...
    testHighGStabilityAndRecovery(
        youknow::VcfTanhMode::Exact, "exact/RK4 x2",
        youknow::VcfFastEarlyMode::Hermite,
        youknow::VcfSolverMode::Rk4HalfSteps);
    testHighGStabilityAndRecovery(
        youknow::VcfTanhMode::Exact, "exact/RK4 x1",
        youknow::VcfFastEarlyMode::Hermite,
        youknow::VcfSolverMode::Rk4Single);
    testHighGStabilityAndRecovery(
        youknow::VcfTanhMode::ZonedHermite, "zoned Hermite/RK4 x1",
        youknow::VcfFastEarlyMode::Hermite,
        youknow::VcfSolverMode::Rk4Single);
    testHighGStabilityAndRecovery(
        youknow::VcfTanhMode::ZonedHermite,
