...
float migrateLegacyVolumePosition (float legacyPosition) noexcept
{
    ...
    const float target = YouKnowEngine::outputCouplingHighGain()
                       * legacy * legacy;
    if (target <= 0.0f)
        return 0.0f;
    if (target >= YouKnowEngine::outputCouplingHighGain (1.0f))
        return 1.0f;
    ...
    for (int iteration = 0; iteration < 24; ++iteration)
    {
        ...
        if (YouKnowEngine::outputCouplingHighGain (middle) < target)
            lower = middle;
        else
            upper = middle;
    }
...
juce::AudioParameterFloatAttributes attackAttributes()
{
    return juce::AudioParameterFloatAttributes()
        ...
        {
            return secondsText (YouKnowEngine::envelopeAttackSeconds (value), 0);
        })
        .withValueFromStringFunction ([] (const juce::String& text)
        {
            return YouKnowEngine::panelPositionForAttack (secondsFromText (text));
        });
}
...
juce::AudioParameterFloatAttributes decayAttributes()
{
    return juce::AudioParameterFloatAttributes()
        .withStringFromValueFunction ([] (float value, int)
        {
            return secondsText (YouKnowEngine::envelopeDecaySeconds (value), 0);
        })
        .withValueFromStringFunction ([] (const juce::String& text)
        {
            return YouKnowEngine::panelPositionForDecay (secondsFromText (text));
        });
}
...
juce::AudioParameterFloatAttributes releaseAttributes()
{
    return juce::AudioParameterFloatAttributes()
        .withStringFromValueFunction ([] (float value, int)
        {
            return secondsText (YouKnowEngine::envelopeReleaseSeconds (value), 0);
        })
        .withValueFromStringFunction ([] (const juce::String& text)
        {
            return YouKnowEngine::panelPositionForRelease (secondsFromText (text));
        });
}
...
juce::AudioParameterFloatAttributes lfoRateAttributes()
{
    return juce::AudioParameterFloatAttributes()
        .withStringFromValueFunction ([] (float value, int)
        {
            const auto hz = YouKnowEngine::lfoRateHz (value);
            return juce::String (hz, hz < 10.0f ? 2 : 1) + " Hz";
        })
        .withValueFromStringFunction ([] (const juce::String& text)
        {
            return YouKnowEngine::panelPositionForLfoRate (hertzFromText (text));
        });
}
...
juce::AudioParameterFloatAttributes lfoDelayAttributes()
{
    return juce::AudioParameterFloatAttributes()
        .withStringFromValueFunction ([] (float value, int)
        {
            return secondsText (YouKnowEngine::lfoDelaySeconds (value), 0);
        })
        .withValueFromStringFunction ([] (const juce::String& text)
        {
            return YouKnowEngine::panelPositionForLfoDelay (secondsFromText (text));
        });
}
...
juce::AudioParameterFloatAttributes cutoffAttributes()
{
    return juce::AudioParameterFloatAttributes()
        .withStringFromValueFunction ([] (float value, int)
        {
            const auto hz = YouKnowEngine::vcfCutoffHz (
                YouKnowEngine::vcfPanelCounts (value));
            if (hz >= 1000.0f)
...
juce::AudioParameterFloatAttributes portamentoAttributes()
{
    return juce::AudioParameterFloatAttributes()
        .withStringFromValueFunction ([] (float value, int)
        {
            ...
            const auto seconds = YouKnowEngine::portamentoSeconds (
                YouKnowEngine::portamentoTravelAdcFraction (value));
            if (seconds <= 0.0f)
        ...
        .withValueFromStringFunction ([] (const juce::String& text)
        {
            if (text.trim().equalsIgnoreCase ("off"))
                return 0.0f;
            return YouKnowEngine::portamentoTravelForAdcFraction (
                YouKnowEngine::panelPositionForPortamento (
...
void YouKnowAudioProcessor::migrateSplitModeParameters (juce::ValueTree& state)
{
    using namespace youknow::parameters;
...
YouKnowAudioProcessor::createParameterLayout()
{
    using namespace youknow::parameters;
    ...
    layout.add (std::make_unique<juce::AudioParameterInt> (
        ...
        youknow::YouKnowEngine::maxVoices,
    ...
    layout.add (std::make_unique<juce::AudioParameterFloat> (
        juce::ParameterID { pitchBend, 8 }, "Pitch Bend",
        juce::NormalisableRange<float> { -1.0f, 1.0f, 0.0f }, 0.0f,
        percentAttributes()));
    layout.add (std::make_unique<juce::AudioParameterFloat> (
        juce::ParameterID { modulation, 8 }, "Modulation",
...
YouKnowAudioProcessor::YouKnowAudioProcessor()
...
float YouKnowAudioProcessor::valueOf (const char* parameterId) const noexcept
{
...
int YouKnowAudioProcessor::choiceOf (const char* parameterId, int maximum) const noexcept
{
    return juce::jlimit (0, maximum, juce::roundToInt (valueOf (parameterId)));
}

float YouKnowAudioProcessor::valueOf (ParameterIndex parameter) const noexcept
{
    const auto index = static_cast<std::size_t> (parameter);
    jassert (index < parameterPointers.size());
...
int YouKnowAudioProcessor::choiceOf (ParameterIndex parameter,
                                        int maximum) const noexcept
{
    return juce::jlimit (0, maximum, juce::roundToInt (valueOf (parameter)));
}

int YouKnowAudioProcessor::getQualityChoice() const noexcept
{
    return choiceOf (ParameterIndex::quality, qualityChoiceCount - 1);
}

void YouKnowAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
...
void YouKnowAudioProcessor::clearDisplayTelemetry() noexcept
{
...
void YouKnowAudioProcessor::releaseResources()
{
    engineReady.store (false, std::memory_order_release);
    engine.allNotesOff();
    engine.reset();
    pendingPitchBend.store (-1, std::memory_order_release);
    pendingModulation.store (-1, std::memory_order_release);
    discardUiMidiEvents();
...
void YouKnowAudioProcessor::reset()
{
...
bool YouKnowAudioProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
void YouKnowAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer,
...
void YouKnowAudioProcessor::processBlockBypassed (
...
{
    const float bend = std::isfinite (value)
                     ? std::clamp (value, -1.0f, 1.0f) : 0.0f;
    pendingPitchBend.store (juce::jlimit (
        0, 16383, juce::roundToInt (8192.0f + bend * 8192.0f)),
        std::memory_order_release);
}

void YouKnowAudioProcessor::queueModulation (float value) noexcept
{
    const float mod = std::isfinite (value)
                    ? std::clamp (value, 0.0f, 1.0f) : 0.0f;
    pendingModulation.store (juce::roundToInt (mod * 127.0f),
                             std::memory_order_release);
}
...
{
    const int pitchWheel = pendingPitchBend.exchange (-1, std::memory_order_acq_rel);
    const int modWheel = pendingModulation.exchange (-1, std::memory_order_acq_rel);
    if (pitchWheel >= 0)
...
thread_local YouKnowAudioProcessor::ScopedParameterWrite*
    YouKnowAudioProcessor::activeParameterWrite = nullptr;

YouKnowAudioProcessor::ScopedParameterWrite::ScopedParameterWrite (
    YouKnowAudioProcessor& owner) noexcept
...
const YouKnowAudioProcessor::ScopedParameterWrite*
YouKnowAudioProcessor::activeWriteForThisThread() const noexcept
{
    ...
    juce::ValueTree state, int program, juce::MemoryBlock& destinationData)
{
    ...
    if (storedParameterValue (state, youknow::parameters::poly1, 0.0f) <= 0.5f
        && storedParameterValue (state, youknow::parameters::poly2, 0.0f)
               <= 0.5f)
        setStoredParameterValue (state, youknow::parameters::poly1, 1.0f);
...
void YouKnowAudioProcessor::randomizeParameters (float amount)
{
...
void YouKnowAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
    ...
    if (restoredStateSchema < 2
        && containsParameterState (state, youknow::parameters::volume))
        setStoredParameterValue (
            state, youknow::parameters::volume,
            migrateLegacyVolumePosition (storedParameterValue (
                state, youknow::parameters::volume, 0.8f)));
    if (restoredStateSchema < calibrationDefaultSchemaVersion
        && ! containsParameterState (state, youknow::parameters::calibration))
        setStoredParameterValue (state, youknow::parameters::calibration,
    ...
    // Schema 2 and earlier numbered the former 32 original YouKnow sounds.
...
{
    return new YouKnowAudioProcessorEditor (*this);
}

juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter()
{
    return new YouKnowAudioProcessor();
}
...
int YouKnowAudioProcessor::hostProgramIndexForMidiProgram (
    int midiProgram) noexcept
{
    ...
    return midiProgram >= 0 && midiProgram < youknow::presets::presetCount
        ? midiProgram + 1
        : -1;
}
...
void YouKnowAudioProcessor::setCurrentProgram (int index)
{
...
bool YouKnowAudioProcessor::currentProgramIsEdited() const
{
    ...
    std::array<std::uint8_t, youknow::sysex::toneByteCount> panel {}, program {};
    youknow::sysex::toneBytesFromPatch (panelPatch, panel.data());
    youknow::sysex::toneBytesFromPatch (selectedPatch, program.data());
    ...
    return differs (valueOf (volume), expected.volume)
        ...
        || panelPoly1 != youknow::poly1Engaged (expected.keyMode)
        || panelPoly2 != youknow::poly2Engaged (expected.keyMode)
...
const juce::String YouKnowAudioProcessor::getProgramName (int index)
{
    if (index < 0 || index >= getNumPrograms())
        return {};
    if (index == 0)
...
void YouKnowAudioProcessor::applyPatchValues (
    const youknow::sysex::Patch& patch)
{
    using namespace youknow::parameters;
...
void YouKnowAudioProcessor::applyProgramValues (
    const youknow::sysex::Patch& patch,
    const youknow::presets::Preset::Controls& controls)
{
    using namespace youknow::parameters;
    ...
    const auto chorus = patch.chorus == youknow::ChorusMode::OneTwo
                          ? youknow::ChorusMode::Two : patch.chorus;
    ...
    set (poly1, youknow::poly1Engaged (controls.keyMode) ? 1.0f : 0.0f);
    set (poly2, youknow::poly2Engaged (controls.keyMode) ? 1.0f : 0.0f);
    publishKeyModePairAuthority();
...
youknow::sysex::Patch YouKnowAudioProcessor::patchFromEngineParameters (
    const youknow::EngineParameters& source) noexcept
{
...
void YouKnowAudioProcessor::applyPatchToEngineParameters (
    youknow::EngineParameters& destination,
    const youknow::sysex::Patch& patch) noexcept
{
...
void YouKnowAudioProcessor::applyPendingMidiEventToEngine (
    const PendingMidiEvent& event) noexcept
{
    ...
    if (event.kind == PendingMidiEventKind::FullPatch)
        pendingMidiToneShadow =
            youknow::sysex::patchFromToneBytes (event.bytes.data());
    else if (event.kind == PendingMidiEventKind::SingleParameter)
        (void) youknow::sysex::applyParameter (
...
bool YouKnowAudioProcessor::importPatchSysExBytes (const void* data,
                                                      std::size_t size,
                                                      int& patchesFound)
{
    patchesFound = 0;
...
bool YouKnowAudioProcessor::stagePendingMidiEvent (
    const PendingMidiEvent& event) noexcept
{
...
void YouKnowAudioProcessor::stageAndApplyPendingMidiEvent (
...
void YouKnowAudioProcessor::tryStagePendingMidiResync() noexcept
{
    if (! pendingMidiNeedsResync)
        return;
...
void YouKnowAudioProcessor::publishPendingMidiResyncMailbox() noexcept
{
    ...
    std::array<std::uint64_t, youknow::sysex::toneByteCount>* toneByteSequences) const noexcept
{
    ...
    for (int attempt = 0; attempt < 2; ++attempt)
    {
        ...
        patch.range = static_cast<youknow::DcoRange> (switches & 0x3u);
        patch.saw = ((switches >> 2u) & 0x1u) != 0;
        patch.pulse = ((switches >> 3u) & 0x1u) != 0;
        patch.pwmSource = static_cast<youknow::PwmSource> (
            (switches >> 4u) & 0x1u);
        patch.vcaMode = static_cast<youknow::VcaMode> (
            (switches >> 5u) & 0x1u);
        patch.envPolarity = static_cast<youknow::EnvPolarity> (
            (switches >> 6u) & 0x1u);
        patch.highPass = static_cast<youknow::HighPassMode> (
            (switches >> 7u) & 0x3u);
        patch.chorus = static_cast<youknow::ChorusMode> (
            (switches >> 9u) & 0x3u);
...
void YouKnowAudioProcessor::publishKeyModePairAuthority() noexcept
{
    const int legacy = choiceOf (youknow::parameters::legacyKeyMode, 2);
...
void YouKnowAudioProcessor::publishChorusPairAuthority() noexcept
{
    const int legacy = choiceOf (youknow::parameters::legacyChorus, 2);
...
int YouKnowAudioProcessor::resolveLegacyMode (int legacyValue, int fromPair,
...
void YouKnowAudioProcessor::forwardLegacyModeParameters()
{
    using namespace youknow::parameters;
    ...
    if (keyMode != lastLegacyKeyMode.load (std::memory_order_relaxed))
    {
        ...
        const auto mode = static_cast<youknow::KeyMode> (keyMode);
        set (poly1, youknow::poly1Engaged (mode));
        set (poly2, youknow::poly2Engaged (mode));
    ...
    if (chorusMode != lastLegacyChorus.load (std::memory_order_relaxed))
    {
        ...
        const auto mode = static_cast<youknow::ChorusMode> (chorusMode);
        set (chorusI, youknow::chorusOneEngaged (mode));
        set (chorusII, youknow::chorusTwoEngaged (mode));
    ...
    bool programChanged = false;
    {
        ...
        else
        {
            ...
            if (event.replacesTone)
                ...
                for (int parameter = 0;
                     parameter < youknow::sysex::toneByteCount; ++parameter)
...
void YouKnowAudioProcessor::applyToneParameterValues (int parameter, int value)
{
    ...
    if (! youknow::sysex::applyParameter (patch, parameter, value))
        return;

    if (parameter == static_cast<int> (youknow::sysex::ToneParameter::SwitchesOne))
    {
        set (range, static_cast<float> (patch.range));
        set (saw, patch.saw ? 1.0f : 0.0f);
        set (pulse, patch.pulse ? 1.0f : 0.0f);
