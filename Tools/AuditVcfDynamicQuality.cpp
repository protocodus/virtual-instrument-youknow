...
struct YouKnowTestAccess
{
    ...
    using VcfHoldInterval = YouKnowEngine::VcfHoldInterval;
    ...
    {
        return YouKnowEngine::vcfHoldSlewSeconds;
    }
                                   ...
                                   double warmSeconds)
    {
        ...
        const float gradient = YouKnowEngine::chassisGradientCelsius(card);
        const float thermalSpread = 1.0f
            + YouKnowEngine::vcfCutoffTempcoPerCelsius * character
                * (gradient - YouKnowEngine::chassisGradientMeanCelsius());
        return {
    ...
    static float cutoffTargetForByte(int byte, float character) noexcept
    {
        ...
        float counts = YouKnowEngine::vcfPanelCounts(panel);
        counts = std::clamp(counts, 0.0f,
                            YouKnowEngine::vcfCountsCeiling);
        const float code = YouKnowEngine::vcfDacCountStep
            * std::floor(counts / YouKnowEngine::vcfDacCountStep);
        return code
            + YouKnowEngine::vcfConverterCarryCounts(code) * character;
    }
    ...
    static float resonanceTargetForByte(int byte) noexcept
    {
        ...
        return static_cast<float>(
            YouKnowEngine::storedControlDacCode(panel)) / 4064.0f;
    }
        ...
        double hostRate = 8000.0, bool hqEnabled = true)
    {
        ...
        const auto phases = YouKnowEngine::converterEventPhases(
            YouKnowEngine::ConverterTimingProfile::NormalizedServiceChart);
        ...
        bool hqEnabled = true)
    {
        ...
        for (std::size_t probe = 0; probe < result.expectedProbes; ++probe)
        {
            ...
            const double resetSeconds = static_cast<double>(
                YouKnowEngine::resetFraction(periodSeconds))
                                      * periodSeconds;
            ...
            const auto cutoff = YouKnowEngine::exactVcfHoldInterval(
                oldCutoff, oldCutoff, true, eventPosition, newCutoff,
                interval);
            const auto resonance = YouKnowEngine::exactVcfHoldInterval(
            ...
            const auto mapHeld = [&](double cutoffCounts,
                ...
                const float feedback = circuitDerivedShape
                    ? YouKnowEngine::CircuitDerivedResonanceProfile::
                        loopGain(panel)
...
// The tiny moving RK64/RK128 residual is sensitive to cross-platform
// floating-point evaluation. With the fixed full-RES service trim, the
// 88.2 kHz convergence is -155.728 dB on Apple Silicon and -155.883 dB on
// Windows/Linux x86_64: a 0.155 dB spread, over 75 dB below the unchanged
// -80 dB convergence gate. Bound this non-admission fingerprint at 0.20 dB
// (less than 2.4% in residual amplitude). The actual convergence admission
// and the 0.05 dB signal/mutation fingerprints stay unchanged.
constexpr double movingConvergenceGoldenToleranceDb = 0.20;
...
struct ScheduledEvent
{
    ...
    int pass;
    std::size_t ordinal;
    YouKnowTestAccess::Destination destination;
    int voice;
};
...
{
    using Destination = YouKnowTestAccess::Destination;
    constexpr std::array<YouKnowTestAccess::Write, 23> expected {{
    ...
    static_assert(expected.size() == YouKnowTestAccess::writeCount());

    const auto shippingPhases = YouKnowTestAccess::eventPhases();
    const auto shippingWrites = YouKnowTestAccess::writes();
    Schedule schedule;
    schedule.passes = passes;
    schedule.phasesExact = true;
    schedule.orderExact = true;
    schedule.scanRateExact = std::abs(
        1.0 / YouKnowTestAccess::scanRate()
            - converterPassSeconds) <= 1.0e-15
        && std::abs(YouKnowTestAccess::scanRate() / converterScanRate
                    - 1.0) <= 1.0e-15;
...
ControlTrajectories buildTrajectories(
    ...
    const YouKnowTestAccess::CardProfile& profile,
    double eventGridRate = 0.0)
{
    ...
    {
        if (event.destination == YouKnowTestAccess::Destination::Vcf
            && event.voice == card)
        {
            cutoffEvents.push_back({ eventTime(event.time),
                YouKnowTestAccess::cutoffTargetForByte(
                    cutoffBytes[payloadIndex(event.pass)],
                    profile.character) });
        }
        else if (event.destination
                     == YouKnowTestAccess::Destination::Resonance)
        {
            resonanceEvents.push_back({ eventTime(event.time),
                YouKnowTestAccess::resonanceTargetForByte(
    ...
    const double initialResonance =
        YouKnowTestAccess::resonanceTargetForByte(resonanceBytes[0]);
...
{
public:
    ReferenceCascade(const YouKnowTestAccess::CardProfile& profile,
...
private:
    ...
    const YouKnowTestAccess::CardProfile& profile_;
...
{
public:
    EndpointCandidateScan(const YouKnowTestAccess::CardProfile& profile,
                          double internalRate)
        : profile_(profile), internalRate_(internalRate),
          cutoffState_(YouKnowTestAccess::cutoffTargetForByte(
              cutoffBytes[0], profile.character)),
          cutoffTarget_(cutoffState_),
          resonanceState_(YouKnowTestAccess::resonanceTargetForByte(
              resonanceBytes[0])),
    ...
    InstantaneousControls advanceEndpoint()
    {
        ...
        const auto writes = YouKnowTestAccess::writes();
        const auto phases = YouKnowTestAccess::eventPhases();
               ...
               && phases[nextWrite_] <= phase_ + 1.0e-12)
        {
                    ...
                    == YouKnowTestAccess::Destination::Resonance)
            {
                ...
                ++resonanceWrites_;
                         ...
                         == YouKnowTestAccess::Destination::Vcf
                     && write.voice == profile_.card)
            {
                cutoffTarget_ = YouKnowTestAccess::cutoffTargetForByte(
        ...
        const float omegaStep = YouKnowTestAccess::omegaStep(
            cutoffState_, feedback, profile_, internalRate_);
        time_ += 1.0 / internalRate_;
        phase_ += YouKnowTestAccess::scanRate() / internalRate_;
        const double input = toneVolts
            * static_cast<double>(
                YouKnowTestAccess::inputCompensation(feedback))
...
public:
    struct Frame
    {
        ...
        YouKnowTestAccess::ControlTrajectory trajectory {};
        bool exactControl {};
    };
    ...
    Frame advance()
    {
        ...
        YouKnowTestAccess::VcfHoldInterval cutoff;
        YouKnowTestAccess::VcfHoldInterval resonance;
        if (exactControl)
        {
            ...
            cutoff = YouKnowTestAccess::exactVcfHoldInterval(
                ...
                interval);
            resonance = YouKnowTestAccess::exactVcfHoldInterval(
        ...
        if (exactControl)
        {
            constexpr auto positions =
                YouKnowTestAccess::controlNodePositions();
        ...
        const float endpointFeedback = YouKnowTestAccess::feedback(
            resonanceState_, profile_);
        const float endpointOmega = YouKnowTestAccess::omegaStep(
            cutoffState_, endpointFeedback, profile_, internalRate_);
        const double endpointInput = toneVolts
            * static_cast<double>(
                YouKnowTestAccess::inputCompensation(endpointFeedback))
...
private:
    struct PhysicalEvent
    {
        bool active {};
        YouKnowTestAccess::Destination destination {
            YouKnowTestAccess::Destination::Noise
        };
        int voice { -1 };
        double position {};
        ...
        const ScheduledEvent& event) const noexcept
    {
        if (event.destination
                == YouKnowTestAccess::Destination::Resonance)
        {
            return YouKnowTestAccess::resonanceTargetForByte(
                resonanceBytes[payloadIndex(event.pass)]);
        }
        if (event.destination == YouKnowTestAccess::Destination::Vcf)
        {
            return YouKnowTestAccess::cutoffTargetForByte(
                cutoffBytes[payloadIndex(event.pass)],
                profile_.character);
        }
        return 0.0f;
               ...
               && schedule_.events[cursor_].time <= start + 1.0e-15)
        {
            ...
            cursorExact_ = cursorExact_
                && event.pass == static_cast<int>(
                    cursor_ / YouKnowTestAccess::writeCount())
                && event.ordinal
                    == cursor_ % YouKnowTestAccess::writeCount();
    ...
    void peekUpcomingEvent(double start, double end) noexcept
    {
        ...
        const bool relevant = event.destination
                == YouKnowTestAccess::Destination::Resonance
            || event.destination == YouKnowTestAccess::Destination::Vcf;
        if (!relevant)
            return;
    ...
    void checkHoldInterval(
        const YouKnowTestAccess::VcfHoldInterval& actual,
        float state, float target, bool hasEvent, double eventPosition,
        float eventTarget, const ExactHold& globalExact,
        double start, double interval) noexcept
    {
        constexpr auto positions = YouKnowTestAccess::controlNodePositions();
        for (std::size_t point = 0; point < positions.size(); ++point)
        {
            ...
            const double intervalResponse = -std::expm1(
                -interval / YouKnowTestAccess::holdSeconds());
...
public:
                  ...
                  DiagnosticControlPolicy policy)
    {
        const double currentOmega = std::clamp(
            ...
            0.0, YouKnowTestAccess::maximumOmegaStep());
    ...
    std::size_t frameAlignment = 4u)
{
    ...
    YouKnowTestAccess::Cascade candidate;
    candidate.reset();
    YouKnowTestAccess::configure(candidate, profile);
    render.resetExact = YouKnowTestAccess::stateExactlyZero(candidate);
    ...
    for (std::size_t frame = 0; frame < frames; ++frame)
    {
        ...
        const bool stateOkay =
            YouKnowTestAccess::stateFiniteAndBounded(candidate);
        render.finite = render.finite && stateOkay
            && std::isfinite(render.samples[frame]);
        render.maximumState = std::max(
            render.maximumState,
            YouKnowTestAccess::maximumStateMagnitude(candidate));
        if (frame > 8u && YouKnowTestAccess::stateExactlyZero(candidate))
            ++render.recoveries;
    }
    ...
    std::span<const float> rawCandidate)
{
    ...
    auto candidate = YouKnowTestAccess::decimate(aligned, factor);
    auto reference = YouKnowTestAccess::decimate(
        referenceTake.rk128, factor);
    const double internalRate = 32000.0;
    const double passSeconds = 1.0 / YouKnowTestAccess::scanRate();
    ...
    std::string name)
{
    ...
    const std::size_t candidateFrames = framesUnaligned
                                      - framesUnaligned % 4u;
    const std::size_t oracleFrames = candidateFrames * 4u;
    const double initialCutoff = YouKnowTestAccess::cutoffTargetForByte(
        cutoffBytes[1], profile.character);
    const double initialResonance =
        YouKnowTestAccess::resonanceTargetForByte(resonanceBytes[1]);
    const ControlTrajectories trajectories {
        ExactHold(initialCutoff, {}, YouKnowTestAccess::holdSeconds()),
        ExactHold(initialResonance, {}, YouKnowTestAccess::holdSeconds()),
        0u, 0u, 0u
    };
    ...
    candidate.reset();
    YouKnowTestAccess::configure(candidate, profile);
    take.resetExact = YouKnowTestAccess::stateExactlyZero(candidate);
...
EightKDiagnostics diagnoseEightK(
    const Schedule& schedule,
    const std::vector<YouKnowTestAccess::CardProfile>& profiles,
    ...
    const quality::KaiserLowPass& filter)
{
    ...
    for (std::size_t index = 0; index < profiles.size(); ++index)
    {
        ...
        constexpr std::size_t expectedWrites =
            renderedPasses * YouKnowTestAccess::writeCount();
        ...
        for (std::size_t frame = 0; frame < frameCount; ++frame)
        {
            for (const double position : nodePositions)
            {
                ...
                result.maximumUnclampedOmegaStep = std::max(
                    ...
                    YouKnowTestAccess::unclampedOmegaStep(
    ...
    result.omegaCapEngaged = result.maximumUnclampedOmegaStep
        >= YouKnowTestAccess::maximumOmegaStep();
    return result;
}
                      ...
                      int expectedPasses = renderedPasses)
{
    ...
    const auto prepared = YouKnowTestAccess::shippingProcessingRate(
        ...
        std::vector<float> alignedCandidate(
            ...
            rawCandidate.samples.begin()
                + static_cast<std::ptrdiff_t>(
                    firstInternal + take.candidate.size()));
        auto candidate = YouKnowTestAccess::decimate(
            alignedCandidate, cell.factor);
        auto rk64 = YouKnowTestAccess::decimate(take.rk64, cell.factor);
        auto rk128 = YouKnowTestAccess::decimate(take.rk128, cell.factor);
        const std::size_t relativeCapture = static_cast<std::size_t>(std::max(
...
double hotDriveAmplitude()
{
    return hotAmplitude
         * static_cast<double>(
             YouKnowTestAccess::inputCompensation(hotFeedback));
}
...
HotCellMetrics auditHotCell(double hostRate)
{
    ...
    const auto profile = YouKnowTestAccess::cardProfile(0, 0.0f, 0.0);
    YouKnowTestAccess::Cascade candidate;
    candidate.reset();
    YouKnowTestAccess::configure(candidate, profile);
    metrics.resetExact = YouKnowTestAccess::stateExactlyZero(candidate);
...
bool sameProfile(const YouKnowTestAccess::CardProfile& left,
                 const YouKnowTestAccess::CardProfile& right) noexcept
{
...
struct Audit
{
    ...
    bool scheduleExact {};
    YouKnowTestAccess::SchedulerContract schedulerContract;
    YouKnowTestAccess::ShippingWiringContract shippingWiringContract;
    ...
    std::array<YouKnowTestAccess::SchedulerContract,
               lowerRateMovingCells.size()> lowerRateSchedulerContracts {};
    YouKnowTestAccess::ShippingWiringContract lowerRateWiringContract;
...
Audit runAudit(bool diagnoseEightKCell = false)
{
    ...
    Audit audit;
    audit.schedulerContract = YouKnowTestAccess::schedulerContract();
    audit.shippingWiringContract =
        YouKnowTestAccess::shippingWiringContract();
    ...
    const auto nominal = YouKnowTestAccess::cardProfile(0, 0.0f, 0.0);
    audit.nominalCollapseExact = true;
    for (int card = 0; card < 6; ++card)
    {
        const auto cold = YouKnowTestAccess::cardProfile(card, 0.0f, 0.0);
        const auto warm = YouKnowTestAccess::cardProfile(
    ...
    std::vector<YouKnowTestAccess::CardProfile> profiles;
    ...
    profiles.push_back(YouKnowTestAccess::cardProfile(
        0, 0.0f, warmSeconds));
    ...
    audit.lowerRateWiringContract =
        YouKnowTestAccess::shippingWiringContract(false);
         ...
         index < lowerRateMovingCells.size(); ++index)
    {
        audit.lowerRateSchedulerContracts[index] =
            YouKnowTestAccess::schedulerContract(
...
void printAudit(const Audit& audit)
{
    ...
    if (audit.eightKDiagnostics.ran)
    {
        ...
        std::cout << "8 kHz isolation (diagnostic only; non-admission):\n"
                  ...
                  << " cap=" << YouKnowTestAccess::maximumOmegaStep()
                  << " engaged="
...
int main(int argc, char** argv)
{
    try
    {
        ...
        {
            std::cerr << "Usage: YouKnowVcfDynamicQualityAudit "
