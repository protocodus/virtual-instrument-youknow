...
constexpr auto expectedParameters = std::to_array<ParameterExpectation> ({
    ...
    { parameters::quality,     0.0f,   1.0e-5f },
    { parameters::vcfTanhMode,
      static_cast<float> (YouKnowAudioProcessor::vcfTanhDefaultChoice),
      1.0e-5f },
    { parameters::vcfFastEarlyMode,
      static_cast<float> (
          YouKnowAudioProcessor::vcfFastEarlyDefaultChoice),
      1.0e-5f },
    { parameters::vcfSolverMode,
      static_cast<float> (YouKnowAudioProcessor::vcfSolverDefaultChoice),
      1.0e-5f },
...
void setParameterValue (YouKnowAudioProcessor& processor, const char* id, float value)
{
...
{
    YouKnowAudioProcessor reference;
...
struct AutomationRecorder final : juce::AudioProcessorParameter::Listener
{
    explicit AutomationRecorder (juce::RangedAudioParameter& value) : parameter (value)
    {
        parameter.addListener (this);
    }
    ~AutomationRecorder() override { parameter.removeListener (this); }

    void parameterValueChanged (int, float value) override
    {
        events.push_back ('v');
        values.push_back (parameter.convertFrom0to1 (value));
    }
    void parameterGestureChanged (int, bool starting) override
    {
        events.push_back (starting ? 'b' : 'e');
    }
...
void renderBlocks (YouKnowAudioProcessor& processor, juce::AudioBuffer<float>& buffer,
                   int blocks)
{
...
void testPublicParameterOrderMatchesTheSharedList()
{
    ...
    const auto& shared = youknow::tests::publicParameterOrder;
    ...
    for (std::size_t index = 0; index < shared.size(); ++index)
    {
        ...
        expect (parameter != nullptr
                    && parameter->isAutomatable() == shared[index].automatable,
                std::string ("unexpected automation policy for ") + shared[index].id);
    }
}
...
void testParameterContract()
{
            ...
            parameters::vcfTanhMode))
    {
        ...
        // either direction.
        expect (YouKnowAudioProcessor::vcfTanhDefaultChoice == 2
                    && choice->getIndex()
                           == YouKnowAudioProcessor::vcfTanhDefaultChoice,
            ...
            parameters::vcfSolverMode))
    {
        ...
        expect (YouKnowAudioProcessor::vcfSolverDefaultChoice
                    == YouKnowAudioProcessor::vcfSolverChoiceCount - 1,
                "the shipped VCF Solver default is no longer the cheapest rung");
        expect (std::strcmp (YouKnowAudioProcessor::vcfSolverChoiceName (
                                 YouKnowAudioProcessor::
        ...
        for (int rung = 0;
             rung < YouKnowAudioProcessor::vcfSolverChoiceCount; ++rung)
        {
            const juce::String name (
                YouKnowAudioProcessor::vcfSolverChoiceName (rung));
            const juce::String technique (
                YouKnowAudioProcessor::vcfSolverChoiceTechnique (rung));
        ...
        expect (youknow::EngineParameters {}.vcfSolverMode
    ...
    {
        const auto& range = aging->getNormalisableRange();
        expect (std::abs (aging->convertFrom0to1 (aging->getDefaultValue())
                         - (range.start + range.end) * 0.5f) < 1.0e-6f,
...
void testReportedDspLatencyIsForwardedForEveryNumericalPath()
{
    ...
    for (const auto& configuration : configurations)
    {
        ...
        setParameterValue (
            ...
            static_cast<float> (
                YouKnowAudioProcessor::choiceForOversamplingFactor (
...
void testShortNoteInsideOneBlockIsHeard()
{
    ...
    const float timingFloor = idleNoiseFloor ([] (YouKnowAudioProcessor& reference)
    {
...
{
    YouKnowAudioProcessor exact;
    YouKnowAudioProcessor hermite;
    const auto configure = [] (YouKnowAudioProcessor& processor,
                               float tanhChoice)
    {
        setParameterValue (processor, parameters::quality, 0.0f);
...
void testVcfSolverSelectorDrivesTheEngine()
{
    ...
    std::array<std::unique_ptr<YouKnowAudioProcessor>,
               YouKnowAudioProcessor::vcfSolverChoiceCount> processors;
    const auto configure = [] (YouKnowAudioProcessor& processor,
                               float solverChoice)
    {
        setParameterValue (processor, parameters::quality, 0.0f);
         ...
         choice < YouKnowAudioProcessor::vcfSolverChoiceCount; ++choice)
    {
        processors[static_cast<std::size_t> (choice)] =
            std::make_unique<YouKnowAudioProcessor>();
    ...
    for (int block = 0; block < 48; ++block)
    {
        std::array<juce::AudioBuffer<float>,
                   YouKnowAudioProcessor::vcfSolverChoiceCount> audio;
...
void testUiPerformanceLeverMatchesMidiAndCoalesces()
{
    ...
    YouKnowAudioProcessor midiDriven;
    ...
    setParameterValue (uiDriven, parameters::pitchBend, 1.0f);
    setParameterValue (uiDriven, parameters::modulation, 1.0f);
    ...
    for (int index = 0; index < 10000; ++index)
    {
        ...
        setParameterValue (uiDriven, parameters::pitchBend, phase);
        setParameterValue (uiDriven, parameters::modulation,
                           static_cast<float> (index % 128) / 127.0f);
    }
    setParameterValue (uiDriven, parameters::pitchBend, 0.0f);
    setParameterValue (uiDriven, parameters::modulation, 0.0f);
    ...
    // Each automation lane only publishes when it changes. Modulation must
    // not overwrite a held external pitch wheel, nor may an old automation
    // value reassert itself on blocks after the host stops changing it.
    float independentAxisDifference = 0.0f;
    for (int block = 0; block < 16; ++block)
    {
        juce::MidiBuffer uiMidi, midi;
        if (block == 0)
            for (auto* events : { &uiMidi, &midi })
            {
                events->addEvent (juce::MidiMessage::pitchWheel (1, 4096), 0);
                events->addEvent (juce::MidiMessage::controllerEvent (1, 1, 77), 0);
            }
        if (block == 2)
        {
            setParameterValue (uiDriven, parameters::modulation, 0.5f);
            midi.addEvent (juce::MidiMessage::controllerEvent (1, 1, 64), 0);
        }
        if (block == 4)
            for (auto* events : { &uiMidi, &midi })
                events->addEvent (juce::MidiMessage::controllerEvent (1, 1, 12), 0);
        if (block == 6)
        {
            setParameterValue (uiDriven, parameters::pitchBend, -1.0f);
        ...
        if (block == 8)
            for (auto* events : { &uiMidi, &midi })
                events->addEvent (juce::MidiMessage::pitchWheel (1, 12000), 0);
        uiBuffer.clear();
        midiBuffer.clear();
        uiDriven.processBlock (uiBuffer, uiMidi);
        midiDriven.processBlock (midiBuffer, midi);
        independentAxisDifference = std::max (
            independentAxisDifference,
            maximumBufferDifference (uiBuffer, midiBuffer));
    }
    expect (independentAxisDifference < 1.0e-6f,
            "performance automation overwrote an unchanged MIDI axis or reasserted later");
...
void testPerformanceAutomationPersistsOnlyInSessionState()
{
    YouKnowAudioProcessor source;
    source.setCurrentProgram (3);
    const auto patchBefore = source.currentPatchAsSysEx (1);
    setParameterValue (source, parameters::pitchBend, -0.72f);
    setParameterValue (source, parameters::modulation, 0.63f);
    const auto patchAfter = source.currentPatchAsSysEx (1);
    expect (patchBefore.getRawDataSize() == patchAfter.getRawDataSize()
                && std::memcmp (patchBefore.getRawData(), patchAfter.getRawData(),
                                static_cast<std::size_t> (patchBefore.getRawDataSize())) == 0
                && ! source.currentProgramIsEdited(),
            "performance automation changed the stored tone or marked it edited");
    ...
    {
        expect (std::abs (parameterValue (restored, parameters::pitchBend) + 0.72f)
                    < 1.0e-6f
                    && std::abs (parameterValue (restored, parameters::modulation) - 0.63f)
                           < 1.0e-6f,
                "session recall, a program or randomisation moved performance automation");
    };
    expectPosition();
    restored.setCurrentProgram (0);
    expectPosition();
    restored.setCurrentProgram (7);
    expectPosition();
    restored.randomizeParameters (1.0f);
    expectPosition();
}
...
void testPerformanceLeverSpringsAndEditorCloseNeutralisesIt()
{
    ...
    YouKnowAudioProcessor closedEditor;
    YouKnowAudioProcessor reference;
    ...
    auto* editorLever = editor != nullptr
        ? dynamic_cast<YouKnowPerformanceLever*> (
    ...
    YouKnowAudioProcessor externalWithEditor;
    YouKnowAudioProcessor externalReference;
...
void testPerformanceLeverAutomationGesturesAndHostFeedback()
{
    YouKnowAudioProcessor processor;
    auto* bend = processor.parameters.getParameter (parameters::pitchBend);
    auto* modulation = processor.parameters.getParameter (parameters::modulation);
    expect (bend != nullptr && modulation != nullptr,
            "the performance axes have no host parameters");
    if (bend == nullptr || modulation == nullptr)
        return;
    AutomationRecorder bendRecorder (*bend), modRecorder (*modulation);
    std::unique_ptr<juce::AudioProcessorEditor> editor (processor.createEditor());
    auto* lever = editor != nullptr
        ? dynamic_cast<YouKnowPerformanceLever*> (
              findDescendantNamed (*editor, "Pitch and modulation lever"))
        : nullptr;
    expect (lever != nullptr, "the host automation check has no performance lever");
    if (lever == nullptr)
        return;
    expect (bendRecorder.events.empty() && modRecorder.events.empty(),
            "creating the lever wrote parameters or began a host gesture");

    setParameterValue (processor, parameters::pitchBend, -0.75f);
    setParameterValue (processor, parameters::modulation, 0.625f);
    expect (std::abs (lever->getPitchBend() + 0.75f) < 1.0e-6f
                && std::abs (lever->getModulation() - 0.625f) < 1.0e-6f,
            "the visible performance lever did not follow host automation");
    expect (bendRecorder.events == std::vector<char> { 'v' }
                && modRecorder.events == std::vector<char> { 'v' },
            "host automation fed back additional values or UI gestures");

    bendRecorder.clear();
    modRecorder.clear();
    editor.reset();
    expect (bendRecorder.events.empty() && modRecorder.events.empty()
                && std::abs (parameterValue (processor, parameters::pitchBend) + 0.75f)
                       < 1.0e-6f
                && std::abs (parameterValue (processor, parameters::modulation) - 0.625f)
                       < 1.0e-6f,
            "closing an untouched lever reset host automation or emitted a gesture");

    editor.reset (processor.createEditor());
    lever = editor != nullptr
        ? dynamic_cast<YouKnowPerformanceLever*> (
              findDescendantNamed (*editor, "Pitch and modulation lever"))
        : nullptr;
    expect (lever != nullptr, "the reopened editor has no performance lever");
    if (lever == nullptr)
        return;
    expect (std::abs (lever->getPitchBend() + 0.75f) < 1.0e-6f
                && std::abs (lever->getModulation() - 0.625f) < 1.0e-6f
                && bendRecorder.events.empty() && modRecorder.events.empty(),
            "reopening the editor did not quietly restore the automated lever position");

    const auto expectCompleteGesture = [&] (const char* action)
    {
        for (auto* recorder : { &bendRecorder, &modRecorder })
        {
            expect (recorder->events.size() >= 3
                        && recorder->events.front() == 'b'
                        && recorder->events.back() == 'e'
                        && std::count (recorder->events.begin(), recorder->events.end(), 'b') == 1
                        && std::count (recorder->events.begin(), recorder->events.end(), 'e') == 1
                        && ! recorder->values.empty()
                        && std::abs (recorder->values.back()) < 1.0e-6f,
                    std::string (action)
                        + " did not record one balanced host gesture ending with spring return");
            recorder->clear();
        }
    };
    const auto upperRight = juce::Point<float> {
        static_cast<float> (lever->getWidth() - 2), 2.0f };
    const auto held = juce::ModifierKeys (juce::ModifierKeys::leftButtonModifier);
    lever->mouseDown (mouseEventFor (*lever, upperRight, held));
    lever->mouseDrag (mouseEventFor (*lever, { 2.0f, 2.0f }, held));
    lever->mouseUp (mouseEventFor (*lever, { 2.0f, 2.0f }, {}));
    expectCompleteGesture ("Mouse drag");

    lever->keyPressed (juce::KeyPress { juce::KeyPress::rightKey });
    lever->keyPressed (juce::KeyPress { juce::KeyPress::upKey });
    lever->keyStateChanged (false);
    expectCompleteGesture ("Arrow keys");

    lever->mouseDown (mouseEventFor (*lever, upperRight, held));
    lever->focusLost (juce::Component::focusChangedDirectly);
    expectCompleteGesture ("Losing focus during a drag");
    lever->mouseDrag (mouseEventFor (*lever, upperRight, held));
    lever->mouseUp (mouseEventFor (*lever, upperRight, {}));
    expect (bendRecorder.events.empty() && modRecorder.events.empty()
                && std::abs (parameterValue (processor, parameters::pitchBend)) < 1.0e-6f
                && std::abs (parameterValue (processor, parameters::modulation)) < 1.0e-6f,
            "a stray pointer event after focus loss reopened the ended gesture");

    lever->mouseDown (mouseEventFor (*lever, upperRight, held));
    editor.reset();
    expectCompleteGesture ("Closing during a drag");
    expect (std::abs (parameterValue (processor, parameters::pitchBend)) < 1.0e-6f
                && std::abs (parameterValue (processor, parameters::modulation)) < 1.0e-6f,
            "closing an active lever left its host automation lanes displaced");
}
...
void testStateRoundTripAndMigration()
{
    ...
    setParameterValue (source, parameters::quality, 2.0f);
    setParameterValue (source, parameters::aging, 0.0f);
    ...
    YouKnowAudioProcessor destination;
    ...
    expect (juce::roundToInt (parameterValue (destination, parameters::quality)) == 2
                && parameterValue (destination, parameters::aging) == 0.0f,
            "new-instance defaults replaced explicit saved Quality or Aging");

    auto preQualityAndAging = source.parameters.copyState();
    removeParameter (preQualityAndAging, parameters::quality);
    removeParameter (preQualityAndAging, parameters::legacyHq);
    removeParameter (preQualityAndAging, parameters::aging);
    juce::MemoryBlock historicalSettings;
    if (serialise (preQualityAndAging, historicalSettings))
    {
        YouKnowAudioProcessor migrated;
        migrated.setStateInformation (historicalSettings.getData(),
                                      static_cast<int> (historicalSettings.getSize()));
        expect (juce::roundToInt (parameterValue (migrated, parameters::quality)) == 2
                    && parameterValue (migrated, parameters::aging) == 0.0f,
                "new-instance defaults changed missing historical Quality or Aging");
    }
    else
    {
        expect (false, "could not serialise historical Quality/Aging settings");
    }
    ...
    if (savedXml != nullptr)
    {
        ...
        if (serialise (legacyVolume, legacyVolumeBytes))
        {
            ...
            expect (std::abs (
                        YouKnowEngine::outputCouplingHighGain (restored)
                        - formerGain) < 2.0e-5f,
...
void testLegacySplitModeMigration()
{
    ...
    expect (restoreWithLegacyHq (1.0f, false, 0)
                == YouKnowAudioProcessor::qualityChoiceCount - 1,
    ...
    YouKnowAudioProcessor canonical;
...
void testEveryStoredPatchFieldRecallsWithoutMovingPerformanceControls()
{
    ...
    constexpr auto performanceValues = std::to_array<std::pair<const char*, float>> ({
        ...
        { parameters::pitchBend,   -0.72f },
        { parameters::modulation,  0.63f },
...
void testHostResetClearsRuntimeStateWithoutUnpreparing()
{
    ...
    const float resetFloor = idleNoiseFloor ([] (YouKnowAudioProcessor& reference)
    {
        setParameterValue (reference, parameters::sustain, 1.0f);
...
void testHostResetKeepsTheModelledChassisWarm()
{
    ...
    const float warmFloor = idleNoiseFloor ([] (YouKnowAudioProcessor& reference)
    {
        setParameterValue (reference, parameters::sustain, 1.0f);
    }, 4);
...
void testPatchFileImportAppliesFirstPatchAndCountsTheRest()
{
    ...
    // A new F0 abandons an incomplete message in a MIDI stream. A damaged
    // prefix must not consume the first valid patch's terminating F7.
    file.insert (file.begin(), { 0xf0, 0x41, 0x31, 0x00 });
    file.insert (file.begin() + 10, { 0xf0, 0x41 });
    patchesFound = 0;
    expect (processor.importPatchSysExBytes (file.data(), file.size(),
                                             patchesFound)
                && patchesFound == 2,
            "an interrupted SysEx prefix swallowed a later valid patch");
...
void testLaterRecallSupersedesPendingMidi()
{
    for (const int recallKind : { 0, 1, 2 })
        ...
        {
            YouKnowAudioProcessor pending, reference, selected;
            selected.setCurrentProgram (9);
            juce::MemoryBlock saved;
            selected.getStateInformation (saved);
            for (auto* processor : { &pending, &reference })
                processor->prepareToPlay (sampleRate, blockSize);

            // The second case also leaves a coalesced overflow mailbox. No
            // message-loop tick runs before the later explicit recall.
            ...
            for (auto* processor : { &pending, &reference })
            {
                ...
                for (int event = 0; event < eventCount; ++event)
                    midi.addEvent (juce::MidiMessage::programChange (1, 2), 0);
                processor->processBlock (zero, midi);
            }
            reference.flushPendingMidiEvents();
            for (auto* processor : { &pending, &reference })
            {
                if (recallKind == 0)
                    ...
                    processor->applyPatch (selected.currentPatch());
            }
            const int selectedProgram = pending.getCurrentProgram();
            const auto expectedPatch = pending.currentPatch();
            if (eventCount > 1)
                pending.flushPendingMidiEvents();
            juce::AudioBuffer<float> actual (2, blockSize), expected (2, blockSize);
            for (auto* processor : { &pending, &reference })
            {
                juce::MidiBuffer midi;
                midi.addEvent (juce::MidiMessage::noteOn (1, 60, 0.8f), 0);
                processor->processBlock (processor == &pending ? actual : expected,
                                         midi);
            }
            expect (maximumBufferDifference (actual, expected) < 2.0e-5f,
                    "a pending MIDI shadow overrode a later explicit recall");

            pending.flushPendingMidiEvents();
            std::array<std::uint8_t, sysex::toneByteCount> actualTone {}, expectedTone {};
            sysex::toneBytesFromPatch (pending.currentPatch(), actualTone.data());
            sysex::toneBytesFromPatch (expectedPatch, expectedTone.data());
            expect (actualTone == expectedTone
                        && pending.getCurrentProgram() == selectedProgram,
                    "deferred MIDI reflection overwrote a later explicit recall");

            // New MIDI remains authoritative, including an all-parameter
            // overflow that must not resurrect the preceding program index.
            std::array<std::uint8_t, sysex::parameterMessageBytes> raw {};
            const auto written = sysex::writeParameterMessage (
                static_cast<int> (sysex::ToneParameter::VcfFreq), 23, 0,
                raw.data(), raw.size());
            juce::MidiBuffer later;
            for (int event = 0; event < 70; ++event)
                later.addEvent (juce::MidiMessage::createSysExMessage (
                    raw.data() + 1, static_cast<int> (written) - 2), 0);
            pending.processBlock (zero, later);
            pending.flushPendingMidiEvents();
            expect (std::abs (parameterValue (pending, parameters::cutoff)
                             - 23.0f / 127.0f) < 1.0e-5f,
                    "an explicit recall discarded newer MIDI parameter changes");
            expect (pending.getCurrentProgram() == selectedProgram,
                    "a later MIDI overflow resurrected a superseded program index");
        }
}
...
void testStateSaveIncludesUnreflectedMidiTone()
{
    for (const bool fullDump : { false, true })
        for (const int eventCount : { 1, 70 })
        {
            YouKnowAudioProcessor source;
            source.setCurrentProgram (7);
            constexpr float volume = 0.4321f;
            setParameterValue (source, parameters::volume, volume);
            source.prepareToPlay (sampleRate, blockSize);
            const auto oldCutoff = parameterValue (source, parameters::cutoff);
            const auto wanted = source.programPatch (3);
            std::array<std::uint8_t, sysex::patchMessageBytes> dump {};
            const auto written = sysex::writePatchMessage (
                wanted, 0, dump.data(), dump.size());
            juce::MidiBuffer midi;
            for (int event = 0; event < eventCount; ++event)
                midi.addEvent (
                    fullDump ? juce::MidiMessage::createSysExMessage (
                                   dump.data() + 1, static_cast<int> (written) - 2)
                             : juce::MidiMessage::programChange (1, 2),
                    event);
            juce::AudioBuffer<float> buffer (2, blockSize);
            ...
            juce::MemoryBlock state;
            std::thread hostSave ([&] { source.getStateInformation (state); });
            hostSave.join();
            expect (source.getCurrentProgram() == 7
                        && std::abs (parameterValue (source, parameters::cutoff)
                                     - oldCutoff) < 1.0e-6f,
                    "state save consumed MIDI reflection or changed live parameters");
            YouKnowAudioProcessor restored;
            restored.setStateInformation (state.getData(),
                                          static_cast<int> (state.getSize()));
            std::array<std::uint8_t, sysex::toneByteCount> expectedTone {}, savedTone {};
            sysex::toneBytesFromPatch (wanted, expectedTone.data());
            sysex::toneBytesFromPatch (restored.currentPatch(), savedTone.data());
            expect (savedTone == expectedTone
                        && restored.getCurrentProgram() == (fullDump ? 7 : 3),
                    "immediate host save omitted queued or overflowed MIDI tone/program");
            expect (std::abs (parameterValue (restored, parameters::volume) - volume)
                        < 1.0e-5f,
                    "saving a MIDI tone replaced a live performance control");

            // A complete UI/host recall is newer than the published MIDI tone,
            // even if no audio block or message-thread reflection follows it.
            source.setCurrentProgram (9);
            source.getStateInformation (state);
            restored.setStateInformation (state.getData(),
                                          static_cast<int> (state.getSize()));
            expect (restored.getCurrentProgram() == 9
                        && ! restored.currentProgramIsEdited(),
                    "state save resurrected MIDI superseded by a later host recall");
            source.flushPendingMidiEvents();
            expect (source.getCurrentProgram() == 9 && ! source.currentProgramIsEdited(),
                    "saving MIDI changed subsequent stale-event retirement");
            source.releaseResources();
        }
}

void testStateSavePreservesEditsAfterMidiReflection()
{
    YouKnowAudioProcessor source;
    source.prepareToPlay (sampleRate, blockSize);
    juce::AudioBuffer<float> buffer (2, blockSize);
    ...
    // Hosts can edit and save synchronously from the reflected program-change
    // notification. The MIDI acknowledgement must already belong to APVTS.
    struct SaveAfterProgramNotification final : juce::AudioProcessorListener
    {
        void audioProcessorParameterChanged (juce::AudioProcessor*, int, float) override {}
                                     ...
                                     const ChangeDetails& details) override
        {
            if (! details.programChanged)
                return;
            auto& source = *static_cast<YouKnowAudioProcessor*> (processor);
            setParameterValue (source, parameters::release, 0.8123f);
            source.getStateInformation (state);
        }
        juce::MemoryBlock state;
    } listener;
    ...
    expect (! listener.state.isEmpty(), "MIDI reflection sent no program notification");
    YouKnowAudioProcessor restored;
    restored.setStateInformation (listener.state.getData(),
                                  static_cast<int> (listener.state.getSize()));
    expect (std::abs (parameterValue (restored, parameters::release) - 0.8123f) < 1.0e-5f,
            "state save overlaid acknowledged MIDI onto a later host edit");

    std::array<std::uint8_t, sysex::parameterMessageBytes> message {};
    const auto written = sysex::writeParameterMessage (
        static_cast<int> (sysex::ToneParameter::VcfFreq), 17, 0,
        message.data(), message.size());
    for (int event = 0; event < 70; ++event)
        midi.addEvent (juce::MidiMessage::createSysExMessage (
                           message.data() + 1, static_cast<int> (written) - 2), event);
    source.processBlock (buffer, midi);
    setParameterValue (source, parameters::release, 0.7345f);
    juce::MemoryBlock state;
    source.getStateInformation (state);
    restored.setStateInformation (state.getData(), static_cast<int> (state.getSize()));
    expect (std::abs (parameterValue (restored, parameters::cutoff) - 17.0f / 127.0f)
                < 1.0e-5f
                && std::abs (parameterValue (restored, parameters::release) - 0.7345f)
                       < 1.0e-5f,
            "saving pending single-parameter MIDI reverted unrelated later automation");
    source.releaseResources();
}
...
{
    YouKnowAudioProcessor recalled;
    ...
    YouKnowAudioProcessor chorusTwo;
...
void testFactoryProgramsLoad()
{
    ...
    expect (processor.getNumPrograms()
                == presets::presetCount + presets::productPresetCount + 1,
            "the host does not see the factory and original product banks");
    ...
    {
        expect (presets::programPreset (index)->exportsLosslessly(),
...
void testOriginalProductProgramsPreserveFactorySlotsAndState()
{
    using Category = presets::Preset::Category;
    YouKnowAudioProcessor processor;
    std::set<std::array<std::uint8_t, sysex::toneByteCount>> tones;
    for (int index = 0; index < presets::presetCount; ++index)
    {
        const auto* preset = presets::programPreset (index + 1);
        expect (preset == &presets::factoryBank()[static_cast<std::size_t> (index)],
                "original collection displaced an archival factory slot");
        std::array<std::uint8_t, sysex::toneByteCount> bytes {};
        sysex::toneBytesFromPatch (preset->patch, bytes.data());
        tones.insert (bytes);
    }
    expect (presets::programPreset (0) == nullptr
                && presets::programPreset (-1) == nullptr
                && presets::programPreset (processor.getNumPrograms()) == nullptr,
            "preset lookup accepted INIT or an invalid bank index");

    for (int index = 0; index < presets::productPresetCount; ++index)
    {
        const int program = presets::presetCount + index + 1;
        const auto& preset = presets::productBank()[static_cast<std::size_t> (index)];
        expect (presets::programPreset (program) == &preset
                    && presets::findByNumber (preset.number) == &preset,
                "original program cannot be found by its stable index or number");
        expect (preset.category == (index < 8 ? Category::Bass : Category::Pad),
                "original collection lost its eight basses/eight pads partition");
        std::array<std::uint8_t, sysex::toneByteCount> bytes {};
        sysex::toneBytesFromPatch (preset.patch, bytes.data());
        expect (tones.insert (bytes).second,
                "an original program duplicates another bank's tone bytes");
        processor.setCurrentProgram (program);
        expect (! processor.currentProgramIsEdited(),
                "an original program was marked edited immediately after recall");
        juce::MemoryBlock state;
        processor.getStateInformation (state);
        YouKnowAudioProcessor restored;
        restored.setStateInformation (state.getData(), static_cast<int> (state.getSize()));
        expect (restored.getCurrentProgram() == program
                    && ! restored.currentProgramIsEdited(),
                "an appended original program did not survive a session round trip");
    }

    processor.prepareToPlay (sampleRate, blockSize);
    juce::AudioBuffer<float> zero (2, 0);
    for (const int midiProgram : { 0, 127 })
    {
        processor.setCurrentProgram (processor.getNumPrograms() - 1);
        juce::MidiBuffer midi;
        midi.addEvent (juce::MidiMessage::programChange (1, midiProgram), 0);
        processor.processBlock (zero, midi);
        processor.flushPendingMidiEvents();
        expect (processor.getCurrentProgram() == midiProgram + 1,
                "the original collection changed hardware MIDI bank addressing");
    }
}
...
void testHardwareProgrammerAddressesCompleteFactoryBank()
{
    ...
    expect (selector != nullptr
                && selector->getNumItems() == processor.getNumPrograms(),
            "the preset menu does not contain every program exactly once");
    if (selector != nullptr)
    {
        std::vector<bool> present (static_cast<std::size_t> (processor.getNumPrograms()));
        for (int item = 0; item < selector->getNumItems(); ++item)
        {
            const int program = selector->getItemId (item) - 1;
            expect (program >= 0 && program < processor.getNumPrograms(),
                    "the sound-family menu has an invalid program ID");
            if (program < 0 || program >= processor.getNumPrograms())
                continue;
            expect (! present[static_cast<std::size_t> (program)],
                    "the sound-family menu lists a program twice");
            present[static_cast<std::size_t> (program)] = true;
            expect (selector->getItemText (item) == processor.getProgramName (program),
                    "the sound-family menu changed a saved program's identity");
        }
        for (const auto* name : { "Basses", "Brass", "Strings", "Pads" })
        {
            int sounds = 0;
            for (juce::PopupMenu::MenuItemIterator item (*selector->getRootMenu()); item.next();)
                if (item.getItem().text == name && item.getItem().subMenu != nullptr)
                    sounds = item.getItem().subMenu->getNumItems();
            expect (sounds >= 8, std::string (name) + " has too few choices in its menu");
        }
    }
    ...
    if (selector != nullptr)
    {
        selector->setSelectedId (processor.getNumPrograms(), juce::sendNotificationSync);
        expect (processor.getCurrentProgram() == processor.getNumPrograms() - 1,
                "the categorized menu cannot recall the final product preset");
        for (auto* group : groups)
            expect (! group->getToggleState(),
                    "a product preset incorrectly lights a hardware memory group");
        groups[0]->onClick();
        expect (processor.getCurrentProgram() == 64,
                "returning from a product preset lost the last hardware bank and patch");
    }
}
...
void testDerivedOriginalPanelSwitchesDriveTheirExistingParameters()
{
    ...
    auto* portamento = dynamic_cast<juce::Slider*> (
        findDescendantNamed (*editor, "PORTA"));
    auto* portamentoSwitch = findDescendantNamed (*editor, "Portamento switch");
    ...
    expect (chorusOff != nullptr && portamento != nullptr && keyTranspose != nullptr,
            "an original panel control is missing from the editor");
    expect (portamentoSwitch == nullptr,
            "the redundant PORTA enable switch is still in the editor");
    ...
    portamento->setValue (0.0, juce::sendNotificationSync);
    expect (parameterValue (processor, parameters::portamento) < 1.0e-5f,
            "zero PORTA did not disable glide");
    portamento->setValue (0.46, juce::sendNotificationSync);
    expect (std::abs (parameterValue (processor, parameters::portamento) - 0.46f)
                < 1.0e-4f,
            "non-zero PORTA did not enable glide at the selected time");
...
void testEveryProductProgramRestoresEveryParameter()
{
    ...
    for (int program = 0; program < processor.getNumPrograms(); ++program)
    {
        ...
        const float poisonedBend =
            parameterValue (processor, parameters::pitchBend);
        const float poisonedModulation =
            parameterValue (processor, parameters::modulation);
        ...
        for (std::size_t index = 0; index < expectedParameters.size(); ++index)
        {
            ...
            if (! isProgramParameter (expected.id))
            {
                ...
                else if (std::strcmp (expected.id, parameters::pitchBend) == 0)
...
void testEveryPanelLegendFitsInTheRealFont()
{
    ...
    const auto engravedFont = [] (float height) {
        auto font = juce::Font (juce::FontOptions (height, juce::Font::bold));
                                       ...
                                       const char* sizeName)
    {
        ...
        const float available = (section.width - 4.0f) * scale;

        float titleSize = juce::jmax (11.0f,
                                       panel::sectionHeadingPointSize * scale);
        const float natural = juce::GlyphArrangement::getStringWidth (
            engravedFont (titleSize), section.displayTitle);
        ...
        expect (drawn <= available + 0.1f,
                say (juce::String (sizeName) + " section heading is truncated: "
                     + section.displayTitle + " needs "
                      + juce::String (drawn, 1) + " in "
                      + juce::String (available, 1)));
        expect (drawn + 18.0f * scale < section.width * scale,
                say (juce::String (sizeName)
                     + " section heading has no room for both spans: "
                     + section.displayTitle));
        expect (titleSize <= panel::headerHeight * scale + 0.1f,
                say (juce::String (sizeName)
                     + " section heading is vertically clipped: "
                     + section.displayTitle));
    };
    ...
    const float maximumScale = juce::jmin (
        static_cast<float> (panel::maximumEditorWidth) / panel::panelWidth(),
        static_cast<float> (panel::maximumEditorHeight) / panel::editorHeight);
    for (const auto& section : panel::sections())
    {
        ...
        expectHeaderFits (section, maximumScale, "maximum-size");
    }
...
void testQualitySelectorDrivesTheEngine()
{
    ...
    if (box == nullptr)
        return;
    expect (box->getNumItems() == YouKnowAudioProcessor::qualityChoiceCount,
    ...
    for (int choice = YouKnowAudioProcessor::qualityChoiceCount; --choice >= 0;)
    {
        const int factor =
    ...
    // And back up again, so the ladder is exercised in both directions.
    box->setSelectedItemIndex (YouKnowAudioProcessor::qualityChoiceCount - 1,
                                            ...
                                            juce::Component& parent) -> void
    {
        for (int index = 0; index < parent.getNumChildComponents(); ++index)
        {
            ...
            const bool isInteractive =
                ...
                || dynamic_cast<YouKnowPerformanceLever*> (component) != nullptr;
            if (isInteractive)
            {
                ++interactiveCount;
...
void testHelpStripPrintsEveryExplanationInFull()
{
    ...
    auto* editor = dynamic_cast<YouKnowAudioProcessorEditor*> (base.get());
    expect (editor != nullptr, "cannot test the contextual-help strip");
    if (editor == nullptr)
        return;

    auto* help = dynamic_cast<YouKnowContextHelp*> (
...
void testPersistentContextHelpAndValueBubbles()
{
    ...
    auto* ourEditor = dynamic_cast<YouKnowAudioProcessorEditor*> (editor.get());
    ...
    help->showNotice ("PATCH SAVED", "Saved the current tone.");
    help->showFor (patchBox);
    expect (help->getHelpTitle() == "PATCH SAVED"
                && help->getHelpText() == "Saved the current tone."
                && help->getHelpValue().isEmpty(),
            "hover polling hid the patch-file result before it could be read");
    help->showIdle();
    expect (help->getHelpTitle() == "PATCH SAVED",
            "idle polling hid the patch-file result before it could be read");
}
...
void testEditorContrastAndFocusContract()
{
    ...
    const auto headingInk = paletteColour (panel::colour::heading)
                                .withAlpha (0.90f);
    for (const auto colour : { panel::colour::faceplate, panel::colour::blueBar,
                               panel::colour::redBar })
    {
        const auto plate = paletteColour (colour);
        expect (contrastRatio (compositeOver (headingInk, plate), plate)
                    >= minimumTextContrast,
                "section-heading text falls below 4.5:1 on the panel or colour bars");
    }
    ...
    const auto focusSurfaces = std::to_array<juce::Colour> ({
        ...
        paletteColour (panel::colour::accentMuted),
        paletteColour (panel::colour::accent),
...
void testKeyboardFocusAndHoverShareContextHelp()
{
    ...
    auto* help = editor != nullptr
        ? dynamic_cast<YouKnowContextHelp*> (
              findDescendantNamed (*editor, "Context help"))
        : nullptr;
...
void testProgrammerRowKeysShareOneKeyFace()
{
    ...
    static constexpr const char* rowKeys[] {
        ...
        "Manual mode", "About YouKnow", "Save patch file", "Quick start",
        "Load patch file",
    };
    ...
    for (const char* name : rowKeys)
    {
        ...
        if (key->getName() == "About YouKnow" || key->getName() == "Quick start")
            expect (key->isEnabled() && static_cast<bool> (key->onClick),
                    key->getName().toStdString() + " has no working action");
...
{
    using Lever = YouKnowPerformanceLever;
    ...
    expect (std::abs (axes (true, false, false).bend + 1.0f) < 1.0e-6f,
...
// the utility-knob sweep below never reaches them. They share one aligned row
// with Unit Character and Aging inside MODEL, and none may crowd another.
                                    ...
                                    bool atSupportedMinimum)
{
    ...
    const auto characterArea = character != nullptr
        ...
        : juce::Rectangle<int>();
    auto* aging = findDescendantNamed (editor, "Aging");
    const auto agingArea = aging != nullptr
        ? editor.getLocalArea (aging, aging->getLocalBounds())
        : juce::Rectangle<int>();
    ...
    for (std::size_t index = 0; index < selectors.size(); ++index)
    {
        ...
        if (! agingArea.isEmpty())
            expect (! boxArea.intersects (agingArea)
                        && ! captionArea.intersects (agingArea),
                    context + selector.boxName + " overlaps the Aging knob");
        if (! characterArea.isEmpty())
            expect (std::abs (boxArea.toFloat().getCentreY()
                             - characterArea.toFloat().getCentreY()) <= 1.5f,
                    context + selector.boxName
                        + " is not vertically centred on the MODEL control row");
        ...
        // The child label is what actually paints the current selection. Its
        // font and bounds can differ from the look-and-feel's nominal budget.
        auto* selectedLabel = findDescendantLabelWithText (*box, box->getText());
        expect (selectedLabel != nullptr,
                context + selector.boxName + " has no selected-entry label");
        if (selectedLabel != nullptr)
            expect (selectedLabel->isVisible()
                        && static_cast<float> (selectedLabel->getHeight())
                               >= selectedLabel->getFont().getHeight()
                        && realTextWidth (selectedLabel->getFont(), selectedLabel->getText())
                               <= static_cast<float> (selectedLabel->getWidth()),
                    context + selector.boxName + " selected text \""
                        + selectedLabel->getText().toStdString()
                        + "\" is clipped in its actual label");

        // Every menu entry must fit the actual closed-box label, including
        // its left padding and the reserved arrow area.
        const auto& lookAndFeel = box->getLookAndFeel();
        const auto boxFont = const_cast<juce::LookAndFeel&> (lookAndFeel)
                                 .getComboBoxFont (*box);
        const float textWidth = selectedLabel != nullptr
            ? static_cast<float> (selectedLabel->getWidth()) : 0.0f;
                             ...
                             bool atSupportedMinimum)
{
    ...
    for (std::size_t index = 0; index < expected.size(); ++index)
    {
        ...
        expect (std::abs (sliderArea.getCentreX() - captionArea.getCentreX()) <= 1,
                context + " caption is not centred over its knob");
    ...
    checkInsideZone ("Quality label", panel::modelZoneX,
                     panel::modelZoneWidth, "Model");
    checkInsideZone ("VCF Solver", panel::modelZoneX,
                     panel::modelZoneWidth, "Model");
    checkInsideZone ("VCF Solver label", panel::modelZoneX,
                     panel::modelZoneWidth, "Model");
                                               ...
                                               const CompactLegend& legend)
    {
        ...
        const float textWidth = panel::compactLegendWidth (
            ...
            panel::editorScaleFor (editor.getWidth(), editor.getHeight()));
    ...
    const auto expectVerticalStack = [&] (const auto& legends,
                                           const char* groupName)
    {
        ...
        for (const auto& legend : legends)
        {
            ...
            const auto area = editor.getLocalArea (button,
                                                   button->getLocalBounds());
            if (! previousAction.isEmpty())
            {
                expect (area.getY() - previousAction.getBottom()
                            >= extensionPadding,
                        std::string (legend.componentName)
                            + " is too close to the preceding action");
                expect (std::abs (area.getX() - previousAction.getX()) <= 1
                            && std::abs (area.getRight()
                                         - previousAction.getRight()) <= 1,
                        std::string (groupName)
                            + " actions no longer share one vertical axis");
            }
            previousAction = area;
    ...
    expectVerticalStack (sessionLegends, "Session");
    expectVerticalStack (variationLegends, "Variation");
...
void testEditorBuildsAndRenders()
{
    ...
    const auto snapshotPath =
        juce::SystemStats::getEnvironmentVariable ("YOUKNOW_EDITOR_SNAPSHOT", {});
    const auto saveSnapshot = [&snapshotPath] (const juce::Image& snapshot,
                                               const char* suffix)
    {
        if (snapshotPath.isEmpty())
            return;

        const juce::File requestedFile { snapshotPath };
        const auto snapshotFile = suffix[0] == '\0' ? requestedFile
            : requestedFile.getSiblingFile (
                requestedFile.getFileNameWithoutExtension() + suffix + ".png");
        ...
        output.flush();
        expect (wrote, "could not write editor snapshot "
                           + snapshotFile.getFullPathName().toStdString());
    };
    ...
    auto* performanceLever = dynamic_cast<YouKnowPerformanceLever*> (
        findDescendantNamed (*editor, "Pitch and modulation lever"));
    auto* contextHelp = dynamic_cast<YouKnowContextHelp*> (
        findDescendantNamed (*editor, "Context help"));
    auto* transpose = dynamic_cast<juce::Slider*> (
        findDescendantNamed (*editor, "Transpose"));
    ...
    auto* logoLabel = findDescendantLabelWithText (*editor, "YouKnow");
    auto* makerLabel = findDescendantLabelWithText (*editor, "by Protocodus");
    const auto checkPresetToolbarLayout = [&editor]
    {
        const int minimumGap = juce::roundToInt (
            6.0f * panel::editorScaleFor (editor->getWidth(), editor->getHeight()));
        juce::Rectangle<int> previousArea;
        for (const auto* name : { "Patch label", "Previous patch", "Next patch",
                                  "Reload patch", "Patch selector", "Program state",
                                  "Load custom patch", "Save custom patch" })
        {
            auto* component = findDescendantNamed (*editor, name);
            expect (component != nullptr,
                    std::string ("the preset toolbar is missing ") + name);
            if (component == nullptr)
                continue;
            const auto area = editor->getLocalArea (component, component->getLocalBounds());
            expect (component->isVisible() && ! area.isEmpty()
                        && editor->getLocalBounds().contains (area)
                        && (previousArea.isEmpty()
                            || (area.getX() - previousArea.getRight() >= minimumGap
                                && std::abs (area.getCentreY()
                                             - previousArea.getCentreY()) <= 1)),
                    std::string (name)
                        + " lost the preset toolbar's spacing or centre alignment at "
    ...
    expect (customPatchLoad != nullptr && customPatchSave != nullptr,
    ...
    expect (logoLabel != nullptr,
            "the product wordmark is missing from the identity field");
    ...
    checkPresetToolbarLayout();
    if (logoLabel != nullptr)
        expect (labelTextFitsAtItsDeclaredSize (*logoLabel),
                "the YouKnow wordmark clips at the default editor size");
                                          ...
                                          panel::maximumEditorHeight } })
    {
        ...
        if (logoLabel != nullptr)
            expect (labelTextFitsAtItsDeclaredSize (*logoLabel),
                    "the YouKnow wordmark clips at "
                        + std::to_string (size.x) + "x"
                        + std::to_string (size.y));
        checkUtilityKnobLayout (*editor, size == supportedMinimum);
        checkProcessingSelectorLayout (*editor, size == supportedMinimum);
        ...
        if (playableKeyboard != nullptr)
        {
            ...
            if (patchSelector != nullptr && firstBankKey != nullptr)
            {
                ...
                if (customPatchLoad != nullptr && customPatchSave != nullptr)
                {
        ...
        saveSnapshot (snapshot, size == supportedMinimum ? "-minimum" : "-maximum");
    }
    ...
    // Reopen an edited factory patch so the captured lamps and program status
    // reflect real processor state without relying on a timer or a host window.
    if (snapshotPath.isNotEmpty())
    {
        ...
        editor.reset (processor.createEditor());
        expect (editor != nullptr, "cannot capture the edited-patch editor");
        if (auto* ourEditor = dynamic_cast<YouKnowAudioProcessorEditor*> (editor.get()))
        {
            auto* frequency = findDescendantNamed (*editor, "FREQ");
            ourEditor->refreshContextHelp (frequency, nullptr, true);
            saveSnapshot (renderEditorSnapshot (*editor), "-edited");
        }
    }
...
int main()
{
    ...
    if (std::getenv ("YOUKNOW_PRESET_TEST_ONLY") != nullptr)
    {
        ...
        testOriginalProductProgramsPreserveFactorySlotsAndState();
    ...
    if (failureCount != 0)
    {
        std::cerr << failureCount << " YouKnow plug-in check(s) failed.\n";
        return EXIT_FAILURE;
    }
    std::cout << "All YouKnow plug-in checks passed.\n";
    return EXIT_SUCCESS;
}
