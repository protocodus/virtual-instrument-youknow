...
| --- | --- | ---: | ---: | ---: |
| `01-chorus-pad.wav` | Saw and sub through the mode-I chorus: the classic pad, hiss and all | 21.9 s | −10.7 dBFS | +7.7 dB |
| `02-pwm-strings.wav` | Pulse-width-modulated strings in the faster mode-II chorus | 15.3 s | −12.5 dBFS | +9.5 dB |
...
| `06-chorus-modes.wav` | The same pad with the effect off, in mode I, mode II, then I+II | 20.5 s | −13.3 dBFS | +10.3 dB |
| `07-unison-glide.wav` | Six-voice unison lead with constant-rate portamento | 11.5 s | −8.3 dBFS | +5.3 dB |
...
Nothing is added to the sum. There is no mixer, no panner and no reverb; the
stereo image is the two antiphase chorus lines, the balance between parts is
each part's own output-volume position, and the only gain applied to the mix is
the single normalisation printed below. It renders at 24-bit/44.1 kHz on the
deepest oversampling rung, which an offline render can afford and a realtime
one cannot.

<!-- composition-table-begin: regenerated by YouKnowRenderComposition; edits between the markers are overwritten -->
*Low Sun* — D aeolian, 114 bpm, 103.2 s, 24-bit/44.1 kHz, 4x oversampled. Rendered mix peak −1.1 dBFS, normalised −1.9 dB.

| Part | Factory preset | Role | Max notes | Rendered peak |
| ---: | --- | --- | ---: | ---: |
| 1 | `A48` Synth Bass I (unison) | bass | 1 | −3.6 dBFS |
| 2 | `A67` Shaker | shaker | 1 | −28.8 dBFS |
| 3 | `A64` Snare Drum (unison) | snare | 1 | −14.8 dBFS |
| 4 | `A54` Funky II | stabs | 4 | −8.6 dBFS |
| 5 | `A28` Elect. Piano II | electric piano | 1 | −19.1 dBFS |
| 6 | `A11` Brass Set 1 | brass | 3 | −11.7 dBFS |
| 7 | `B11` Strings | strings | 3 | −14.7 dBFS |
| 8 | `A17` Choir | choir | 3 | −11.3 dBFS |
| 9 | `B13` Chorus Vibes | chorus vibes | 4 | −26.6 dBFS |
