...
struct YouKnowTestAccess
{
    ...
    {
        return youknow::YouKnowEngine::subMixVolts;
    }
                             ...
                             float duty) noexcept
    {
        ...
        const auto divider = static_cast<std::uint32_t>(std::llround(
            YouKnowEngine::rangeClockHz(range) / frequency));
    ...
    static float renderRecoveredMixed(YouKnowEngine& engine) noexcept
    {
        auto& voice = engine.voices_[0];
    ...
    static std::vector<float> decimate(YouKnowEngine& engine,
                                       ...
                                       int factor)
    {
        ...
        YouKnowEngine::HalfbandDecimator first;
        YouKnowEngine::HalfbandDecimator second;
        first.reset();
        second.reset();
    ...
    static ScanCursor scanCursor(const YouKnowEngine& engine) noexcept
    {
        return { engine.controlScanPhase_, engine.nextConverterWrite_ };
    }

    static void processOne(YouKnowEngine& engine) noexcept
    {
        float left = 0.0f;
        float right = 0.0f;
    ...
    static void prepareHoldStep(YouKnowEngine& engine,
                                double internalRate) noexcept
    {
        ...
        engine.nextConverterWrite_ = YouKnowEngine::converterWritesPerPass;
        auto& voice = engine.voices_[0];
        voice.dcoCv = 0.0f;
    ...
    static HoldState holdState(const YouKnowEngine& engine) noexcept
    {
    ...
    {
        return YouKnowEngine::dcoDivider(frequencyHz);
    }

    static double quantisedFrequency(std::uint32_t divider,
                                     DcoRange range) noexcept
    {
        return YouKnowEngine::dcoQuantisedFrequency(divider, range);
    }
};
} // namespace youknow
...
int main(int argc, char** argv)
{
    ...
    for (int index = 1; index < argc; ++index)
    {
        ...
        {
            std::cout << "usage: YouKnowDcoScanQualityAudit [--self-test]\n";
            return 0;
        }
        else
        {
