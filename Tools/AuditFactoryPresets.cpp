...
#ifndef YOUKNOW_FACTORY_AUDIT_DEFAULT_OUTPUT
#define YOUKNOW_FACTORY_AUDIT_DEFAULT_OUTPUT "Docs/audio/factory-presets"
#endif
...
// enforced below; Source/DSP/YouKnowPresets.cpp carries the per-preset VR1
...
// (YouKnowEngine::outputLevelPolicyDb). This is an ABSOLUTE figure, so it
...
std::string toneHex (const Preset& preset)
{
    ...
    youknow::sysex::toneBytesFromPatch (preset.patch, bytes.data());
                        ...
                        double commonPreviewGain, double corpusMedianDb)
{
    ...
    report << ownershipMarker << "\n\n# YouKnow factory-preset gain audit\n\n"
           << "Generated by `YouKnowAuditFactoryPresets` from the immutable "
              ...
              "positions are listed in `Source/DSP/YouKnowPresets.cpp` and "
...
bool ensureOwnedDirectory (const std::filesystem::path& directory)
{
    ...
    return writeText (marker, "youknow factory preset audit output v1\n");
}
         ...
         bool writeStandaloneReport)
{
    ...
    for (const auto& preset : bank)
        if (preset.controls.volume
            > youknow::presets::Preset::Controls {}.volume + 1.0e-6f)
        {
            std::fprintf (stderr,
